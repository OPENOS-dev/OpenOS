...
class RunNonOpenOSPostInstallTest : public PostInstallTest {};

// Test that RunNonOpenOSPostInstall fails with bios_type kSecure.
TEST_F(RunNonOpenOSPostInstallTest, ErrorSecure) {
  install_config_.bios_type = BiosType::kSecure;
  EXPECT_FALSE(RunNonOpenOSPostInstall(platform_, install_config_));
}

// Test a successful call to RunNonOpenOSPostInstall with bios_type
// kLegacy.
TEST_F(RunNonOpenOSPostInstallTest, Legacy) {
  install_config_.bios_type = BiosType::kLegacy;
  EXPECT_TRUE(RunNonOpenOSPostInstall(platform_, install_config_));
...
// kLegacy.
TEST_F(RunNonOpenOSPostInstallTest, ErrorLegacy) {
  install_config_.bios_type = BiosType::kLegacy;
  CHECK(brillo::DeleteFile(rootfs_boot_.Append("syslinux/root.A.cfg")));
...
// kLegacy.
TEST_F(RunNonOpenOSPostInstallTest, LegacyNonFatalUefiError) {
  install_config_.bios_type = BiosType::kLegacy;
  CHECK(brillo::DeletePathRecursively(esp_.Append("efi/boot")));
...
// Test a successful call to RunNonOpenOSPostInstall with bios_type
// kEFI.
TEST_F(RunNonOpenOSPostInstallTest, Uefi) {
  install_config_.bios_type = BiosType::kEFI;
  EXPECT_TRUE(RunNonOpenOSPostInstall(platform_, install_config_));
...
// Test that RunNonOpenOSPostInstall conditionally deletes legacy
// kernels.
TEST_F(RunNonOpenOSPostInstallTest, UefiKernelDelete) {
  CHECK(base::WriteFile(esp_.Append("syslinux/vmlinuz.A"), "kern_a"));
  CHECK(base::WriteFile(esp_.Append("efi/boot/crdybootx64.efi"), "crdyboot"));
  install_config_.bios_type = BiosType::kEFI;
  install_config_.is_update = true;
...
// kEFI.
TEST_F(RunNonOpenOSPostInstallTest, ErrorUefi) {
  install_config_.bios_type = BiosType::kEFI;
  CHECK(brillo::DeletePathRecursively(esp_.Append("efi/boot")));
...
// bios_type kEFI.
TEST_F(RunNonOpenOSPostInstallTest, UefiNonFatalLegacyError) {
  install_config_.bios_type = BiosType::kEFI;
  CHECK(brillo::DeleteFile(rootfs_boot_.Append("syslinux/root.A.cfg")));
