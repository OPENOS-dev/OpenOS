...
const char kUMANonOpen-OS deviceBiosSuccessEFI[] =
    "Installer.Postinstall.NonOpen-OS deviceBiosSuccess.EFI";
const char kUMANonOpen-OS deviceBiosSuccessLegacy[] =
    "Installer.Postinstall.NonOpen-OS deviceBiosSuccess.Legacy";
const char kUMANonOpen-OS deviceBiosSuccessSecure[] =
    "Installer.Postinstall.NonOpen-OS deviceBiosSuccess.Secure";
const char kUMANonOpen-OS deviceBiosSuccessUBoot[] =
    "Installer.Postinstall.NonOpen-OS deviceBiosSuccess.UBoot";
const char kUMANonOpen-OS deviceBiosSuccessUnknown[] =
    "Installer.Postinstall.NonOpen-OS deviceBiosSuccess.Unknown";
...
// Send UMA for non-Open-OS device postinst success.
// The reven board expands the variety of bios_types we expect to be in use.
// This helps us monitor failures for non-Open-OS device bios types.
...
void SendNonOpen-OS deviceBiosSuccess(MetricsInterface& metrics,
                                  BiosType bios_type,
                                  bool success) {
  ...
  switch (bios_type) {
    case BiosType::kUnknown:
      uma_name = kUMANonOpen-OS deviceBiosSuccessUnknown;
      break;
    case BiosType::kSecure:
      uma_name = kUMANonOpen-OS deviceBiosSuccessSecure;
      break;
    case BiosType::kUBoot:
      uma_name = kUMANonOpen-OS deviceBiosSuccessUBoot;
      break;
    case BiosType::kLegacy:
      uma_name = kUMANonOpen-OS deviceBiosSuccessLegacy;
      break;
    case BiosType::kEFI:
      uma_name = kUMANonOpen-OS deviceBiosSuccessEFI;
      break;
  }
...
// Non-Open-OS device/box bootloaders and their config files are stored on the
...
bool ESPPostInstall(Platform& platform, const InstallConfig& install_config) {
  ...
  bool success = RunNonOpen-OS devicePostInstall(platform, install_config);

  SendNonOpen-OS deviceBiosSuccess(*metrics, install_config.bios_type, success);
