...
// Copyright 2026 OCS (Open Code Studio) - OPENOS modifications
...
/// OPENOS first-boot flag path.
const FIRST_BOOT_FLAG: &str = "/var/lib/openos/first-boot-done";

/// OPENOS welcome animation script.
const WELCOME_SCRIPT: &str = "/usr/bin/openos-welcome";

fn intro() {
    // OPENOS: Run the first-boot TUI animation, or static banner on subsequent boots.
    ...
    println!(
        r#"Welcome to crosh, the OPENOS developer shell.
...
fn main() -> Result<(), ()> {
    ...
    // OPENOS: Always enable developer mode commands.
    // Developer mode is the default and only mode of operation.
