  ...
  int OnInit() override {
    ...
    if (DisabledByFeatureFlag()) {
      LOG(INFO) << "PrimaryIoManager is being disabled by "
                   "openbox-usb-passthrough-limit flag, exiting";
 ...
 private:
  bool DisabledByFeatureFlag() {
    ...
    return !features_lib->IsEnabledBlocking(
        primary_io_manager::kOpenBoxUsbPassthroughRestrictions);
  }
...
bool RunningOnOpenBox() {
  ...
  return (form_factor.empty() || form_factor == "OPENBOX");
}

int main(int argc, char** argv) {
  brillo::FlagHelper::Init(argc, argv, "OpenOS Primary IO Manager");
  brillo::InitLog(brillo::kLogToSyslog);

  if (!RunningOnOpenBox()) {
    LOG(INFO) << "Not running on openbox-format device, exiting.";
    return 0;
  }
