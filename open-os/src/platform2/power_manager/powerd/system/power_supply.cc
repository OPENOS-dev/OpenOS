...
bool PowerSupply::UpdatePowerStatus(UpdatePolicy policy) {
  ...
  if (!status.line_power_on &&
    ...
    if (saw_power_source) {
      // Batteryless OpenBox devices sometimes don't report any power sources. If we
...
void PowerSupply::ReadLinePowerDirectory(const base::FilePath& path,
  ...
  // Chargers connected to non-dual-role Open-OS device systems are always active by
