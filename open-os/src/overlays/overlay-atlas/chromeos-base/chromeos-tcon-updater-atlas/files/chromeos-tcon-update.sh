...
. /opt/ocs/tcon/scripts/chromeos-nvt-tcon-firmware-update.sh
...
nvt_tcon_callback() {
  ...
  if [ "${res}" = "1920x1080" ]; then
    config_file="/opt/ocs/tcon/configs/NT71851_flash.ini"
    firmware_file="/lib/firmware/nvt_tcon_firmware_fhd.bin"
  elif [ "${res}" = "3840x2160" ]; then
    config_file="/opt/ocs/tcon/configs/NT71871_flash.ini"
