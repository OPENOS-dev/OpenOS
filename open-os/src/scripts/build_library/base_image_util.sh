...
install_libc_for_abi() {
  ...
  local idx pkg pkg_cp pkg_atom pkg_path pkg_excludes
  for (( idx = 0; idx < ${#pkgs[@]}; idx++ )); do
    pkg="${pkgs[${idx}]}"
    pkg_cp="cross-${abi}/${pkg}"
    pkg_atom="$(info_run portageq match / "${pkg_cp}")"
...
create_base_image() {
  ...
  if should_build_image "${CHROMEOS_FACTORY_INSTALL_SHIM_NAME}"; then
    info "Skip creating dev_image.blocks in factory shim environment."
  else
    if has "default_key_stateful" "$(portageq-"${BOARD}" envvar USE)"; then
      create_developer_tools_image "${image_name}"
    fi
  fi
  ...
  if should_build_image "${CHROMEOS_FACTORY_INSTALL_SHIM_NAME}"; then
    ...
    # to the image.
    info_run emerge_custom_kernel "${root_fs_dir}"
  fi
  ...
  info_run emerge_to_image --root="${root_fs_dir}" ${BASE_PACKAGE}
  ...
  if type get_arc_build_info &>/dev/null; then
    ...
    info_run get_arc_build_info "${root_fs_dir}"
  fi
  local arc_flags=()
  ...
  if type board_finalize_base_image &>/dev/null; then
    info_run board_finalize_base_image || die "board_finalize_base_image failed"
  fi
  ...
  if should_build_image "${CHROMEOS_FACTORY_INSTALL_SHIM_NAME}"; then
    info "Skipping minios for factory shim images."
  elif has "minios" "$(portageq-"${BOARD}" envvar USE)"; then
