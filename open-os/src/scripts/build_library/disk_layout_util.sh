...
get_verity_hash_algo() {
  get_disk_layout_path

  cgpt_py readverityhashalgo "${DISK_LAYOUT_PATH}"
}
...
EOF
      if [[ "${label}" == "STATE" ]]; then
        cat <<-EOF >>"${mount}"
if [[ -e "${dir}/unencrypted/dev_image.block" ]]; then
  m=( sudo mount "${dir}/unencrypted/dev_image.block" "${dir}/developer_tools" )
  if ! "\${m[@]}"; then
    if ! "\${m[@]}" -o ro; then
      exit 0
    fi
  fi
  m=( sudo mount --bind "${dir}/developer_tools/dev_image" "${dir}/dev_image" )
  if ! "\${m[@]}"; then
    exit 0
  fi
  m=( sudo mount --bind "${dir}/developer_tools/var_overlay" "${dir}/var_overlay" )
  if ! "\${m[@]}"; then
    exit 0
  fi
fi
...
mk_fs() {
  ...
  if [ "${fs_label}" = "STATE" ]; then
    ...
    cmds+=("sudo mkdir -p '${mount_dir}/mnt/stateful_partition'"
           "sudo mkdir -p '${mount_dir}/mnt/chromeos_metadata_partition'"
