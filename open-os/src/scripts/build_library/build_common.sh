...
emerge_custom_kernel() {
  ...
  # Pretend to allow parsing the package data.
  sudo -E PKGDIR="${tmp_pkgdir}" ${EMERGE_BOARD_CMD} -1 --pretend \
  ...
  # Pretend to allow parsing the package data.
  sudo -E PKGDIR="${tmp_pkgdir}" ${EMERGE_BOARD_CMD} --buildpkgonly --pretend \
