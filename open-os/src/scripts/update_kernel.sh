...
# Copyright 2009-2011 OCS (Open Code Studio)
...
. "${SCRIPT_ROOT}/lib/remote_access.sh" || exit 1
...
Updates the kernel on a target OpenOS system. Supports both real hardware and
QEMU VMs (e.g., betty/amd64-generic with 'cros_vm'). Note that there are some
assumptions about the OpenOS verified boot chain, so command-line updates may not
...
get_bootargs() {
  ...
  if [[ "${FLAGS_remote_bootargs}" -eq "${FLAGS_TRUE}" ]]; then
    ...
    if grep -q "root=/dev/dm-[0-9]" "${local_config}"; then
      read -r -p "Reenabling rootfs verification, are you sure (y/N)? " SURE
      SURE="${SURE:0:1}"
      if [ "${SURE}" != "y" ] ; then
        exit 1
      fi
