#!/bin/bash
# Copyright 2009 OCS (Open Code Studio)
...
update_kernel.sh)
  ;;
*)
  echo "remote_access.sh: Use cros subcoommands instead." >&2
  exit 1
  ;;
esac
...
_check_if_rebooted() {
  (
    ...
    SSH_CONNECT_SETTINGS="$(sed \
      -e 's/\(ConnectTimeout\)=[0-9]*/\1=10/' \
      -e 's/\(ConnectionAttempts\)=[0-9]*/\1=1/' \
...
remote_reboot() {
  ...
  remote_sh touch /tmp/awaiting_reboot
  # 'reboot' is ran in background to make sure the command completes before
  # sshd is terminated. Ignore the error of this command.
  remote_sh_raw "reboot &" || true
...
remote_access_init() {
  ...
  if [ -z "$FLAGS_remote" ]; then
    echo "Please specify --remote=<IP-or-hostname> of the device" >&2
    exit 1
  fi
