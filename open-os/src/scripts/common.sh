...
die_err_trap() {
  ...
  if [[ ! -o errexit ]]; then
    # Ignore the error trap when errexit is not set
    # Let callers able to use 'set +e' / 'set -e' to wrap codes where the
    # errors are expected to be ignored and prevent from dying.
    return 0
  fi
...
FLEXOR_KERNEL_NAME="flexor_vmlinuz"
