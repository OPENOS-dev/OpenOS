# Analyze
Analyze is a command-line tool for analyzing ApiTrace profiles collected with
Harvest.

## Producing profiles
Profiles are generated by running an apitrace command, such as `glretrace`, with
a trace file on a target device. Analyze can parse profile info with GPU and CPU
timing information. To produce such files, you must run `glretrace` or
`eglretrace` with options `--pcpu` and `--pgpu`. Most of the time, you'll also
want to use option `--min-cpu-time=0` to ensure that the profile includes all calls.
Without it, only calls taking 1 microsecond or more will be captured.
...
(e)glretrace insert query operations around each call. Round-tripping this query
...
Profiles can be very large and it might take a few minutes for analyze to load
them. Once that is done, analyze enters console mode:
...
on different devices. Run analyze with the two profiles as follows:
...
still load successfully in analyze, but they will be harder to compare accurately.
...
- https://docs.google.com/document/d/1H4DUKunAlOi4dx8LKF9Wf8uPgU9GQagmI43Dg_Bn8_k
- https://docs.google.com/document/d/1xaEX_F2j1voQ-KQd1pdK1Jb6EGJdqYf4vAfiqIcRslA
- https://docs.google.com/document/d/1S7DWSHnnCaNKmZ9nCEkI3qflrAdiA-fiB0ifQK6QJwk
- https://docs.google.com/document/d/1Spa90owZgbhggcDczIjZ2qzFsPjTOHjTKsqpx-Alcrc
