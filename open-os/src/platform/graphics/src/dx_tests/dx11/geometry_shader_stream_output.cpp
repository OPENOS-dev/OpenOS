...
TEST_F(streamoutputgs, geometryShaderStreamOutput) {
  RunTest([](ID3D11Device* device, ID3D11DeviceContext* context,
    ...
    ShaderOutput<ID3D11GeometryShader> gs0;
    ...
    ASSERT_EQ(S_OK, device->CreateGeometryShaderWithStreamOutput(
                        gs0.blob_output.data(), gs0.blob_output.size(), pDecl, 1,
                        NULL, 0, D3D11_SO_NO_RASTERIZED_STREAM, NULL, &gs.get()));
