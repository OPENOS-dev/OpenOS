...
ControlUnit::init()
{
    ...
    __attribute__((unused)) bool supportDigiGain = false;
    if (cap)
...
{
    status_t status = NO_ERROR;
    const MediaCtlConfig *mediaCtlConfig = mStreamCfgProv.getMediaCtlConfig(IStreamConfigProvider::IMGU_VIDEO);
    for (unsigned int i = 0; i < mediaCtlConfig->mSelectionParams.size(); i++) {
        ...
        if (strstr(param.entityName.c_str(), MEDIACTL_IMGUNAME)) {
            switch (param.pad) {
...
ControlUnit::notifyCaptureEvent(CaptureMessage *captureMsg)
{
    ...
    switch (captureMsg->data.event.type) {
        ...
        case CAPTURE_REQUEST_DONE:
        {
            ...
            msg.requestId = captureMsg->data.event.reqId;
            base::OnceCallback<status_t()> closure =
                base::BindOnce(&ControlUnit::handleRequestDone,
                               base::Unretained(this), std::move(msg));
            mCameraThread.PostTaskAsync<status_t>(FROM_HERE,
                                                  std::move(closure));
            break;
