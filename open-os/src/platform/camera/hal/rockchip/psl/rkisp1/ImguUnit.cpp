...
ImguUnit::~ImguUnit()
{
    ...
    if (mPollerThreadMeta) {
        mPollerThreadMeta->requestExitAndWait();
...
status_t ImguUnit::mapStreamWithDeviceNode()
{
    ...
    int previewIdx = -1;
    int listenerIdx = -1;
    __attribute__((unused)) bool isVideoSnapshot = false;
    NodeTypes listenToNode = IMGU_NODE_NULL;

    if (streamNum == 1) {
        // Force use video, rk use the IMGU_NODE_VIDEO firstly.
        // if second stream is needed, then IMGU_NODE_PREVIEW will be usde.
        videoIdx = 0;
    } else if (streamNum == 2) {
        ...
        if (streamSizeEQ(availableStreams[0], availableStreams[1])
            ...
            if (streamSizeGT(availableStreams[0], availableStreams[previewIdx])) {
                ...
                listenToNode = IMGU_NODE_PREVIEW;
            } else {
                videoIdx = previewIdx;
    ...
    if (previewIdx >= 0) {
        mStreamNodeMapping[IMGU_NODE_PREVIEW] = availableStreams[previewIdx];
...
ImguUnit::createProcessingTasks(std::shared_ptr<GraphConfig> graphConfig)
{
    ...
    status = mMediaCtlHelper.configure(mGCM, IStreamConfigProvider::IMGU_VIDEO);
    if (status != OK)
        return UNKNOWN_ERROR;
    ...
    for (const auto &it : mConfiguredNodesPerName) {
        ...
        } else if (it.first == IMGU_NODE_VIDEO) {
        ...
        } else if (it.first == IMGU_NODE_PREVIEW) {
            std::shared_ptr<OutputFrameWorker> vfWorker =
                std::make_shared<OutputFrameWorker>(it.second, mCameraId,
                    mStreamNodeMapping[it.first], it.first, pipelineDepth);
            mPipeConfig.deviceWorkers.insert(mPipeConfig.deviceWorkers.begin(), vfWorker);
            mPipeConfig.pollableWorkers.insert(mPipeConfig.pollableWorkers.begin(), vfWorker);
            mPipeConfig.nodes.insert(mPipeConfig.nodes.begin(), vfWorker->getNode());
                          ...
                          bool updateMeta)
{
    ...
    base::OnceCallback<status_t()> closure = base::BindOnce(
        &ImguUnit::handleCompleteReq, base::Unretained(this), std::move(msg));
...
status_t ImguUnit::notifyPollEvent(PollEventMessage *pollMsg)
{
    ...
    if (pollMsg->id == POLL_EVENT_ID_EVENT) {
        ...
        base::OnceCallback<status_t()> closure = base::BindOnce(
            &ImguUnit::handlePoll, base::Unretained(this), std::move(msg));
...
ImguUnit::flush(void)
{
    ...
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&ImguUnit::handleFlush, base::Unretained(this));
    mCameraThread.PostTaskSync<status_t>(FROM_HERE, std::move(closure),
                                         &status);
    return status;
}
