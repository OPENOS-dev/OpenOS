...
StreamingFeaturePipeUsage::StreamingFeaturePipeUsage(UsageHint hint,
  ...
  mEnableFake = (getPropertyValue(KEY_ENABLE_FAKE, SUPPORT_FAKE_NODE) == 1);
...
MBOOL StreamingFeaturePipeUsage::supportFake() const {
  return mEnableFake;
}
...
MSize StreamingFeaturePipeUsage::getRrzoSizeByIndex(MUINT32 index) {
  ...
  if (index >= mResizedRawSizeList.size()) {
    MY_LOGE("index(%d) >= resized raw size list(%zu)", index,
