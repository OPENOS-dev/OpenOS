...
#include "IIBuffer.h"
#include "BuiltinBufferWrapper.h"
...
class GraphicBufferHandle : public BufferHandle<GraphicBufferHandle> {
 ...
 public:
  ...
  std::shared_ptr<BuiltinBufferWrapper> mGraphicBuffer;
...
class GraphicBufferPool : public BufferPool<GraphicBufferHandle> {
 ...
 public:
  class BufferInfo {
   public:
    std::shared_ptr<BuiltinBufferWrapper> mGraphic;
    std::shared_ptr<IImageBuffer> mImage;
    MUINT32 mSize;

    BufferInfo() {}

    BufferInfo(std::shared_ptr<BuiltinBufferWrapper> graphic,
 ...
 public:
  static std::shared_ptr<GraphicBufferPool> create(
      ...
      MUINT32 usage,
      BuiltinBufferWrapper::ColorSpace color = BuiltinBufferWrapper::NOT_SET);
 ...
 protected:
  ...
  MBOOL init(MUINT32 width,
             ...
             android_pixel_format_t format,
             MUINT32 usage,
             BuiltinBufferWrapper::ColorSpace color);
  MVOID uninit();
 ...
 private:
  ...
  size_t mStride[3];
  size_t mBoundary[3];
  BuiltinBufferWrapper::ColorSpace mColorSpace;
