...
int IspMgrIpcServerAdapter::create(void* addr, int dataSize) {
  ...
  CheckAndLogError((dataSize < sizeof(ispmgr_create_params)), UNKNOWN_ERROR,
...
int IspMgrIpcServerAdapter::querylcso(void* addr, int dataSize) {
    CheckAndLogError((dataSize < sizeof(ispmgr_querylcso_params)),
                     UNKNOWN_ERROR, "@%s, buffer is small", __FUNCTION__);

    ispmgr_querylcso_params* params =
        static_cast<ispmgr_querylcso_params*>(addr);
...
int IspMgrIpcServerAdapter::ppnr3d(void* addr, int dataSize) {
    CheckAndLogError((dataSize < sizeof(ispmgr_ppnr3d_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
