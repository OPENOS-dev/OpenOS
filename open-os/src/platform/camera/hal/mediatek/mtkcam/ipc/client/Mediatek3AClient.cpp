...
Mediatek3AClient::Mediatek3AClient()
  ...
  mCallback = base::BindRepeating(&Mediatek3AClient::callbackHandler,
                                  base::Unretained(this));
  Mediatek3AClient::return_callback = returnCallback;

  mNotifyCallback = base::BindRepeating(&Mediatek3AClient::notifyHandler,
                                        base::Unretained(this));
...
int Mediatek3AClient::allocateShmMem(std::string const& name,
  ...
  *addr = nullptr;
  ...
  CheckAndLogError((ret == -1), UNKNOWN_ERROR, "@%s, call ftruncate fail",
                   __FUNCTION__);
...
int Mediatek3AClient::Runner::requestSync(IPC_CMD cmd,
  ...
  CheckAndLogError((ret != OK), UNKNOWN_ERROR,
                   "@%s, cmd:%d call waitCallback fail", __FUNCTION__, cmd);

  LOG1("@%s, cmd:%d, group:%d, mCbResult:%d, done!", __FUNCTION__, cmd, mGroup,
       mCbResult);

  // check callback result
  CheckAndLogError((mCbResult != true), UNKNOWN_ERROR,
                   "@%s, %s(%d) callback fail", __FUNCTION__,
                   Mediatek3AIpcCmdToString(cmd).c_str(), cmd);
