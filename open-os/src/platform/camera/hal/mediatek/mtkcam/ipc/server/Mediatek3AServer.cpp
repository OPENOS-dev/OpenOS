...
int32_t Mediatek3AServer::initialize(
  ...
  CheckAndLogError((!callback_ops), -EINVAL, "@%s, the callback_ops is nullptr",
                   __FUNCTION__);
...
int32_t Mediatek3AServer::registerBuffer(int buffer_fd) {
  ...
  CheckAndLogError((addr == reinterpret_cast<void*>(-1)), -EBADFD,
...
void Mediatek3AServer::request(uint32_t req_id,
  ...
  mThreads[servGroup]->task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&Mediatek3AServer::handleRequest,
