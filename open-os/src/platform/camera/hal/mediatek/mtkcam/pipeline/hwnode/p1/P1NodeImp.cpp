...
P1NodeImp::fetchJob(P1QueJob* rOutJob) {
  ...
  mTagList.set(cnt);
  if (rOutJob->empty()) {
    MY_LOGI("using-fake-request");
    if (2 <= mLogLevelI) {
      mpTaskCollector->dumpRoll();
    }
    //
    P1TaskCollector FakeCollector(mpTaskCtrl);
    for (int i = 0; i < mBurstNum; i++) {
      P1QueAct newAct;
      FakeCollector.enrollAct(&newAct);
      createAction(&newAct, nullptr, REQ_TYPE_FAKE);
      FakeCollector.verifyAct(&newAct);
    }
    FakeCollector.requireJob(rOutJob);
  }
  ...
  if (EN_INIT_REQ_RUN && (mInitReqCnt < mInitReqNum)) {
    ...
    if (mp3A && IS_BURST_OFF && mFirstReceived && (pSetAct != nullptr)) {
      if (preSet.PreSetKey <= P1_PRESET_KEY_NULL) {
        ...
        if (preSet.Fake > 0) {
          MY_LOGI("Pre-Set-Meta is fake (%d)", preSet.Fake);
        }
        ...
        if (1 <= mLogLevelI) {  // P1_LOGI(1)
          pAct->msg += base::StringPrintf(
              " | [PreSet][Key:%d] Num(%d) "
              "fake(%d) MetaCnt[APP:%d,HAL:%d]",
              preSet.PreSetKey, preSet.MagicNum, preSet.Fake,
...
void P1NodeImp::ensureStartReady(MUINT8 infoType, MINT32 infoNum) {
  ...
  if ((getActive()) && (!getReady())) {
    MY_LOGE("Wait StartReady Timeout (%d*%d ms) - "
            ...
            infoNum, EN_START_CAP, EN_INIT_REQ_RUN, getStartState(),
            static_cast<int>(res), needRetry);
  }
  return;
}
...
P1NodeImp::onProcessDequeFrame() {
      ...
      deqBuf.mvOut[0].mBuffer, deqBuf.mvOut[0].mMetaData.mMagicNum_hal, &job);
  {
    ...
    while ((match) && (mTransferJobIdx != P1ACT_ID_NULL) &&
      ...
      needRetry--;
      MY_LOGI("TransferJob(%d) ThisJob(%d) - WaitStatus(%d) "
              "NeedRetry(%d)",
              mTransferJobIdx, job.getIdx(), static_cast<int>(res), needRetry);
      if (res != std::cv_status::timeout) {
        MY_LOGI("Got Job");
        break;
      }
    ...
    if ((res == std::cv_status::timeout) && (mTransferJobIdx == job.getIdx())) {
      MY_LOGE("TransferJob(%d) Not-Ready : (%d)", mTransferJobIdx,
              static_cast<int>(res));
    }
  }
    ...
    if (match && (act->expRec != EXP_REC_NONE)) {
      switch (act->reqType) {
        case REQ_TYPE_NORMAL:
          ...
          break;
        default:  // REQ_TYPE_INITIAL/REQ_TYPE_FAKE/REQ_TYPE_PADDING
      ...
      if (!act->getFlush()) {  // if flush type did not set
        if (act->getType() == ACT_TYPE_INTERNAL) {
          if (act->reqType == REQ_TYPE_INITIAL) {
            ...
            type = FLUSH_PADDING;
          } else if (act->reqType == REQ_TYPE_FAKE) {
            type = FLUSH_FAKE;
          } else {
...
P1NodeImp::hardwareOps_enque(P1QueJob* job, ENQ_TYPE type, MINT64 data) {
  ...
  if (EN_START_CAP && (!getReady()) && (type == ENQ_TYPE_NORMAL)) {
    ...
    while (mStartCaptureState == START_CAP_STATE_WAIT_CB) {
      ...
      if (res == std::cv_status::timeout) {
        MY_LOGI("StartCap(%d) Cnt(%d) Res(%d)", mStartCaptureState, cnt,
                static_cast<int>(res));
...
P1NodeImp::hardwareOps_deque(QBufInfo* deqBuf) {
  ...
  for (size_t i = 0; i < deqBuf->mvOut.size(); i++) {
    MY_LOGI(
        ...
        "%zu, mMetaData.mDstSize.w %d, mMetaData.mDstSize.h %d",
...
P1NodeImp::createAction(P1QueAct* rAct,
  ...
  if (act->reqType == REQ_TYPE_NORMAL || act->reqType == REQ_TYPE_INITIAL ||
      act->reqType == REQ_TYPE_PADDING || act->reqType == REQ_TYPE_FAKE) {
    if (IS_PORT(CONFIG_PORT_IMGO, mConfigPort) &&
  ...
  if (1 <= mLogLevelI) {  // P1_LOGI(1)
    ...
    act->msg += base::StringPrintf(
        ...
        (appFrame != nullptr) ? "New-Request" : "New-fake", eType);
    act->msg += info;
