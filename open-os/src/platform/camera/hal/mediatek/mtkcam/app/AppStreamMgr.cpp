...
NSCam::v3::Imp::AppStreamMgr::checkStreams(
  ...
  for (size_t i = 0; i < stream_list->num_streams; i++) {
    ...
    if (stream_list->streams[0]->crop_rotate_scale_degrees!=
    stream_list->streams[i]->crop_rotate_scale_degrees) {
      MY_LOGE("streams[%zu] crop_rotate_scale_degrees : %d inconsistency", i,
      stream_list->streams[i]->crop_rotate_scale_degrees);
      return -EINVAL;
    }
    //
...
NSCam::v3::Imp::AppStreamMgr::createRequest(camera3_capture_request_t* request,
  ...
  for (size_t i = 0; i < request->num_output_buffers; i++) {
    ...
    //
    if (pStreamBuffer == nullptr) {
      MY_LOGE("NULL AppImageStreamBuffer of request->output_buffers[%zu]", i);
      return -EINVAL;
    }
    ...
    for (size_t i = 0; i < cbParcel.vOutputImageItem.size(); i++) {
      ...
      if (HAL_PIXEL_FORMAT_BLOB == rDst.stream->format &&
        ...
        if (pImageBufferHeap->lockBuf(
          ...
          MY_LOGD("jpegBuf:%#" PRIxPTR " bufsize:%d datasize:%zu", jpegBuf,
