...
HalSensorList::searchSensors() {
  ...
  for (MUINT i = IMGSENSOR_SENSOR_IDX_MIN_NUM; i < IMGSENSOR_SENSOR_IDX_SUB;
   ...
   if (getSensorName((IMGSENSOR_SENSOR_IDX)i)) {
      addAndInitSensorEnumInfo_Locked(
        ...
        CAM_LOGE("Can't get sensor name, something wrong.");
        return 0;
    }
  }
  ...
  if (CC_UNLIKELY(pIPCHalSensorList == nullptr)) {
    ...
    for (size_t i = 0; i < mEnumSensorList.size(); i++) {
      ...
      CAM_LOGD("IPCHalSensorList: sensor (idx,type,deviceid)=(%#zx, %#x, %#x)",
               i, type, deviceId);
    }
  ...
  for (i = 0; i < num; i++) {
    ...
    if (sensor_id == psensor_list->id) {
      const char* sensor_name =
          reinterpret_cast<const char*>(psensor_list->name);
      CAM_LOGI("sensorName %d %d %s", idx, i, sensor_name);
      return sensor_name;
    }
  }
  return NULL;
