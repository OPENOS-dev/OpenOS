...
public:
    ...
    static status_t cropFrame(std::shared_ptr<T> srcBuf, std::shared_ptr<T> dstBuf) {
        ...
        int ret = libyuv::NV12ToI420(srcBufY, srcW,
                                     ...
                                     srcI420BufV, srcW / 2,
                                     srcW, srcH);
        CheckAndLogError((ret != 0), UNKNOWN_ERROR, "@%s, NV12ToI420 fails",
                         __FUNCTION__);
        ...
        int left = (srcBuf->width() - dstBuf->width()) / 2;
        int top = (srcH - dstH) / 2;
        ret = libyuv::ConvertToI420(static_cast<uint8_t*>(srcI420Buf.get()), srcI420BufSize,
                                    ...
                                    left, top,
                                    srcW, srcH, dstBuf->width(), dstH,
                                    libyuv::RotationMode::kRotate0, libyuv::FourCC::FOURCC_I420);
        CheckAndLogError(ret != 0, UNKNOWN_ERROR, "@%s, ConvertToI420 fails",
                         __FUNCTION__);
    ...
    static status_t rotateFrame(std::shared_ptr<T> input, std::shared_ptr<T> output,
        ...
        CheckAndLogError((angle < 0 || angle >= 360 || angle % 90 != 0),
                         BAD_VALUE, "angle value:%d is wrong", angle);
        ...
        if (rotationModeMap[angle] == libyuv::RotationMode::kRotate0) {
            ...
            CheckAndLogError((ret < 0), UNKNOWN_ERROR, "@%s, rotate fail [%d]!",
                             __FUNCTION__, ret);
            ...
            ret = libyuv::I420ToNV12(I420Buffer, outStride,
                                    ...
                                    outW, outH);
            CheckAndLogError((ret < 0), UNKNOWN_ERROR,
                             "@%s, convert fail [%d]!", __FUNCTION__, ret);
        }
        return OK;
    }
};
