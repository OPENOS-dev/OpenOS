...
status_t MediaController::init()
{
    ...
    if (mPath == "/dev/media") {
        LOGW("Skipping /dev/media since it is assumed to be a directory. Please ignore warnings about this device.");
        return BAD_INDEX;
    }
...
status_t MediaController::open()
{
    ...
    if (mFd < 0) {
        if (mFd == -EPERM) {
            ...
            LOGD("Error opening media-ctl %s: %s", mPath.c_str(),
                 strerror(errno));
            ret = UNKNOWN_ERROR;
        }
    }
...
status_t MediaController::queryIPU3ImguMode(const char* entityName, const std::string& mode, int* value)
{
    ...
    status_t status = getMediaEntity(entity, entityName);
    CheckAndLogError(status != NO_ERROR, UNKNOWN_ERROR,
                     "@%s: getting MediaEntity \"%s\" failed", __FUNCTION__,
                     entityName);

    status = entity->getDevice(dev);
    CheckAndLogError((!dev || status != NO_ERROR), UNKNOWN_ERROR,
                     "@%s: error: %d, opening device \"%s\"", __FUNCTION__,
                     status, entityName);

    v4l2_queryctrl qCtrl = {};
    qCtrl.id = V4L2_CID_INTEL_IPU3_MODE;
    int ret = dev->QueryControl(&qCtrl);
    CheckAndLogError(ret != 0, UNKNOWN_ERROR, "@%s, QueryControl fails",
                     __FUNCTION__);
    ...
    for (qMenu.index = qCtrl.minimum; qMenu.index <= qCtrl.maximum; qMenu.index += qCtrl.step) {
        ...
        CheckAndLogError(ret != 0, UNKNOWN_ERROR, "@%s, QueryMenu fails",
                         __FUNCTION__);
...
status_t MediaController::configureLink(const MediaCtlLinkParams &linkParams)
{
    ...
    std::unique_ptr<media_link_desc[]> links =
        std::unique_ptr<media_link_desc[]>(new media_link_desc[entityDesc.links]());

    struct media_links_enum linksEnum = {};
    linksEnum.entity = entityDesc.id;
    linksEnum.pads = nullptr;
    linksEnum.links = links.get();
    status = enumLinks(linksEnum);
    CheckAndLogError(status != NO_ERROR, status, "Enum links failed");

    for (int i = 0; i < entityDesc.links; i++) {
        if (linksEnum.links[i].sink.entity == linkDesc.sink.entity &&
            linksEnum.links[i].source.entity == linkDesc.source.entity) {
            if (!(linksEnum.links[i].flags & MEDIA_LNK_FL_IMMUTABLE)) {
