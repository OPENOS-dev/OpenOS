...
RequestThread::deinit()
{
    ...
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&RequestThread::handleExit, base::Unretained(this));
...
RequestThread::configureStreams(camera3_stream_configuration_t *stream_list)
{
    ...
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&RequestThread::handleConfigureStreams,
                                            ...
                                            camera_metadata_t** meta)
{
    ...
    status_t status = NO_ERROR;
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&RequestThread::handleConstructDefaultRequest,
                       base::Unretained(this), std::move(msg));
    mCameraThread.PostTaskSync<status_t>(FROM_HERE, std::move(closure),
                                         &status);
    return status;
...
RequestThread::processCaptureRequest(camera3_capture_request_t *request)
{
    ...
    status_t status = NO_ERROR;
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&RequestThread::handleProcessCaptureRequest,
                       base::Unretained(this), std::move(msg));
    mCameraThread.PostTaskSync<status_t>(FROM_HERE, std::move(closure),
                                         &status);
...
RequestThread::returnRequest(Camera3Request* req)
{
    ...
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&RequestThread::handleReturnRequest,
                       base::Unretained(this), std::move(msg));
    mCameraThread.PostTaskAsync<status_t>(FROM_HERE, std::move(closure));
    return 0;
}
...
RequestThread::captureRequest(Camera3Request* request)
{
    ...
    if (inStream) {
        ...
        CheckAndLogError(status != NO_ERROR, status, "%s, processRequest fails",
                         __FUNCTION__);
    }
...
void RequestThread::deleteNilStreams()
{
    size_t i = 0;
    while (i < mStreams.size()) {
        if (!mStreams[i]->priv) {
