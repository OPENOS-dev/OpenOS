...
status_t AiqLibrary::aiq_init(void* pData, int dataSize)
{
    ...
    CheckAndLogError((aiq == nullptr), UNKNOWN_ERROR, "@%s, ia_aiq_init failed",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_af_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(af_run_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
    ...
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverUnflatten fails",
                     __FUNCTION__);

    ia_aiq_af_results* afResults = nullptr;
    ia_err err = ia_aiq_af_run(reinterpret_cast<ia_aiq*>(params->aiq_handle), afParams, &afResults);
    CheckAndLogError((err != 0), UNKNOWN_ERROR, "@%s, ia_aiq_af_run failed %d",
                     __FUNCTION__, err);

    ret = mIpc.serverFlattenAf(*afResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverFlatten fails",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_gbce_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(gbce_run_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
    ...
    CheckAndLogError((err != 0), UNKNOWN_ERROR,
                     "@%s, ia_aiq_gbce_run failed %d", __FUNCTION__, err);

    bool ret = mIpc.serverFlattenGbce(*gbceResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR,
                     "@%s, serverFlattenGbce fails", __FUNCTION__);
...
status_t AiqLibrary::statistics_set(void* pData, int dataSize)
{
    ...
    CheckAndLogError((err != 0), UNKNOWN_ERROR,
                     "@%s, ia_aiq_statistics_set failed %d", __FUNCTION__, err);
...
status_t AiqLibrary::aiq_ae_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((err != 0), UNKNOWN_ERROR, "@%s, ia_aiq_ae_run failed %d",
                     __FUNCTION__, err);

    ret = mIpc.serverFlattenAe(*aeResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverFlattenAe fails",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_awb_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((err != 0), UNKNOWN_ERROR, "@%s, ia_aiq_awb_run failed %d",
                     __FUNCTION__, err);

    ret = mIpc.serverFlattenAwb(*awbResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverFlattenAwb fails",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_pa_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(pa_run_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
    ...
    CheckAndLogError(ret == false, UNKNOWN_ERROR,
                     "@%s, serverUnflattenPa fails", __FUNCTION__);

    ia_aiq_pa_results* paResults = nullptr;
    ia_err err = ia_aiq_pa_run(reinterpret_cast<ia_aiq*>(params->aiq_handle), paParams, &paResults);
    CheckAndLogError((err != 0), UNKNOWN_ERROR, "@%s, ia_aiq_pa_run failed %d",
                     __FUNCTION__, err);

    ret = mIpc.serverFlattenPa(*paResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverFlattenPa fails",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_sa_run(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(sa_run_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
    ...
    CheckAndLogError(ret == false, UNKNOWN_ERROR,
                     "@%s, serverUnflattenSa fails", __FUNCTION__);

    ia_aiq_sa_results* saResults = nullptr;
    ia_err err = ia_aiq_sa_run(reinterpret_cast<ia_aiq*>(params->aiq_handle), saParams, &saResults);
    CheckAndLogError((err != 0), UNKNOWN_ERROR, "@%s, ia_aiq_sa_run failed %d",
                     __FUNCTION__, err);

    ret = mIpc.serverFlattenSa(*saResults, params);
    CheckAndLogError(ret == false, UNKNOWN_ERROR, "@%s, serverFlattenSa fails",
                     __FUNCTION__);
...
status_t AiqLibrary::aiq_get_aiqd_data(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(ia_binary_data_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
    ...
    CheckAndLogError((err != 0), UNKNOWN_ERROR,
                     "@%s, ia_aiq_get_aiqd_data failed %d", __FUNCTION__, err);
...
status_t AiqLibrary::aiq_get_version(void* pData, int dataSize)
{
    ...
    CheckAndLogError((pData == nullptr), UNKNOWN_ERROR, "@%s, pData is nullptr",
                     __FUNCTION__);
    CheckAndLogError((dataSize < sizeof(ia_aiq_version_params)), UNKNOWN_ERROR,
                     "@%s, buffer is small", __FUNCTION__);
