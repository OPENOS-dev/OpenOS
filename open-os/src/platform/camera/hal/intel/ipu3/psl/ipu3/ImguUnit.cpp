...
status_t ImguUnit::configStreams(std::vector<camera3_stream_t*> &activeStreams)
{
    ...
    CheckAndLogError(status != OK, status, "Sort streams failed %d", status);
    ...
    CheckAndLogError(blobNum > 2, BAD_VALUE, "Don't support blobNum %d",
                     blobNum);
    CheckAndLogError(yuvNum > 3, BAD_VALUE, "Don't support yuvNum %d", yuvNum);
    ...
    if (yuvNum > 0) {
        ...
        CheckAndLogError(status != OK, status, "Configure Video Pipe failed");
    }
    ...
    if (blobNum > 0) {
        ...
        CheckAndLogError(status != OK, status, "Configure Still Pipe failed");
    }
...
status_t ImguUnit::ImguPipe::startWorkers()
{
    ...
    for (const auto &it : mPipeConfig.deviceWorkers) {
        ...
        CheckAndLogError(status != OK, status,
                         "Failed to start workers, status %d", status);
    }
                                                     ...
                                                     std::vector<camera3_stream_t*> &streams)
{
    ...
    CheckAndLogError(streamNum == 0, UNKNOWN_ERROR, "streamNum is 0");
                           ...
                           std::shared_ptr<SharedItemPool<ia_aiq_rgbs_grid>> rgbsGridBuffPool)
{
    ...
    if (mFace && GraphConfig::PIPE_VIDEO == mPipeType) {
        ...
        for (const auto &it : mStreamNodeMapping) {
            ...
            // if they are same resulotions, we pick the usage which is
            // GRALLOC_USAGE_HW_COMPOSER
            if (s->format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
                if (preStream && ((preStream->width * preStream->height < s->width * s->height) ||
                    ((preStream->width * preStream->height == s->width * s->height) &&
            ...
            if (s->format == HAL_PIXEL_FORMAT_YCbCr_420_888) {
                if (yuvStream && ((yuvStream->width * yuvStream->height < s->width * s->height) ||
                    ((yuvStream->width * yuvStream->height == s->width * s->height) &&
                     !(s->usage & GRALLOC_USAGE_HW_COMPOSER)))) {
                    continue;
                }
                yuvStream = s;
    ...
    for (const auto &it : mPipeConfig.deviceWorkers) {
        ...
        CheckAndLogError(status != OK, status,
                         "Failed to configure workers, status %d.", status);
    }
                                    ...
                                    bool updateMeta)
{
    ...
    base::OnceCallback<status_t()> closure =
        base::BindOnce(&ImguUnit::ImguPipe::handleCompleteReq,
                       base::Unretained(this), std::move(msg));
    mCameraThread.PostTaskAsync<status_t>(FROM_HERE, std::move(closure));
    return NO_ERROR;
}
...
status_t ImguUnit::ImguPipe::processNextRequest()
{
    ...
    CheckAndLogError(msg == nullptr, BAD_VALUE,
                     "@%s: mMessagePending[0] is nullptr", __FUNCTION__);
    mMessagesPending.erase(mMessagesPending.begin());

    // update and return metadata firstly
    Camera3Request *request = msg->cbMetadataMsg.request;
    CheckAndLogError(request == nullptr, BAD_VALUE, "Request is nullptr");
    ...
    if (mFirstRequest) {
        ...
        CheckAndLogError(status != OK, status,
                         "failed to kick start, status %d", status);
    }
    ...
    for (;it != mPipeConfig.deviceWorkers.end(); ++it) {
        ...
        CheckAndLogError(status != OK, status,
                         "failed to prepare works, status %d", status);
    }
    ...
    status = mPollerThread->pollRequest(request->getId(),
                                        ...
                                        &(mPipeConfig.nodes));
    CheckAndLogError(status != OK, status, "failed to poll request, status %d",
                     status);
...
status_t ImguUnit::ImguPipe::kickstart(Camera3Request* request)
{
    ...
    CheckAndLogError(status != OK, status, "@%s, fail to call prepareRun",
                     __FUNCTION__);
    ...
    for (;firstit != mFirstWorkers.end(); ++firstit) {
        status |= (*firstit)->run();
        CheckAndLogError(status != OK, status, "failed to run works, status %d",
                         status);
    }
    ...
    for (;firstit != mFirstWorkers.end(); ++firstit) {
        ...
        CheckAndLogError(status != OK, status,
                         "failed to post-run works, status %d", status);
    }
                                          ...
                                          std::shared_ptr<ProcUnitSettings> settings)
{
    ...
    CheckAndLogError(ctrlUnitResult == nullptr, UNKNOWN_ERROR,
                     "Failed to retrieve Metadata buffer for reqId = %d",
                     request.getId());
...
status_t ImguUnit::ImguPipe::notifyPollEvent(PollEventMessage *pollMsg)
{
    ...
    if (pollMsg->id == POLL_EVENT_ID_EVENT) {
        ...
        base::OnceCallback<status_t()> closure =
            base::BindOnce(&ImguUnit::ImguPipe::handlePoll,
                           base::Unretained(this), std::move(msg));
        mCameraThread.PostTaskAsync<status_t>(FROM_HERE, std::move(closure));
...
ImguUnit::ImguPipe::flush(void)
{
    LOG2("%s pipe type %d", __FUNCTION__, mPipeType);
    status_t status = NO_ERROR;
    base::OnceCallback<status_t()> closure = base::BindOnce(
        &ImguUnit::ImguPipe::handleFlush, base::Unretained(this));
    mCameraThread.PostTaskSync<status_t>(FROM_HERE, std::move(closure),
                                         &status);
    return status;
}
