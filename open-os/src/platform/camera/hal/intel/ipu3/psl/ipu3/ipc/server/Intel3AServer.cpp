...
int32_t Intel3AServer::initialize(const camera_algorithm_callback_ops_t* callback_ops)
{
    ...
    if (mIaLogInitialized == false) {
        ...
        CheckAndLogError(ret != ia_err_none, -ENOMEM,
                         "@%s, ia_log_init fails, ret:%d", __FUNCTION__, ret);
                            ...
                            int32_t buffer_handle)
{
    ...
    mThreads[group]->task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&Intel3AServer::handleRequest,
