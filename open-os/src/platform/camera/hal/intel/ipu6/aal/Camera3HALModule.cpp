...
static int hal_get_camera_info(int cameraId, struct camera_info* cameraInfo) {
    ...
    if (sCameraMetadata[cameraId] == nullptr) {
        ...
        MetadataConvert::HALCapabilityToStaticMetadata(*(info.capability),
                                                       sCameraMetadata[cameraId], cameraId);
    }
...
static int hal_dev_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    CheckAndLogError(!name || !module || !device, -EINVAL, "Camera name is nullptr");
    LOG1("<id%s>%s", name, __func__);

#ifdef ENABLE_SANDBOXING
    if (!icamera::IntelAlgoClient::getInstance()->isIPCFine()) {
        CheckAndLogError(icamera::IntelAlgoClient::getInstance()->initialize() != icamera::OK,
                         -EINVAL, "%s, Connect to algo service fails", __func__);
        icamera::PlatformData::parseGraphFromXmlFile();
    }
#endif

    int camera_id = atoi(name);
    CheckAndLogError(camera_id < 0 || camera_id >= hal_get_number_of_cameras(), -ENODEV,
                     "<id%d>%s: Camera id is out of bounds, num. of cameras (%d)", camera_id,
                     __func__, hal_get_number_of_cameras());

    std::lock_guard<std::mutex> l(sCameraHalMutex);
    CheckAndLogError(sInstanceCount > 0 && sInstances[camera_id], -EUSERS,
                     "Camera already has been opened!");
...
static int hal_dev_close(hw_device_t* device) {
    LOG1("@%s", __func__);
    CheckWarning(!device || sInstanceCount == 0, -EINVAL, "hal close, instance count %d",
                 sInstanceCount);
    ...
    if (camera_priv != nullptr) {
        ...
        sInstanceCount--;
        sInstances[id] = false;
        FaceDetectionResultCallbackManager::getInstance().unsetCallback(id);
    }
...
static int hal_init(void) {
    ...
    auto infos = camera3::HalV3Utils::getCrosCameraInfo();

    int crosCameraNum = camera3::HalV3Utils::getCrosConfigCameraNumber(infos);
    ...
    if (xmlCameraNum == -1 && crosCameraNum == -1) {
        ...
        CheckAndLogError(currentCameraNum < sCameraNumber, -EINVAL,
    ...
    if (sCameraNumber != 0) {
        ...
        CheckAndLogError(ret != icamera::OK, -EINVAL, "@%s, camera_hal_init fails, ret:%d",
                         __func__, ret);
        // Update orientation
        camera3::HalV3Utils::updateOrientation(infos);
    }
...
#ifdef ENABLE_SANDBOXING
    // Set nullptr to the mojo manager token
    icamera::IntelAlgoClient::getInstance()->setMojoManagerToken(nullptr);
#endif
}

void set_face_detection_result_callback(int camera_id,
                                        cros::FaceDetectionResultCallback callback) {
    FaceDetectionResultCallbackManager::getInstance().setCallback(camera_id,
                                                                  std::move(callback));
}

void hal_set_privacy_switch_state(bool on) {
    PrivacyControl::setPrivacyMode(on);
}

static int camera_device_open_ext(const hw_module_t* module, const char* name,
                                  hw_device_t** device, cros::ClientType client_type) {
    int camera_id = atoi(name);
    CheckAndLogError(camera_id < 0 || camera_id >= hal_get_number_of_cameras(), -ENODEV,
                     "<id%d>%s: Camera id is out of bounds, num. of cameras (%d)", camera_id,
                     __func__, hal_get_number_of_cameras());

    if (client_type == cros::ClientType::kAndroid) {
        icamera::PlatformData::setSensorMode(camera_id, icamera::SENSOR_MODE_FULL);
    } else {
        icamera::PlatformData::setSensorMode(camera_id, icamera::SENSOR_MODE_UNKNOWN);
    }

    return hal_dev_open(module, name, device);
}
...
static hw_module_t camera_common = {.tag = HARDWARE_MODULE_TAG,
                                    ...
                                    .name = "Intel IPU6 Camera HAL Module",
                                    .author = "Intel",
...
camera_module_t VISIBILITY_PUBLIC HAL_MODULE_INFO_SYM = {
    ...
    .get_vendor_tag_ops = VendorTags::getVendorTagOps,
    .open_legacy = nullptr,
...
cros::cros_camera_hal_t VISIBILITY_PUBLIC CROS_CAMERA_HAL_INFO_SYM = {
    ...
    .camera_device_open_ext = camera_device_open_ext,
    .set_face_detection_result_callback = set_face_detection_result_callback,
    .set_privacy_switch_state = hal_set_privacy_switch_state
};
}
