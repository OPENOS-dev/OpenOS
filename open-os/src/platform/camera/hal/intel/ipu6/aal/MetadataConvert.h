...
class MetadataConvert {
 ...
 private:
    ...
    static int convertAdvancedFeatureParameter(const icamera::Parameters& parameter,
                                               const icamera::CameraMetadata& meta,
                                               android::CameraMetadata* settings);
    static int convertVendorTagParameter(const icamera::Parameters& parameter,
                                         const icamera::CameraMetadata& meta,
                                         android::CameraMetadata* settings);
    ...
    static int convertLensMetadata(const android::CameraMetadata& settings,
                                   icamera::Parameters* parameter);
    static int convertVendorControlMetadata(const android::CameraMetadata& settings,
                                            icamera::Parameters* parameter);
    ...
    static int convertToMetadataRegion(const icamera::camera_window_list_t& windows, int dataCount,
                                       int32_t* data);
    static int convertScalerMetadata(const android::CameraMetadata& settings,
                                     icamera::Parameters* parameter);
...
class StaticCapability {
 public:
    static StaticCapability& getInstance();
    const icamera::CameraMetadata* getCapability(int cameraId);

 private:
    std::mutex mLock;
    std::unordered_map<int, icamera::CameraMetadata> mMetadataMap;
};
