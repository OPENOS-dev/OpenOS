...
ResultProcessor::ResultProcessor(int cameraId, const camera3_callback_ops_t* callback,
        ...
        : mCameraId(cameraId),
          ...
          mVideoPipeOnly(false) {
    UNUSED(mCameraId);
    LOG1("<id%d>@%s", mCameraId, __func__);

    mLastSettings = acquireMetadataMemory();

    mResultThread = std::unique_ptr<ResultThread>(new ResultThread(this, result));
...
void ResultProcessor::updateMetadata(const icamera::metadata_entry_t& entry) {
    std::unordered_map<uint32_t, std::pair<uint32_t, uint8_t>> metaMap = {
        {CAMERA_TONEMAP_CURVE_RED, {ANDROID_TONEMAP_CURVE_RED, ICAMERA_TYPE_FLOAT}},
        {CAMERA_TONEMAP_CURVE_BLUE, {ANDROID_TONEMAP_CURVE_BLUE, ICAMERA_TYPE_FLOAT}},
        {CAMERA_TONEMAP_CURVE_GREEN, {ANDROID_TONEMAP_CURVE_GREEN, ICAMERA_TYPE_FLOAT}},
        {INTEL_VENDOR_CAMERA_TONE_MAP_CURVE, {INTEL_VENDOR_CAMERA_TONE_MAP_CURVE,
         ICAMERA_TYPE_FLOAT}},
        {INTEL_VENDOR_CAMERA_RGBS_STATS_BLOCKS, {INTEL_VENDOR_CAMERA_RGBS_STATS_BLOCKS,
         ICAMERA_TYPE_BYTE}},
    };

    std::lock_guard<std::mutex> l(mLock);
    for (uint32_t i = 0; i < mRequestStateVector.size(); i++) {
        if (mRequestStateVector.at(i).frameNumber != entry.frameNumber)
            continue;

        MetadataMemory* metaResult = mRequestStateVector[i].metaResult;
        android::CameraMetadata* meta = metaResult->getMetadata();
        if (metaMap.find(entry.tag) != metaMap.end()) {
            switch (metaMap[entry.tag].second) {
                case ICAMERA_TYPE_BYTE:
                    meta->update(metaMap[entry.tag].first, entry.data.u8, entry.count);
                    break;
                case ICAMERA_TYPE_INT32:
                    meta->update(metaMap[entry.tag].first, entry.data.i32, entry.count);
                    break;
                case ICAMERA_TYPE_FLOAT:
                    meta->update(metaMap[entry.tag].first, entry.data.f, entry.count);
                    break;
                case ICAMERA_TYPE_INT64:
                    meta->update(metaMap[entry.tag].first, entry.data.i64, entry.count);
                    break;
                case ICAMERA_TYPE_DOUBLE:
                    meta->update(metaMap[entry.tag].first, entry.data.d, entry.count);
                    break;
                default:
                    LOGW("Wrong data type %d", metaMap[entry.tag].second);
                    break;
            }
            LOG2("%s, tag %u, type %d, count %d", __func__, metaMap[entry.tag].first,
                 metaMap[entry.tag].second, entry.count);
        } else {
            LOGW("Meta tag %d not found", entry.tag);
        }
    }
}
...
void ResultProcessor::callbackNotify(const icamera::camera_msg_data_t& data) {
    ...
    if (data.type == icamera::CAMERA_METADATA_ENTRY) {
        updateMetadata(data.data.metadata_entry);
        return;
    }
...
int ResultProcessor::registerRequest(const camera3_capture_request_t* request,
    ...
    LOG2("<fn%u>@%s inputCam3Buf:%p", request->frame_number, __func__, inputCam3Buf.get());
...
int ResultProcessor::shutterDone(const ShutterEvent& event) {
    ...
    for (uint32_t i = 0; i < mRequestStateVector.size(); i++) {
        ...
        LOG2("<fn%u>@%s shutter timestamp:%lu", notifyMsg.message.shutter.frame_number, __func__,
             notifyMsg.message.shutter.timestamp);
...
void ResultProcessor::videoPipeOnly(bool videoOnly) {
    mVideoPipeOnly = videoOnly;
}

/*
 * Check if return metadata with buffer done.
 * Below cases return metadata immediately:
 * 1, not video pipe only;
 * 2, all buffers are returned;
 * 3, RGBS callback is required;
 * 4, not in low power mode.
 */
bool ResultProcessor::isReturnMetadataWithBuf(RequestState curReqState) {
    if (!mVideoPipeOnly || !curReqState.metaResult) return false;

    bool hasCb = false;
    bool lowPower = false;
    android::CameraMetadata* meta = curReqState.metaResult->getMetadata();
    camera_metadata_entry entry = meta->find(INTEL_VENDOR_CAMERA_CALLBACK_RGBS);
    if (entry.count == 1) {
        hasCb = (entry.data.u8[0] == INTEL_VENDOR_CAMERA_CALLBACK_RGBS_TRUE) ? true : false;
    }
    entry = meta->find(INTEL_VENDOR_CAMERA_POWER_MODE);
    if (entry.count == 1) {
        lowPower = (entry.data.u8[0] == INTEL_VENDOR_CAMERA_POWER_MODE_LOW_POWER) ? true : false;
    }

    if (curReqState.buffersToReturn == curReqState.buffersReturned || hasCb || !lowPower) {
        return false;
    }

    LOG2("<fn%u>@%s, pending metadata for buffer done", curReqState.frameNumber, __func__);
    return true;
}
...
int ResultProcessor::metadataDone(const MetadataEvent& event) {
    ...
    for (auto& reqStat : mRequestStateVector) {
        if (reqStat.frameNumber == event.frameNumber &&
            ...
            reqStat.isMetadataReady = true;
            curReqState = reqStat;

            LOG2("<fn%u>@%s, metadataReady", reqStat.frameNumber, __func__);
            if (mInputCam3Bufs.find(reqStat.frameNumber) == mInputCam3Bufs.end()) {
                android::CameraMetadata* metaResult = reqStat.metaResult->getMetadata();
    ...
    if (isReturnMetadataWithBuf(curReqState)) return icamera::OK;

    /**
     * Return metadata in request order, because metadata event from device might be not in FIFO.
     * (video/still stream return metadata separately).
     */
    for (int32_t i = 0; i < mRequestStateVector.size(); i++) {
        RequestState& reqStat = mRequestStateVector[i];

        if (!reqStat.isShutterDone) return icamera::OK;
        if (!reqStat.isMetadataReady) return icamera::OK;
        if (reqStat.isMetadataDone) continue;

        camera3_capture_result_t result;
        CLEAR(result);
        result.frame_number = reqStat.frameNumber;
        result.output_buffers = nullptr;
        result.num_output_buffers = 0;
        result.result = reqStat.metaResult->getMemory();
        result.partial_result = 1;
        result.num_physcam_metadata = 0;
        result.physcam_ids = nullptr;
        result.physcam_metadata = nullptr;
        mCallbackOps->process_capture_result(mCallbackOps, &result);
        if (reqStat.partialResultCount == reqStat.partialResultReturned)
            reqStat.isMetadataDone = true;

        LOG2("<fn%u>@%s, return @ %u", result.frame_number, __func__, event.frameNumber);

        if (checkRequestDone(reqStat)) {
            returnInputBuffer(reqStat.frameNumber);
            returnRequestDone(reqStat.frameNumber);
            releaseMetadataMemory(reqStat.metaResult);
            mRequestStateVector.erase(mRequestStateVector.begin() + i);
            // The element i + 1 will be element i after erase, so reset i
            i--;
        }
    }
...
int ResultProcessor::bufferDone(const BufferEvent& event) {
    ...
    for (int32_t i = 0; i < mRequestStateVector.size(); i++) {
        ...
        // Check if the previous metadatas'd returned
        if (!reqStat.isShutterDone) break;
        if (!reqStat.isMetadataReady) break;
        if (reqStat.frameNumber < event.frameNumber && !reqStat.isMetadataDone) break;

        if ((reqStat.frameNumber == event.frameNumber) && !reqStat.isMetadataDone) {
            curReqState = reqStat;
            break;
        }
    }
    ...
    result.result
        = (curReqState.metaResult) ? curReqState.metaResult->getMemory() : nullptr;
    result.partial_result = (curReqState.metaResult) ? 1 : 0;
    result.num_physcam_metadata = 0;
    result.physcam_ids = nullptr;
    result.physcam_metadata = nullptr;
    ...
    for (uint32_t i = 0; i < mRequestStateVector.size(); i++) {
        if (mRequestStateVector.at(i).frameNumber == event.frameNumber) {
            ...
            if (curReqState.metaResult) mRequestStateVector.at(i).isMetadataDone = true;
            if (checkRequestDone(mRequestStateVector.at(i))) {
    ...
    if (!found) {
        LOGW("<fn%u>%s wasn't found!", event.frameNumber, __func__);
    } else {
        LOG2("<fn%u>%s %s was returned", event.frameNumber, __func__,
             (curReqState.metaResult) ? "with metadata" : "without metadata");
    }
    ...
    if (event.timestamp != 0 && event.sequence != -1) {
        ...
        mOpaqueRawInfoMap[event.timestamp] = event.sequence;
    }
    return icamera::OK;
}
...
void ResultProcessor::checkAndChangeRawbufferInfo(long* sequence, uint64_t* timestamp) {
    CheckAndLogError(!sequence || !timestamp, VOID_VALUE, "invilid input parameter!");

    std::lock_guard<std::mutex> lock(mmOpaqueRawInfoMapLock);
    if (mOpaqueRawInfoMap.empty()) return;
    if (mOpaqueRawInfoMap.find(*timestamp) != mOpaqueRawInfoMap.end()) {
        *sequence = mOpaqueRawInfoMap[*timestamp];
    } else {
...
void ResultProcessor::updateMetadata(const icamera::Parameters& parameter,
     ...
     */
    if (icamera::PlatformData::isFaceDetectionSupported(mCameraId)) {
...
void ResultProcessor::returnInputBuffer(uint32_t frameNumber) {
    ...
    if (s) {
        LOG2("<fn%u>@%s, w:%d, h:%d, f:%d", frameNumber, __func__, s->width, s->height, s->format);
    }
    ...
    LOG2("<fn%u>@%s, return the input buffer", frameNumber, __func__);
    mCallbackOps->process_capture_result(mCallbackOps, &result);
...
bool ResultProcessor::checkRequestDone(const RequestState& requestState) {
    ...
    return (requestState.isShutterDone && requestState.isMetadataDone &&
...
void ResultProcessor::releaseMetadataMemory(MetadataMemory* metaMem) {
    CheckAndLogError(metaMem == nullptr, VOID_VALUE, "%s: null metaMem!", __func__);
...
bool ResultProcessor::ResultThread::threadLoop() {
    ...
    // mutex only protects mEventQueue
    {
        ...
        if (mEventQueue.empty()) {
            ...
            if (ret == std::cv_status::timeout) {
                LOG2("%s, wait event timeout", __func__);
            }
    ...
    switch (data.type) {
        ...
        case icamera::CAMERA_ISP_BUF_READY: {
            ...
            LOG2("<fn%u>@%s, timestamp %ld, mResultProcessor:%p", event.frameNumber, __func__,
                 event.timestamp, mResultProcessor);
            mResultProcessor->shutterDone(event);
            break;
        }
        // Regards metadata ready as metadata event
        case icamera::CAMERA_METADATA_READY: {
            icamera::Parameters* parameter = mParameterResult->getParameter(
                data.data.metadata_ready.frameNumber, data.data.metadata_ready.sequence);
            MetadataEvent metadataEvent = {data.data.metadata_ready.frameNumber, parameter};
            mResultProcessor->metadataDone(metadataEvent);
            break;
        }
        case icamera::CAMERA_IPC_ERROR:
        case icamera::CAMERA_DEVICE_ERROR: {
