...
bool IPCGraphConfig::clientFlattenConfigStreams(void* pData, uint32_t size, GraphBaseInfo info,
    ...
    GraphConfigStreamParams* params = new (pData) GraphConfigStreamParams();
...
bool IPCGraphConfig::serverUnflattenConfigStreams(void* pData, uint32_t size, GraphBaseInfo* info,
    ...
    CheckAndLogError(!pData || size < sizeof(GraphConfigStreamParams) || !info || !type ||
                     !streams || !dummyStillSink || !sensorMode, false,
                     "@%s, pData:%p, size:%u, info:%p, type:%p, streams:%p, sink:%p, mode:%p",
                     __func__, pData, size, info, type, streams, dummyStillSink, sensorMode);
...
bool IPCGraphConfig::serverFlattenGetGraphData(void* pData, uint32_t size,
    ...
    params->graphNum = graphData.graphIds.size();
    CheckAndLogError(params->graphNum > MAX_GRAPH_COUNT, false, "@%s: more graphs %d", __func__,
                     params->graphNum);
    int32_t idx = 0;
    for (auto it = graphData.graphIds.cbegin(); it != graphData.graphIds.cend(); ++it) {
        params->graphIds[idx++] = *it;
    }
...
bool IPCGraphConfig::clientUnflattenGetGraphData(void* pData, uint32_t size,
    ...
    for (uint32_t i = 0; i < params->graphNum; i++)
        graphData->graphIds.insert(params->graphIds[i]);
...
bool IPCGraphConfig::clientFlattenGetConnection(void* pData, uint32_t size, GraphBaseInfo info,
    ...
    GraphGetConnectionParams* params = new (pData) GraphGetConnectionParams();
