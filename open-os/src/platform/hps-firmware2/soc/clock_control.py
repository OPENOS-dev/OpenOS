...
"""Modules for controlling clock signals within the FPGA"""
...
class CfuCpuClockCtrl(Module):
    """Controls clocks between CFU and CPU so that power usage is optimized.

    Attributes:
        cfu_bus: Input signal with layout cfu_bus_minimized_layout.
            This should be connected to (a subset of) the bus between
            the CFU and the CPU.
    ...
    def __init__(self):
        ...
        fsm.act(
            ...
            If(
                ...
                self.cpu_cen.eq(0),
                # Enable CPU and disable CFU if CPU received a response
                # and has no next command
                If(
