...
class PixelAddressGenerator(SimpleElaboratable):
    ...
    Attributes:
        ...
        next (Signal(), in):
            Indicates current address has been used. Address will be updated on
            next cycle with next row address.
    """
...
class PixelAddressRepeater(SimpleElaboratable):
    ...
    Attributes:
        ...
        repeats (Signal(unsigned_upto(64)), in):
            The number of times each stream of input pixel data is to be
            repeated.
...
class ValueAddressGenerator(SimpleElaboratable):
    ...
    Attributes:
        ...
        depth (Signal(3), in):
            Number of 16-byte blocks to read per pixel. Max depth is 7 which is
            112 values/pixel).
...
class Mode1InputFetcher(SimpleElaboratable):
    ...
    Attributes:
        ...
        depth (Signal(3), in):
            Number of 16-byte blocks to read per pixel. Max depth is 7 which is
            112 values/pixel). Total number of cycles per output pixel is
    ...
    def elab(self, m):
        ...
        delayed_last = Signal()
        m.d.sync += [
            delayed_first.eq(running & (cycle_counter == 0)),
            delayed_last.eq(running & (cycle_counter == max_cycle_counter)),
            self.first.eq(delayed_first),
            self.last.eq(delayed_last),
        ]
