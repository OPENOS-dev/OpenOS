...
from .post_process import AccumulatorReader
from .post_process import OutputWordAssembler
from .post_process import ParamWriter
from .post_process import POST_PROCESS_PARAMS
from .post_process import POST_PROCESS_PARAMS_WIDTH
from .post_process import PostProcessPipeline
from .post_process import ReadingProducer
from .post_process import StreamLimiter
...
class AcceleratorCore(SimpleElaboratable):
    ...
    2. reset
    3. Add filter and post process param data
    ...
    Attributes:
        ...
        output (Endpoint(unsigned(32)), out):
            The 8 bit output values as 4 byte words.
            Values are produced in an algorithm dependent order.
    """

    def __init__(self, specialize_nx=False):
        """Inits AcceleratorCore.
    ...
    def build_input_fetcher(self, m, stop):
        ...
        repeats = self.config.output_channel_depth // Const(
            Constants.SYS_ARRAY_WIDTH
        )
        m.d.comb += [
    ...
    def build_param_store(self, m):
        ...
        param_mem = Memory(
            width=POST_PROCESS_PARAMS_WIDTH, depth=Constants.MAX_CHANNEL_DEPTH
        )
