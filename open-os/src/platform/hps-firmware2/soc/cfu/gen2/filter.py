...
class FilterStore(SimpleElaboratable):
    ...
    Attributes:
        ...
        values_out ([Signal(32) * num_memories], out):
            Filter values output in the correct manner for a Systolic Array

        start (Signal(), in):
            Causes filter output to begin with addr(0), 3 cycles from now.
    """
    ...
    def elab(self, m):
        ...
        for i in range(Constants.NUM_FILTER_STORES):
            ...
            if i == 0:
                ...
                addr = Signal(
                    range(-i, Constants.FILTER_WORDS_PER_STORE),
                    name=f"addr_{i}",
                )
            ...
            m.d.sync += self.values_out[i].eq(mem.read_data)
