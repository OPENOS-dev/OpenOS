...
def delay(m, signal, cycles):
    ...
    Args:
        m (Module): The module to work in.
        signal (Signal): The signal to delay.
        cycles (int): The number of cycles to delay.
    """
    name = signal.name
    ...
    def elab(self, m: Module):
        ...
        m.d.comb += [
            ...
            srdhm.input.payload.a.eq(
                self.input.payload + self.params.payload.bias
            ),
...
class AccumulatorReader(SimpleElaboratable):
    ...
    def __init__(self, accumulator_shape=signed(32)):
        self.accumulator = [
            Signal(accumulator_shape, name=f"acc_{i}") for i in range(8)
        ]
    ...
    def elab(self, m):
        ...
        with m.If(self.half_mode):
            ...
            m.d.comb += Cat(next_index[0], next_index[2]).eq(
                Cat(index[0], index[2]) + 1
            )
        with m.Else():
...
class StreamLimiter(SimpleElaboratable):
    """Allows only a certain number of elements to pass through stream.

    Counts a given number of items, then signals that it is done.

    This module can be parameterized as follows:
...
class OutputWordAssembler(SimpleElaboratable):
    ...
    Attributes:
        ...
        half_mode (Signal, in):
          In normal mode, buffers data over four pixels. In half mode only
          buffers data over two pixels.
    ...
    def elab(self, m):
        ...
        m.d.sync += last_pixel_index.eq(
            Mux(self.half_mode, self._num_pixels // 2 - 1, self._num_pixels - 1)
        )
        next_pixel_index = Signal.like(pixel_index)
        m.d.comb += next_pixel_index.eq(
            Mux(pixel_index == last_pixel_index, 0, pixel_index + 1)
        )
