...
from ..util import RegisteredReadPort
from ..util import TestBase
from .conv2d_data import fetch_data
from .mode1_input import Mode1InputFetcher
from .mode1_input import PixelAddressGenerator
from .mode1_input import PixelAddressRepeater
from .mode1_input import ValueAddressGenerator
        ...
        for row in range(4):
            for col in range(4):
                for i in range(depth):
                    expected += [
                        start_addr + row * num_blocks_y + col * depth + i
                    ] * 4
...
class Mode1InputFetcherTest(TestBase):
    ...
    def create_dut(self):
        ...
        for i in range(4):
            ...
            rp = RegisteredReadPort(mem.read_port(transparent=False))
            self.m.d.comb += [
    ...
    def pixel_input_values(self, index):
        ...
        result = []
        for row in range(4):
            row_start_addr = ((pixel_y + row) * in_x_dim + pixel_x) * word_depth
            result += data.input_data[
                row_start_addr : row_start_addr + word_depth * 4
            ]
        return result
    ...
    def test_it(self):
        ...
        def process():
            ...
            # For each of the four outputs, capture values between
            # first and last
