...
from ..cfu import CfuTestBase
from ..cfu import InstructionTestBase
from ..util import pack_vals
from .constants import Constants
from .conv2d_data import fetch_data
from .hps_cfu import HpsCfu
from .hps_cfu import PoolInstruction
...
class HpsCfuTest(CfuTestBase):
    ...
    def load_filter_data(self, data, chan_start, chan_count):
        ...
        for _ in range(0, chan_count, 2):
            for store in range(2):
                addr = addr_base
                for _ in range(num_filter_words_per_output):
                    value = filter_data[chan_data_index]
                    chan_data_index += 1
                    yield self.do_set(
                        Constants.REG_FILTER_WRITE, (store << 16 | addr), value
                    )
                    addr += 1
    ...
    def configure(self, data, output_chan_start, output_chan_count):
        ...
        yield do_set(
            C.REG_NUM_OUTPUT_VALUES, self.NUM_OUTPUT_PIXELS * output_chan_count
        )
        ...
        for i in range(output_chan_count):
            ...
            yield do_set(
                C.REG_POST_PROCESS_MULTIPLIER, data.output_multipliers[index]
            )

        # Load filters
        yield from self.load_filter_data(
            data, output_chan_start, output_chan_count
        )
        ...
        def ram():
            ...
            while True:
                for i in range(4):
                    ...
                    data_value = data.input_data[
                        data_addr % len(data.input_data)
                    ]
                    yield self.dut.lram_data[i].eq(next_out[i])
                    next_out[i] = data_value
                yield
