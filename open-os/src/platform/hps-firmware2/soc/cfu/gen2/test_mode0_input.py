...
from .mode0_input import ValueAddressGenerator
from .mode0_input import ValueReader
...
class ValueReaderTest(TestBase):
    ...
    def create_dut(self):
        ...
        def make_ram(n, init):
            ...
            self.m.submodules[f"rp{n}"] = rp = RegisteredReadPort(
    ...
    def test_it(self):
        ...
        def process():
            for i in range(len(DATA) + 2):
                # Output address on cycle i
                if i < len(DATA):
                    addr, _, _ = DATA[i]
                yield dut.addr.eq(addr)
                yield
                # Check first output 2 cycles later
                if i > 2:
                    actual = yield dut.data_out[0]
                    expected = DATA[i - 2][1]
                    self.assertEqual(
                        actual,
                        expected,
                        f"{i}: 0x{actual:08X} != 0x{expected:08X}",
                    )
                # Check second output 3 cycles later
                if i > 3:
                    actual = yield dut.data_out[1]
                    expected = DATA[i - 3][2]
                    self.assertEqual(
                        actual,
                        expected,
