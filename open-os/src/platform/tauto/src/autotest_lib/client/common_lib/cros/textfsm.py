...
class TextFSMValue(object):
  ...
  """
    ...
    """
        ...
        if (not re.match(r'^\(.*\)$', self.regex)
                    or self.regex.count('(') != self.regex.count(')')):
            raise TextFSMTemplateError(
    ...
    def __str__(self):
        ...
        if self.options:
            return 'Value %s %s %s' % (','.join(
                    self.OptionNames()), self.name, self.regex)
        else:
...
class TextFSMRule(object):
  ...
  """
    ...
    """
        ...
        if 'new_state' in action_re.groupdict() and action_re.group(
                'new_state'):
