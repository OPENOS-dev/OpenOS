...
# The DEV_DISABLE_BOOT flag location
VB2_SECDATA_FWMP_DEV_DISABLE_BOOT=0

# check if the argument is a valid hex value (no 0x prefix)
is_valid_hex() {
    local hex_str="$1"
    ...
    # Use a regular expression to check if the string contains only valid hex characters
    if echo "$hex_str" | grep -Eq '^[0-9a-fA-F]+$'; then
        return 0
    else
        return 1
    fi
...
# Check if FWMP DEV_DISABLE_BOOT
# Fail if set or not accessible (return 1)
# otherwise return 0 (meaning enabled)
is_fwmp_dev_enabled() {
  local bit_position=VB2_SECDATA_FWMP_DEV_DISABLE_BOOT
  local tpmc_output

  tpmc_output=$(tpmc read 0x100a 8)

  if [ $? -eq 0 ]; then
    set -- $tpmc_output  # Split the output only if tpmc succeeded
  else
    return 1 # error
  fi
  ...
  for element; do
    if [ $count -eq 8 ]; then
        break  # Stop the loop after checking 8 elements
    fi
    if ! is_valid_hex "$element"; then
      return 1  # error
    fi
    count=$((count + 1))
  done

  local flags=$5 # 5th byte is the flags
  local bit_val=$(( ( 0x$flags >> bit_position ) & 1 ))
  return $bit_val
}
...
is_developer_mode_blocked() {
  ...
  if ! is_fwmp_dev_enabled; then
    dlog "dev mode boot blocked by fwmp"
    return 0
  fi
...
verify_part_hash() {
  ...
  # We want to pull the `cros_part_hash` from the recovery kernel command line.
  if [ -z "${KERN_ARG_CROS_PART_HASH}" ]; then
    # If there's no hash on the command line, there's nothing to do.
    return 0
  fi

  # The `cros_part_hash` is in the format of:
  # part_num:base64(hextobin(digest))
  #
  # There can be multiple separated by comma.
  #
  # We need to convert it back to the hex digest to compare with the actual
  # hash of the partition.
  OLD_IFS="${IFS}"
  IFS=','
  set -- ${KERN_ARG_CROS_PART_HASH}
  IFS="${OLD_IFS}"

  local part_info
  local part_num
  local real_part_hash
  while [ -n "$1" ]; do
    part_info="$1"
    shift

    # Split into the partition number and the base64(hextobin(digest)).
    part_num="${part_info%%:*}"
    part_hash_base64_binary="${part_info#*:}"

    # Decode the base64 encoded binary to get the hex digest.
    part_hash=$(echo "${part_hash_base64_binary}" | base64 -d \
        | hexdump -v -e '/1 "%02x"')

    # Get the hash of the real partition.
    real_part_hash=$(sha256sum "${REAL_USB_DEV%[0-9]*}${part_num}" \
        | cut -f1 -d' ')

    if [ "${part_hash}" != "${real_part_hash}" ]; then
      dlog "Partition hash mismatch!"
      dlog "Partition number: ${part_num}"
      dlog "Expected: ${part_hash}"
      dlog "Got:      ${real_part_hash}"
      return 1
    fi
  done
...
setup_install_mounts() {
  ...
  mount -t tmpfs -o mode=0755 none "${USB_MNT}/var" || return 1
...
recover_system() {
  ...
  call_image_recovery_script ${extra_flags} || return 1
