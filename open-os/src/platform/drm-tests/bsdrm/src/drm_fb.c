...
void bs_drm_fb_builder_gbm_bo(struct bs_drm_fb_builder *self, struct gbm_bo *bo)
{
	...
	for (size_t plane_index = 0; plane_index < self->plane_count; plane_index++) {
		self->handles[plane_index] = gbm_bo_get_handle_for_plane(bo, plane_index).u32;
...
uint32_t bs_drm_fb_builder_create_fb(struct bs_drm_fb_builder *self)
{
	...
	if (self->modifier != DRM_FORMAT_MOD_INVALID &&
		...
		ret = drmModeAddFB2WithModifiers(self->fd, self->width, self->height, format,
						 self->handles, self->strides, self->offsets,
	...
	if (ret) {
		bs_debug_error("failed to create drm fb: %s returned %d",
			       self->modifier ? "drmModeAddFB2WithModifiers" : "drmModeAddFB2",
			       ret);
		return 0;
	}
