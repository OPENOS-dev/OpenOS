...
static int test_reinit()
{
    ...
    struct gbm_bo *bo;
    bo = gbm_bo_create(
        gbm, 1024, 1024, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
    CHECK(check_bo(bo));
    gbm_bo_destroy(bo);
...
static int test_alloc_free_sizes()
{
    ...
    for(i = 1; i < 1920; i++) {
        struct gbm_bo *bo;
        bo = gbm_bo_create(
            gbm, i, i, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
        CHECK(check_bo(bo));
        gbm_bo_destroy(bo);
    }
    ...
    for(i = 1; i < 1920; i++) {
        struct gbm_bo *bo;
        bo = gbm_bo_create(
            gbm, i, 1, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
        CHECK(check_bo(bo));
        gbm_bo_destroy(bo);
    }
    ...
    for(i = 1; i < 1920; i++) {
        struct gbm_bo *bo;
        bo = gbm_bo_create(
            gbm, 1, i, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
        CHECK(check_bo(bo));
        gbm_bo_destroy(bo);
    }
...
static int test_import_dmabuf()
{
    ...
    bo1 = gbm_bo_create(
        gbm, width, height, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
    CHECK(check_bo(bo1));
...
static int test_import_modifier()
{
    ...
    for (i = 0; i < ARRAY_SIZE(format_list); i++) {
        ...
        if (gbm_device_is_format_supported(gbm, format, GBM_BO_USE_RENDERING)) {
            bo1 = gbm_bo_create(
                gbm, width, height, format, GBM_BO_USE_RENDERING);
            CHECK(check_bo(bo1));
            ...
            for (p = 0; p < num_planes; p++) {
                CHECK(
                    fd_data.strides[p] == gbm_bo_get_stride_for_plane(bo2, p));
...
static int test_gem_map()
{
    ...
    addr = gbm_bo_map(
        bo, 0, 0, width, height, GBM_BO_TRANSFER_READ_WRITE, &stride,
              &map_data);
    ...
    addr = gbm_bo_map(
        bo, 0, 0, width, height, GBM_BO_TRANSFER_READ_WRITE, &stride,&map_data);
    ...
    pixel = (uint32_t *)addr;
    CHECK(pixel[(
        height / 2) * (stride / pixel_size) + width / 2] == 0xABBAABBA);
...
static int test_dmabuf_map()
{
    ...
    bo = gbm_bo_create(
        gbm, width, height, GBM_FORMAT_ARGB8888, GBM_BO_USE_LINEAR);
    CHECK(check_bo(bo));
    ...
    addr = mmap(
        NULL, length, (PROT_READ | PROT_WRITE), MAP_SHARED, prime_fd, 0);
    CHECK(addr != MAP_FAILED);
...
static int test_gem_map_tiling(enum gbm_bo_flags buffer_create_flag)
{
    ...
    bo = gbm_bo_create(
        gbm, width, height, GBM_FORMAT_ARGB8888, buffer_create_flag);
    CHECK(check_bo(bo));

    addr = gbm_bo_map(
        bo, 0, 0, width, height, GBM_BO_TRANSFER_WRITE, &stride,
              &map_data);
                   ...
                   enum gbm_bo_flags buffer_create_flag)
{
    ...
    const uint32_t pixel_format =
     mappable_format_list[format_index].pixel_format;
