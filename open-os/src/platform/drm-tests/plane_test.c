...
static enum plane_type parse_type(const char* str) {
  if (!strcasecmp(str, "primary")) {
    return PRIMARY;
  } else if (!strcasecmp(str, "overlay")) {
    return OVERLAY;
  } else if (!strcasecmp(str, "cursor")) {
    return CURSOR;
  }
...
static bool find_overlay_planes(int fd,
  ...
  for (uint32_t plane_index = 0;
    ...
    enum plane_type current_type = UNSPECIFIED;
...
struct test_plane {
  ...
  enum plane_type type; // Primary, overlay, cursor.
...
static void print_help(const char* argv0) {
  ...
  printf("  -y, --type TYPE           plane type (defaults to PRIMARY for first plane with unspecified\n");
...
int main(int argc, char** argv) {
  ...
  enum plane_type test_planes_types[MAX_TEST_PLANES] = {0};
  ...
  int c;
  while ((c = getopt_long(argc, argv, "em:pf:y:z:c:ts:d:h", longopts, NULL)) != -1) {
  ...
  // We need to use GBM_BO_USE_LINEAR here since we're going to map |bg_bo|
  // next; this might prevent using some compression mechanisms.
  struct gbm_bo* bg_bo =
      gbm_bo_create(gbm, mode.hdisplay, mode.vdisplay, GBM_FORMAT_XRGB8888,
        GBM_BO_USE_SCANOUT | GBM_BO_USE_SW_WRITE_RARELY | GBM_BO_USE_LINEAR);
  if (!bg_bo) {
  ...
  for (size_t test_plane_index = 0; test_plane_index < test_planes_count;
    ...
    // We need to use GBM_BO_USE_LINEAR here since we're going to map |tp->bo|
    // next; this might prevent using some compression mechanisms.
    tp->bo = gbm_bo_create(gbm, tp->bo_w, tp->bo_h,
                           bs_get_pixel_format(tp->format),
  ...
  if (!find_overlay_planes(pipe.fd, pipe.crtc_id, test_planes_count,
                           test_planes_formats, test_planes_types,
                           test_planes_ids)) {
