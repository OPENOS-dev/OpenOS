...
def build_slim_rootfs(board, destination):
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        subprocess.run(
            ["/sbin/resize2fs", "-M", slim_rootfs_img_path], check=True
        )
        ...
        if destination.startswith("gs://"):
            ...
            subprocess.run(
                ["gsutil", "cp", slim_rootfs_img_path, destination], check=True
            )
        else:
            print("Copying to:", destination)
            subprocess.run(
                ["cp", slim_rootfs_img_path, destination], check=True
            )


def main():
    parser = argparse.ArgumentParser(
        description="Build a slim rootfs for a Termina guest"
    )
