...
from termina_util import extract_vmlinux
from termina_util import mount_disk


def extract_squashfs_partition(dest_path, disk_path, part_num):
    base_cmd = ["sudo", "partx", "--raw", "--noheadings", "--nr", str(part_num)]
    part_start_sectors = subprocess.check_output(
        base_cmd + ["--output", "START", disk_path]
    )
    part_start_bytes = int(part_start_sectors) * 512
    part_size_sectors = subprocess.check_output(
        base_cmd + ["--output", "SECTORS", disk_path]
    )
...
def can_hardlink(src_path, target_path):
    ...
    return (
        src_stat.st_mode == target_stat.st_mode
        and src_stat.st_uid == target_stat.st_uid
        and src_stat.st_gid == target_stat.st_gid
    )


def dedupe_hardlinks(target_dir):
    # -r is --recursive for some versions and --recurse on more recent versions.
    dupes = subprocess.check_output(["fdupes", "-r", str(target_dir)]).decode(
        "utf-8"
    )
...
def create_fs_image(img_path, src_path, label=None):
    ...
    du_output = subprocess.check_output(["du", "-bsx", src_path]).decode(
        "utf-8"
    )
...
def repack_rootfs(output_dir, disk_path):
    with tempfile.TemporaryDirectory() as tempdir_path:
        ...
        if len(images) > 0:
            ...
            shutil.copy(
                str(rootfs_dir / "boot" / "vmlinuz"), str(output_kernel)
            )
        ...
        credits_path = (
            rootfs_dir / "opt/google/chrome/resources/about_os_credits.html"
        )
        ...
        try:
            ...
            with gzip.open(
                credits_path.with_suffix(".html.gz"), "rb"
            ) as decompressed:
        ...
        if tools_dir.exists():
            ...
            create_fs_image(
                output_dir / "vm_tools.img", tools_dir, label="cros-vm-tools"
            )
...
def main():
    ...
    parser.add_argument(
        "image", help="source disk image to build the Termina image from"
    )
