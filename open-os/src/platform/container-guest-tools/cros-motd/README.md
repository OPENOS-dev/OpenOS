# About
cros-motd provides a way to present Crostini users with a textual message printed into the Terminal app (or
any other terminal emulator launching a "login" shell in the Crostini container) upon startup.

The message is defined in the same script (cros-motd.sh) that contains the presentation logic and will only
...
The cros-motd concept is delivered as a versioned Debian package, to be automatically installed into the
...
`COUNTER_MAX` and `COUNTER_INITIAL` constants in the `cros-motd.sh` file accordingly.
