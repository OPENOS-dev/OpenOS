...
Inside chroot, do:
    ...
    cros build-image --board $BOARD factory_install
...
by a prompt to select an action.
    ...
    Please select an action and press Enter.
      ...
      b Battery cutoff       Performs a battery cutoff
...
Please check [Edit lsb-factory config in an RMA shim](https://chromium.googlesource.com/chromiumos/platform/factory/+/HEAD/setup/RMA_SHIM.md#edit-lsb_factory-config-in-an-rma-shim) for more information.
...
can do this in `cros build-image`:
...
add `--boot-args` as explained in previous section). Some devices may need
