...
main() {
  tty_init
  # tty_init will delete frecon console, so we need to
  # redirect logs to the log file.
  exec 2>"${LOG_FILE}"
  exec 1>"${LOG_FILE}"
  ...
  local omahaserver
  omahaserver="$(kernel_get_var omahaserver)"
  ...
  # Substituting `busybox awk` with `mawk` as `busybox awk` not working on
  # boards like dedede. See b/329532904 for details.
  ln -sf /usr/bin/mawk /bin/awk
