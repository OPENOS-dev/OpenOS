...
def _git_apply_patch(patch: str, use_git_apply: bool, threeway: bool,
    ...
    if use_git_apply:
        # `git apply` is more lenient than `git am` and accepts patches without
        # commit metadata. Manually try to detect and report these:
        with open(patch, 'r') as f:
            first_line = f.readline()
            if not first_line.startswith('From '):
                raise RuntimeError(
                    f"Patch {patch} is in an invalid format. "
                    "Make sure it's generated by `git format-patch`.")
