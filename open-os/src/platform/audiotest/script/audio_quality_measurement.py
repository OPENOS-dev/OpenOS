...
def hilbert_analysis(signal, rate, block_size):
    ...
    frequency = numpy.append(frequency, frequency[len(frequency) - 1])
...
def delay_detection(
    ...
    for index in range(start_index, end_index):
        if (
            block_amplitude[index]
            > average_amplitude * delay_amplitude_threshold
        ):
            continue
        ...
        right_threshold = (
            delay_amplitude_threshold * right_block_amplitude[index]
        )
...
def burst_detection(
    ...
    for index in range(start_index, end_index):
        ...
        if (
            block_amplitude[index]
            <= average_amplitude * DEFAULT_BURST_TOO_SMALL
        ):
            continue
        ...
        right_threshold = (
            burst_amplitude_threshold * right_block_amplitude[index]
        )
...
def quality_measurement(
    ...
    if not dominant_frequency:
        dominant_frequency = audio_analysis.spectral_analysis(signal, rate)[0][
            0
        ]
    ...
    teager_value = average_teager_value(
        signal[start_index:end_index], average_amplitude
    )

    # Finds out the noise level.
    noise = noise_level(
        average_amplitude, dominant_frequency, rate, teager_value
    )
