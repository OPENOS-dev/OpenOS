...
impl SecureBootKeyPaths {
    ...
    pub fn priv_ecdsa_pem(&self) -> Utf8PathBuf {
        self.dir.join("key_ecdsa.priv.pem")
    }
}
...
/// Set up the ECDSA key used for signing the second-stage bootloader
/// so that crdyshim can verify it.
///
/// This uses the test key from the vboot repo rather than generating a
/// new one.
pub fn prepare_ecdsa_key(conf: &Config) -> Result<()> {
    ...
    fs::copy(
        devkeys_dir.join("uefi/crdyshim-v2.priv.pem"),
        paths.priv_ecdsa_pem(),
    )?;
...
/// Created a detached ECDSA signature of `src`. The signature is
/// written to `dst`.
pub fn create_detached_signature(
    ...
    let sha256_path = dst.with_extension("sha256");

    // Calculate the hash and write to a binary file. This could be
    // skipped by passing `-rawin` to the `pkeyutl` command below, but
    // doing it this way more closely matches the signing code in
    // vboot_reference.
    #[rustfmt::skip]
    Command::with_args("openssl", [
        "dgst", "-sha256", "-binary",
        "-out", sha256_path.as_str(),
        src.as_str(),
    ]).run()?;
    ...
    Command::with_args("openssl", [
        "pkeyutl", "-sign",
        "-in", sha256_path.as_str(),
        "-inkey", key_paths.priv_ecdsa_pem().as_str(),
        "-out", dst.as_str(),
    ]).run()?;
...
/// Check the dev and official public keys used by crdyshim to make sure
/// they match the PEM files.
pub fn check_crdyshim_public_keys(conf: &Config) -> Result<()> {
    /// Check that the SEC-1 encoded public key at `sec1_path` matches
    /// the PEM-encoded public key in `pem`.
    fn check_public_key(pem: &str, sec1_path: &Utf8Path) -> Result<()> {
        // Strip off metadata after the public key so that it can be parsed.
        let marker = "-----END PUBLIC KEY-----";
        let index = pem.find(marker).context("missing end public key marker")?;
        let pem = &pem[..index + marker.len()];

        // Parse the PEM and convert to SEC1.
        let dev_key: p256::ecdsa::VerifyingKey =
            pem.parse().map_err(|_| anyhow!("invalid PEM file"))?;
        let expected_sec1 = dev_key.to_sec1_bytes();

        // Verify the SEC1 file has the correct contents.
        let actual_sec1 = fs::read(sec1_path)?;
        if *actual_sec1 != *expected_sec1 {
            // Write the expected contents to a temporary file to make
            // it easier to examine.
            let expected_path = Utf8Path::new("/tmp").join(sec1_path.file_name().unwrap());
            fs::write(&expected_path, expected_sec1)?;

            bail!(
                "crdyshim dev key at {} is incorrect: expected contents written to {}",
                sec1_path,
                expected_path,
            );
        }
    ...
    // Official ECDSA public key from
    // keyset/public/RevenMPKeys-v10/uefi/crdyshim-v2.pub.pem
    let official_public_key = "
-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEIRSKY61W52tNPbNFZVcxgRkoSKz6
04tHxsA35BVLVsiZvwbyKfQfMNLW/tcQlUwdUuTYxBXkjFqu9X66iDemIQ==
-----END PUBLIC KEY-----
Public-Key: (256 bit)
pub:
    04:21:14:8a:63:ad:56:e7:6b:4d:3d:b3:45:65:57:
    31:81:19:28:48:ac:fa:d3:8b:47:c6:c0:37:e4:15:
    4b:56:c8:99:bf:06:f2:29:f4:1f:30:d2:d6:fe:d7:
    10:95:4c:1d:52:e4:d8:c4:15:e4:8c:5a:ae:f5:7e:
    ba:88:37:a6:21
ASN1 OID: prime256v1
NIST CURVE: P-256
";
    check_public_key(
        official_public_key,
        Utf8Path::new("crdyshim/src/pubkey_official.sec1"),
    )?;
    check_public_key(
        &fs::read_to_string(conf.vboot_devkeys_path().join("uefi/crdyshim-v2.pub.pem"))?,
        Utf8Path::new("crdyshim/src/pubkey_dev.sec1"),
    )?;

    println!("all pubkey checks passed");
    Ok(())
}
