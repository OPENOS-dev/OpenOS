...
def main():
    ...
    * --board=BOARD --variant=VARIANT [--bug=BUG]
    """
    board, variant, bug, branch, trunk, continue_flag, abort_flag = get_args()

    if not check_flags(board, variant, bug, continue_flag, abort_flag):
        return False

    status = get_status(
        board, variant, bug, branch, trunk, continue_flag, abort_flag
    )
    if status is None:
        return False
    ...
    while status.step is not None:
        ...
        if not perform_step(status):
            logging.debug(
                "perform_step %s returned False; exiting ...", status.step
            )
            return False
...
def get_args():
    ...
    parser.add_argument(
        "--trunk", type=str, help="Trunk or root path name within chroot"
    )
    ...
    group.add_argument(
        ...
        action="store_true",
        dest="continue_flag",
        help="Continue the process from where it paused",
    )
    group.add_argument(
        "--abort",
        action="store_true",
        dest="abort_flag",
        help="Cancel the process and abandon all commits",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        dest="verbose_flag",
        help="Enable verbose output of progress",
    )
    ...
    trunk = args.trunk or "chromiumos"
...
def check_flags(board, variant, bug, continue_flag, abort_flag):
    ...
    if continue_flag or abort_flag:
        if board is not None or variant is not None or bug != "None":
            logging.error(
                "Do not use --board, --variant, or --bug with "
                "--continue or --abort"
            )
            return False
        return True
...
def get_status(board, variant, bug, branch, trunk, continue_flag, abort_flag):
    ...
    * branch - optional text for a BRANCH= value in the commit message for
        ...
        then None.
    * trunk - optional path for root folder within chroot.
        Default path will be chromiumos but in case of multiple instance
        path can be changed using --trunk argument.
    ...
    * commit - a map of maps that tracks all of the git commit and gerrit CL
    ...
    status.coreboot_reference = getattr(
        module, "coreboot_reference", status.board
    )
    status.cb_config_dir = getattr(module, "cb_config_dir", None)
    status.depthcharge_dir = getattr(
        module, "depthcharge_base", "platform/depthcharge"
    )
    status.depthcharge_base = getattr(
        module, "depthcharge_base", status.coreboot_base
    )
    status.depthcharge_ref = getattr(
        module, "depthcharge_ref", status.coreboot_base
    )
    ...
    status.fitimage_bin = getattr(module, "fitimage_bin", "fitimage-%s.bin")
    status.fitimage_dir = getattr(module, "fitimage_dir", None)
    status.fitimage_bin_dir = getattr(module, "fitimage_bin_dir", None)
    status.fitimage_versions_dir = getattr(
        module, "fitimage_versions_dir", status.fitimage_bin_dir
    )
    status.fitimage_versions = getattr(
        module, "fitimage_versions", "fitimage-%s-versions.txt"
    )
    status.fitimage_pkg = getattr(module, "fitimage_pkg", None)
    status.fitimage_cmd = getattr(module, "fitimage_cmd", None)
    status.fitimage_script = getattr(
        module, "fitimage_script", "files/add_fitimage.sh"
    )
...
def get_git_commit_data(cwd):
    ...
    return {"dir": cwd, "branch_name": branch_name, "change_id": change_id}
...
def project_config(status):
    ...
    try:
        if not emerge_with_workon(
            status,
            status.config_workon_pkgs,
            status.emerge_cmd,
            status.config_emerge_pkgs,
        ):
        ...
        logging.error(str(e))
        logging.error(
            "Please file a bug in ChromeOS > Infra"
            " to have the project configuration updated."
        )
        logging.error(
            "(go/cros-boxster-bug or "
...
def fw_build_config(status):
    ...
    if rc:
        status.commits[step_names.FW_BUILD_CONFIG] = get_git_commit_data(
            os.path.join(
                "/mnt/host/source/src/project", status.base, status.variant
            )
        )
    return rc
...
def create_coreboot_config(status):
    ...
    create_coreboot_config_sh = os.path.join(
        status.my_loc, "create_coreboot_config.sh"
    )
    rc = run_process(
        [
...
def create_depthcharge_variant(status):
    ...
    environ = {
        **os.environ,
        "DC_SRC_DIR": dc_src_dir,
        "NEW_VARIANT_BRANCH": status.branch,
    }
    create_depthcharge_variant_sh = os.path.join(
        status.my_loc, "create_depthcharge_variant.sh"
    )
    rc = run_process(
        [
...
def add_fitimage(status):
    ...
    add_fitimage_sh = os.path.expanduser(
        os.path.join(
            "/mnt/host/source/src", status.fitimage_dir, status.fitimage_script
        )
    )
    rc = run_process([add_fitimage_sh, status.variant, status.bug])
    if rc:
...
def check_fit_image_files(status):
    ...
    """
    fitimage_bin_dir = os.path.join(
        "/mnt/host/source/src", status.fitimage_dir, status.fitimage_bin_dir
    )
    logging.debug('fitimage_bin_dir = "%s"', fitimage_bin_dir)
    fitimage_versions_dir = os.path.join(
        "/mnt/host/source/src",
        status.fitimage_dir,
        status.fitimage_versions_dir,
    )
    ...
    if not os.path.isfile(
        os.path.join(fitimage_versions_dir, fitimage_versions_file)
    ):
...
def commit_fitimage(status):
    ...
    commit_fitimage_sh = os.path.expanduser(
        os.path.join(
            "/mnt/host/source/src",
            status.fitimage_dir,
...
def create_initial_ec_image(status):
    ...
    create_initial_ec_image_sh = os.path.join(
        status.my_loc, "create_initial_ec_image.sh"
    )
    if not run_process(
        [
...
def create_zephyr_ec_image(status):
    ...
    environ = {**os.environ, "NEW_VARIANT_BRANCH": status.branch}
    create_zephyr_ec_image_sh = os.path.join(
        status.my_loc, "create_zephyr_ec_image.sh"
    )
    if not run_process(
        [
...
def add_variant_to_public_yaml(status):
    ...
    add_variant_to_yaml_sh = os.path.join(
        status.my_loc, "add_variant_to_yaml.sh"
    )
    rc = run_process(
        [
...
def add_variant_to_private_yaml(status):
    ...
    add_variant_sh = os.path.expanduser(
        os.path.join(status.private_yaml_dir, "add_variant.sh")
    )
    rc = run_process([add_variant_sh, status.variant, status.bug])
    if rc:
...
def build_config(status):
    ...
    if not status.variant in osutils.ReadFile(config_yaml):
        logging.error(
            "variant name %s not found in yaml file %s",
            status.variant,
            config_yaml,
        )
        return False
    ...
    for commit_key in status.coreboot_push_list:
        ...
        if "gerrit" not in commit or "cl_number" not in commit:
            ...
            if cl is not None:
                ...
                logging.error(
                    "Please push the branch to review.coreboot.org, "
...
def find_coreboot_upstream(status):
    ...
    if step_names.CB_VARIANT not in status.commits:
        logging.error(
            "Key %s not found in status.commits", step_names.CB_VARIANT
        )
        return False
    if "change_id" not in status.commits[step_names.CB_VARIANT]:
        logging.error(
            "Key change_id not found in status.commits[%s]",
    ...
    gerrit_query_args = {"message": f"Change-Id:{change_id}"}
    ...
    for key in status.depends:
        ...
        for depend_key in status.depends[key]:
            ...
            if not "gerrit" in depend_commit:
                logging.error(
                    "Commit %s does not have a gerrit instance", depend_key
                )
                return False
            if not "cl_number" in depend_commit:
                logging.error("Commit %s does not have a CL number", depend_key)
                return False
...
def add_cq_depend_to_commit_msg(git_repo, change_id, cq_depend_str):
    ...
    if sha is None:
        logging.error(
            "Cannot find the SHA for Change-Id %s in %s", change_id, git_repo
        )
        return False
...
def add_cq_depend(status):
    ...
    for key in status.commits:
        ...
        if "cq_depend" in commit:
            ...
            if not add_cq_depend_to_commit_msg(
                commit["dir"], commit["change_id"], commit["cq_depend"]
            ):
                return False
...
def abort(status):
    ...
    for step in status.commits:
        ...
        if (branch, cwd) in abandoned:
            logging.debug(
                "Branch %s in directory %s already abandoned", branch, cwd
            )
        else:
            logging.info("Abandoning branch %s in directory %s", branch, cwd)
