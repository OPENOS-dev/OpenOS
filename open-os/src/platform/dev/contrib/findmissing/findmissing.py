...
def findmissing():
    ...
    """
    abandon_restore_function_map = {
        "abandon": main.abandon_fix_cl,
        "restore": main.restore_fix_cl,
        "reopen": main.reopen_fix_cl,
        "status": main.status_fix_cl,
    }
    parser = argparse.ArgumentParser(
        description="Local functions to update database"
    )
    parser.add_argument(
        "command",
        type=str,
        choices=tuple(abandon_restore_function_map.keys()),
        help="Function to either abandon/restore/reopen changes.",
    )
    parser.add_argument(
        "-t",
        "--table",
        type=str,
        choices=("stable", "chrome"),
    ...
    parser.add_argument(
        "-r",
        "--reason",
        required="status" not in sys.argv,
        type=str,
    ...
    abandon_restore_function_map[args.command](
        fixes_table, args.sha, args.reason, args.force
    )
