...
PATCHWORK_URLS = (
    ("https://patchwork.kernel.org", "rest"),
    ("https://patchwork.ozlabs.org", "xmlrpc"),
    ("https://patchwork.freedesktop.org", "xmlrpc"),
)
...
def _get_patch_from_message_id(args, message_id):
    for url_template, mbox_suffix in [
        ...
        try:
            headers = {
                "User-Agent": "fromupstream.py (ChromeOS)",
            }
            ...
            if (opener.url != this_url) and mbox_suffix:
                ...
                req = urllib.request.Request(
                    opener.url + mbox_suffix, headers=headers
                )
                opener = urllib.request.urlopen(req)
                ...
                if mo is not None:
                    l, m = mo.groups()
                    errprint(
                        "Warning: multiple hits on marc.info; picking %s" % l
                    )
...
def _match_msgid(match, args):
    ...
    url = None
    for url, approach in PATCHWORK_URLS:
        res = None
        ...
        if approach == "xmlrpc":
            ...
            # See https://patchwork.readthedocs.io/en/latest/api/rest/schemas/v1.3/#get--api-1.3-patches.
            rest_url = url + "/api/patches?msgid=" + msgid
            try:
                with urllib.request.urlopen(rest_url) as response:
                    res = json.loads(response.read())
            except urllib.error.HTTPError as e:
                errprint(
                    'Error: server "%s" gave an error "%s", skipping' % (url, e)
                )
...
def _match_gitweb(match, args):
    ...
    # Web navigations to https://git.kernel.org starts being redirected to the
    # new URL https://web.git.kernel.org, while the Git remote repository
    # URL remains unchanged, which causes Git fetch to fail.
    # Remove the extra "web." prefix to adapt the URL redirect update.
    remote = re.sub(
        r"^https://web\.git\.kernel\.org", "https://git.kernel.org", remote
    )
...
def main(args):
    ...
    parser.add_argument(
        ...
        default="N/A",
        type=str,
        help="Kernel branches where the fix should be dispatched. Set N/A if "
        "the patch should not land in any version "
        "(see go/kernel-cl-dispatch-docs).",
    )
    ...
    for location in args["locations"]:
        ...
        commit_message += "\nBranches: %s" % args["branches_dispatch"]
