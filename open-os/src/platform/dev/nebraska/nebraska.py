...
class Request(object):
    ...
    def ParseRequest(self):
        ...
        update_check_count = len(
            [
                x
                for x in app_elements
                if x.find(self.UPDATE_CHECK_TAG) is not None
            ]
        )
        ...
        def _CheckAttributesAndReturnIt(
            attribute, in_all=False, ignore_value=None
        ):
            ...
            if len(unique_attrs) > 1:
                raise InvalidRequestError(
                    'Attribute "%s" is not the same in all app tags.'
                    % attribute
                )
        ...
        if self.request_type == Request.RequestType.UPDATE:
            ...
            self.version = _CheckAttributesAndReturnIt(
                self.APP_VERSION_ATTR, in_all=True
            )
        else:
        ...
        if self.track is None or self.board is None:
            raise InvalidRequestError(
                "Either track(%s) or board(%s) attributes are "
    ...
    class AppRequest(object):
        ...
        def __str__(self):
            ...
            if self.request_type == Request.RequestType.EVENT:
                ...
                return "%s update %s from v%s" % (
                    "delta" if self.delta_okay else "full",
                    self.appid,
                    self.version,
                )
        ...
        def ParseApp(self, app):
            ...
            self.rollback_allowed = (
                self.has_update_check
                and update_check.get(Request.ROLLBACK_ALLOWED_ATTR) == "true"
            )
            ...
            if event is not None:
                ...
                self.previous_version = event.get(
                    Request.EVENT_PREVIOUS_VERSION_ATTR
                )
...
class Response(object):
    ...
    def __init__(self, request, config):
        ...
        self._elapsed_seconds = int(
            (
                curr - datetime.datetime.combine(curr.date(), datetime.time.min)
            ).total_seconds()
        )

        if self._request.request_type in (
            Request.RequestType.INSTALL,
    ...
    def GetXMLString(self):
        ...
        try:
            ...
            ElementTree.SubElement(
                ...
                "daystart",
                attrib={
                    "elapsed_days": str(self._elapsed_days),
            ...
            for app_request in self._request.app_requests:
                response_xml.append(
                    self.AppResponse(
                        app_request, self._config, matched_apps
                    ).Compile()
                )
    ...
    class AppResponse(object):
        ...
        def __init__(self, app_request, config, matched_apps):
            ...
            if self._app_data:
                ...
                logging.debug(
                    "No matching updates payload available for App ID %s",
                    self._app_request.appid,
                )
            elif (
                self._app_request.has_update_check
                and self._app_request.request_type
                == Request.RequestType.INSTALL
            ):
                logging.debug(
                    "No matching install payload available for App ID %s",
                    self._app_request.appid,
                )
        ...
        def Compile(self):
            ...
            if self._app_request.ping:
                ElementTree.SubElement(
                    app_response, "ping", attrib={"status": "ok"}
                )
            if self._app_request.event_type is not None:
                ElementTree.SubElement(
                    app_response, "event", attrib={"status": "ok"}
                )
            ...
            if self._app_data is not None:
                ...
                if self._config.extended_date is not None:
                    update_check_attribs["_extended_date"] = str(
                        self._config.extended_date
                    )
                if self._config.extended_opt_in_required:
                    update_check_attribs["_extended_opt_in_required"] = "true"
                ...
                action = ElementTree.SubElement(
                    ...
                    "action",
                    attrib={
                        "ChromeOSVersion": self._app_data.target_version,
                        "ChromeVersion": "1.0.0.0",
                        "DisablePayloadBackoff": str(
                            self._config.disable_payload_backoff
                        ).lower(),
                ...
                ElementTree.SubElement(
                    ...
                    "package",
                    attrib={
                        "fp": "1.%s" % self._app_data.sha256_hex,
            ...
            # response. Just a no update in the app tag's status attribute.
                ...
                and not self._app_request.has_update_check
            ):
...
class AppIndex(object):
    ...
    def _Scan(self):
        ...
        for f in os.listdir(self._directory):
            if f.endswith(".json"):
                try:
                    with open(
                        os.path.join(self._directory, f), "r"
                    ) as metafile:
                    ...
                    logging.error(
                        "Failed to read app data from %s (%s)", f, str(err)
                    )
                    raise
    ...
    def Find(self, request, matched_apps, full_payload, ignore_appid=False):
        ...
        matches = [
            app_data
            for app_data in self._index
            if request.MatchAppData(app_data)
        ]
        ...
        if not matches:
            matches = [
                app_data
                for app_data in self._index
                if request.MatchAppData(app_data, check_against_canary=True)
            ]
        ...
        if not matches:
            ...
            matches = [
                app_data
                for app_data in self._index
                if request.MatchAppData(app_data, partial_match_appid=True)
            ]
        ...
        if not matches and ignore_appid:
            matches = [
                app_data
                for app_data in self._index
                if request.MatchAppData(app_data, ignore_appid=True)
            ]

        # Now remove App ID matches that have already been matched by other
        # requests.
        matches = [
            app_data for app_data in matches if app_data not in matched_apps
        ]

        if full_payload is False and not request.delta_okay:
            logging.error(
                "The update client indicated that it can not accept a delta"
        ...
        if full_payload or not request.delta_okay:
            ...
            match = (
                delta_match
                if full_payload is False or delta_match
                else full_match
            )
    ...
    class AppData(object):
        ...
        def __init__(self, app_data):
            ...
            if len(self.appid) >= len(_CANARY_APP_ID):
                self.canary_appid = (
                    _CANARY_APP_ID + self.appid[len(_CANARY_APP_ID) :]
                )
...
class Config(object):
    ...
    def __init__(self):
        ...
        # The number of days from unix epoch when extended auto update period
        # starts.
        self.extended_date = None
        # Indicates that the device requires opt-in to enter extended auto
        # updates.
        self.extended_opt_in_required = None
...
class Nebraska(object):
    ...
    def GetResponseToRequest(self, request):
        ...
        response_str = minidom.parseString(response).toprettyxml(
...
class NebraskaServer(object):
    ...
    class NebraskaHandler(server.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                self.send_error(
                    http.client.BAD_REQUEST, b"Invalid request (header)."
                )
                return
            ...
            try:
                ...
                logging.error(traceback.format_exc())
                self.send_error(
                    http.client.INTERNAL_SERVER_ERROR, traceback.format_exc()
                )
    ...
    def Start(self):
        ...
        if self._runtime_root:
            ...
            try:
                ...
                if err.errno == errno.EACCES:
                    print(
                        "Permission error: You need to run the script as root/sudo or "
        ...
        logging.info(
            "Started nebraska on port %d and pid %d.", self._port, os.getpid()
        )
    ...
    def Stop(self):
        ...
        if self._created_runtime_root:
            try:
                ...
                logging.warning(
                    "Failed to remove directory %s with error %s",
                    self._runtime_root,
                    e,
                )
...
def main(argv):
    ...
    update_payloads_address = os.path.join(
        opts.update_payloads_address or "", ""
    )
    install_payloads_address = (
        os.path.join(opts.install_payloads_address or "", "")
    ...
    nebraska_server = NebraskaServer(
        nebraska, runtime_root=opts.runtime_root, port=opts.port
    )
