...
def fetch(myuris, mysettings, listonly=0, fetchonly=0,
	...
	if "nomirror" in restrict:
		print(_("!!! Use RESTRICT=mirror, not RESTRICT=nomirror.\n"))
		raise PortageException("Invalid RESTRICT setting")
		...
		if myfile not in filedict:
			...
			for y in range(0,len(locations)):
				filedict[myfile].append(
					locations[y] + "/distfiles/" + (
					urlquote(myfile) if locations[y].startswith('http') else myfile))
		if myuri is None:
			continue
		if force_mirror:
			continue
