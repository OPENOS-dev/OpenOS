...
def _doebuild_spawn(phase, settings, actionmap=None, **kwargs):
	...
	artifacts_tmp_dir = settings["CROS_ARTIFACTS_TMP_DIR"]

	if phase == "clean":
		shutil.rmtree(artifacts_tmp_dir, ignore_errors=True)
	if phase == "setup":
		...
		os.makedirs(artifacts_tmp_dir, exist_ok=True)
		os.chmod(artifacts_tmp_dir, 0o777)
	...
	try:
		...
		if phase == "postinst":
			...
			if not is_empty:
				...
				src = artifacts_tmp_dir
				dst = os.path.join(artifacts_dir,
					os.path.basename(artifacts_tmp_dir))
				# src and dst may be on different filesystems
				# (different bind mounts), so os.rename()
				# doesn't work. We'd like to use shutil.move(),
				# but portage wraps and overrides it with a
				# string encoding wrapper that breaks
				# shutil.move() when used with directories. See
				# https://bugs.gentoo.org/914722 for more info.
				# Thus, we're left open-coding shutil.move()
...
def spawn(mystring, mysettings, debug=False, free=False, droppriv=False,
	...
	# CHROMIUM: We want to allow fakeroot to execute when UID = 0.
