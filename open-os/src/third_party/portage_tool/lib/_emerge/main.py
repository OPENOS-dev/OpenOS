...
@tracer.start_as_current_span("parse_opts")
def parse_opts(tmpcmdline, silent=False):
	...
	actions = frozenset([
		...
		"prune", "rage-clean", "rage-install", "regen",  "search",
	...
	argument_options = {
		...
		"--with-bdepends": {
			"help":"include BDEPENDS dependencies",
			"choices":("y", "n")
		},
		"--reinstall": {
		...
		"--root-cros-vdb-strip-prefix": {
		 "help"   : "specify the root prefix used to find the vdb root",
		 "action" : "store"
		},
...
@tracer.start_as_current_span("emerge_main")
def emerge_main(args=None):
	...
	if "--root-cros-vdb-strip-prefix" in myopts:
		os.environ["ROOT_CROS_VDB_STRIP_PREFIX"] = myopts["--root-cros-vdb-strip-prefix"]
	if "--prefix" in myopts:
