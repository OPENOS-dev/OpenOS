...
class depgraph(object):
	...
	def _slot_operator_check_reverse_dependencies(self, existing_pkg, candidate_pkg,
	...
	def _slot_operator_update_probe(self, dep, new_child_slot=False,
		...
		for replacement_parent in self._iter_similar_available(dep.parent,
			...
			if not self._slot_operator_check_reverse_dependencies(dep.parent,
				replacement_parent):
				continue
			...
			for atom in atoms:
				...
				for pkg in self._iter_similar_available(
					...
					if not insignificant and \
						self._slot_operator_check_reverse_dependencies(dep.child,
							pkg, replacement_parent=replacement_parent):
	...
	def _add_pkg_deps(self, pkg, allow_unsatisfied=False):
		...
		ignore_bdepend_deps = self._dynamic_config.myparams.get("bdepends", "y") == "n"
		ignore_hdepend_deps = ignore_bdepend_deps
	...
	def _show_unsatisfied_dep(self, root, atom, myparent=None, arg=None,
		...
		for pkg in missing_use:
			...
			if not missing_iuse and myparent and atom.unevaluated_atom.use.conditional:
				...
				if not violated_atom.use:
					msg = [
						"violated_atom.use not set:",
						str(violated_atom),
					]
					message = "\n".join(msg)
					writemsg_level(f"{message}\n", noiselevel=-1, level=logging.WARNING)
					continue
...
def _spinner_start(spinner, myopts):
	...
	if show_spinner:
		spinner.start_time = datetime.datetime.utcnow()
...
def _spinner_stop(spinner):
	...
	portage.writemsg_stdout("... done in %s\n" % (end_time - spinner.start_time))
