...
from portage.util import normalize_path, writemsg_stdout
...
class Display(object):
	...
	def write_counters(self):
		...
		counters_file = logdir / "depgraph_counters.log"
		try:
			with open(counters_file, "a") as f:
				f.write(repr(self.counters) + "\n")
		except Exception as e:
			writemsg_stdout(f"!!! Error writing package counters from depgraph analysis to file. Error info: {str(e)}")
			# soft failure should be the desired behavior here.
			pass
	...
	def __call__(self, depgraph, mylist, favorites=None, verbosity=None):
		...
		# write depgraph counters to file. For now, do not write counters
		# if --pretend is passed via command.
		if "--pretend" not in depgraph._frozen_config.myopts:
			self.write_counters()
