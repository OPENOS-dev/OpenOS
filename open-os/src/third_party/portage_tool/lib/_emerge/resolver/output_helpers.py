...
class _PackageCounters(object):
	...
	def __repr__(self):
		"""Provide a structured string representation of depgraph results
		for consumption by logs and/or telemetry."""
		total_installs = self.upgrades + self.downgrades + self.newslot + self.new + self.reinst
		result = {"upgraded_packages": self.upgrades,
				"downgraded_packages": self.downgrades,
				"new_packages": self.new,
				"new_slot": self.newslot,
				"reinstalled_packages": self.reinst,
				"uninstalled_packages": self.uninst,
				"packages_blocked_by_conflict": self.blocks,
				"packages_blocked_by_conflict_then_satisfied": self.blocks_satisfied,
				"packages_with_unsatisfied_conflicts": self.blocks - self.blocks_satisfied,
				"total_installed_packages": total_installs,
				"total_installed_from_binary": self.binary,
				"sys.argv": sys.argv,
		}
		# currently omitted: totalsize, restrict_fetch, restrict_fetch_satisfied, interactive
