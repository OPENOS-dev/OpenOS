...
private:
	...
	void cutout(ICeiImage** ppOut, long back);
...
void CFrameworkEndSequence::cutout(ICeiImage** ppOut, long back)
{
	...
	if (m_psettings->card_scan()) {
		if (back) {
			...
			long new_width = window.width() * window.xdpi() / mode.mud();
			ceisdk_cutout_simple(ppOut,
				pimg->width() - new_width,
				0,
				new_width,
				window.length() * window.ydpi() / mode.mud());
