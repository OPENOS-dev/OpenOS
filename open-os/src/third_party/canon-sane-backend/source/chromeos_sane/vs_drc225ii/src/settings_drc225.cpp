	...
	m_fpga(1)
{
	...
	m_inqex[TO_SCANNER].cdb()[4] = 48;
	m_inqex[TO_SCANNER].transfer_length(m_inqex[TO_SCANNER].cdb()[4]);
	CScsiCommand scanner(m_info.pscanner);
	scanner.read(m_inqex[TO_SCANNER]);
	m_inqex[FROM_CLIENT] = m_inqex[TO_SCANNER];
	if (m_inqex[TO_SCANNER].data()[32] && 0x4) {
		std::vector<char>buf(0x8000, 0);
		CStreamCmd s((char*)&buf[0], (long)buf.size());
		s.transfer_data_type(0xaa);
		scanner.read(s);
		if (strstr(&buf[0], "ScannerFunctions") &&
			strstr(&buf[0], "Value") &&
			strstr(&buf[0], "F3F1B2F2B3B1")) {
			m_fpga = 0;
...
long CSettingsFramework::has_fpga()
{
	return m_fpga;
}
