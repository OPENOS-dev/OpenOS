...
public:
	...
	void on_page_end(ICeiMessage *pmsg);
		...
		void on_page_end_simplex_fpga();
		void on_page_end_duplex_fpga();
			void on_page_end_duplex_fpga_main();
			void on_page_end_duplex_fpga_end();
...
private:
	...
	void convert_rate(ICeiImage* pIn);
...
{
 //   struct stat stDirectory;
	//g_isWriteDebugImg = stat("/tmp/VSWriteDebugImg", &stDirectory)==0 ? true:false;
...
void CFrameworkEndSequence::on_image(ICeiMessage *pmsg)
{
	...
	//WriteLog((char*)"CFrameworkEndSequence::on_image() p=%p", p);
}
...
void CFrameworkEndSequence::resolution_convert(ICeiImage **ppInOut)
{
	...
	resolution_convert_internal(ppInOut, dst_w, dst_h, w.xdpi(), w.ydpi());
}

void CFrameworkEndSequence::convert_rate(ICeiImage* pIn)
{
	unsigned char gamma[256] = { 0 };
	const unsigned int RATE_FOR_SHADING = 1430;
	if (RATE_FOR_SHADING == 0) return;

	for (int i = 0; i < 256; i++) {
		int val = i * RATE_FOR_SHADING / 1000;
		gamma[i] = val > 255 ? 255 : val;
	}
	if (pIn->spp() == 3) {
		CImageWrapper iw(pIn);
		iw.sync(pIn->sync() * 3);
		ceisdk_gamma(pIn, gamma, 256);
	}
	else {
		ceisdk_gamma(pIn, gamma, 256);
	}
...
void CFrameworkEndSequence::on_page_end_simplex()
{
	...
	ICeiImage* pimg = NULL;
	if (scanparamr.autosize()) drc225toimage::toImage_autosize(m_imgPieces, &pimg, window);
	else                       drc225toimage::toImage_fixedsize(m_imgPieces, &pimg, window);
	sort_simplex(&pimg);
	trim_invalid_pixel(&pimg);
	ceisdk_mirror_lineorder(pimg);
	shading_front(pimg);
...
void CFrameworkEndSequence::on_page_end_simplex_fpga()
{
	CEISDK_POINT pos[4] = { 0 };
	CWindow window;
	CScanParam scanparamr = m_psettings->scanParam_bothr();
	...
	window.length(window_from_client.length());
	ICeiImage* pimg = NULL;
	if (scanparamr.autosize()) drc225fpgatoimage::toImage_autosize(m_imgPieces, &pimg, window);
	else                       drc225fpgatoimage::toImage_fixedsize(m_imgPieces, &pimg, window);
	detect_4points(pimg, pos, FRONT);
	convert_rate(pimg);
	ceisdk_to_pixelorder_simple(&pimg);
	autosize_deskew(&pimg, pos);
...
void CFrameworkEndSequence::on_page_end_duplex_main()
{
	...
	ICeiImage* back = NULL;
	if (scanparamr.autosize()) drc225toimage::toImage_autosize(m_imgPieces, &front, &back, window[0]);
	else                       drc225toimage::toImage_fixedsize(m_imgPieces, &front, &back, window[0]);
	ceisdk_mirror_lineorder(front);
	shading_front(front);
...
void CFrameworkEndSequence::on_page_end_duplex_fpga_main()
{
	CEISDK_POINT pos[4 + 4] = { 0 };
	CWindow window[2];
	CMode mode;
	...
	ICeiImage* front = NULL;
	ICeiImage* back = NULL;
	if (scanparamr.autosize()) drc225fpgatoimage::toImage_autosize(m_imgPieces, &front, &back, window[0]);
	else                       drc225fpgatoimage::toImage_fixedsize(m_imgPieces, &front, &back, window[0]);
	detect_4points(front, pos, FRONT);
	convert_rate(front);
	cutoffset(&back);
	detect_4points(back, pos + 4, BACK);
...
void CFrameworkEndSequence::on_page_end_duplex_fpga()
{
	on_page_end_duplex_fpga_main();
	on_page_end_duplex_fpga_end();
}
void CFrameworkEndSequence::on_page_end_duplex_fpga_end()
{
	on_page_end_duplex_end();
}
...
void CFrameworkEndSequence::on_page_end(ICeiMessage *pmsg)
{
	...
	try {
		...
		if (scancmd_to_scanner.duplex()) {
			if (m_psettings->has_fpga()) {
				on_page_end_duplex_fpga();
			}
			else {
				on_page_end_duplex();
			}
