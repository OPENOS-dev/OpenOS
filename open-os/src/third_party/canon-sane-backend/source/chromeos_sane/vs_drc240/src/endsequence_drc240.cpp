...
namespace {
	...
	inline short max_dpi(short xdpi, short ydpi) {
...
private:
	...
	void resolution_convert(ICeiImage **ppOut);
...
void CFrameworkEndSequence::gamma(ICeiImage* pIn)
{
	...
	w = m_psettings->window(CSettingsFramework::FROM_CLIENT);
	if (pIn->spp()==3) {
		ceisdk_gamma(pIn, ceisdk_get_gamma_table_color, w.brightness(), range255to7(w.contrast()));
	}
	else {
		if (w.bps() == 8) {
			ceisdk_gamma(pIn, ceisdk_get_gamma_table_gray, w.brightness(), range255to7(w.contrast()));
		}
		else {
			ceisdk_gamma(pIn, ceisdk_get_gamma_table_bw, w.brightness(), range255to7(w.contrast()));
		}
	}
}
void CFrameworkEndSequence::resolution_convert(ICeiImage** ppOut)
{
	...
	resolution_convert_internal(ppOut, dst_w, dst_h, w.xdpi());
}
...
void CFrameworkEndSequence::on_page_end_simplex()
{
	...
	ICeiImage* pimg = NULL;
	if (scanparamr.autosize()) toImage_autosize(m_imgPieces, &pimg, window[0]);
	else                       toImage_fixedsize(m_imgPieces, &pimg, window[0]);
	sort_simplex(&pimg);
...
void CFrameworkEndSequence::on_page_end_duplex_main()
{
	...
	CScanParam scanparamr = m_psettings->scanParam_bothr();
	CWindow window[CSettingsFramework::S_C_COUNT];
	window[CSettingsFramework::TO_SCANNER] = m_psettings->window(CSettingsFramework::TO_SCANNER);
	window[CSettingsFramework::FROM_CLIENT] = m_psettings->window(CSettingsFramework::FROM_CLIENT);
	window[CSettingsFramework::TO_SCANNER].length(window[CSettingsFramework::FROM_CLIENT].length()+get_scanner_specific_offset(1200));
	ICeiImage* front = NULL;
	ICeiImage* back = NULL;
	if (scanparamr.autosize()) toImage_autosize(m_imgPieces, &front, &back, window[CSettingsFramework::TO_SCANNER]);
	else                       toImage_fixedsize(m_imgPieces, &front, &back, window[CSettingsFramework::TO_SCANNER]);
	cutoffset(&front);
...
void CFrameworkEndSequence::on_page_end(ICeiMessage *pmsg)
{
	...
	try {
		...
		WriteLog("std::bad_alloc is thrown:%s", ba.what());
		m_error = VS3_NOMEM;
		return;
	}
	catch(...)
