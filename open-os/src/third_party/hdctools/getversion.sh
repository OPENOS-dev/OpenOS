...
#   - in the absence of a git repository e.g. by building through cros_workon
...
#     (dirty repo)
#   - without a git repository, it defaults to the build time
...
gen_version_py() {
	...
	[[ -n "${USER}" ]] && BLDR="${USER}@$(hostname)" || BLDR="$(hostname)"
