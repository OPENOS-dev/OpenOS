...
"""Port Pigweed Unit Test for detokenize to servod."""
...
class DetokenizeTest(unittest.TestCase):
    ...
    def test_detokenize_zero_extend_short_token_with_no_args(self) -> None:
        ...
        self.assertEqual(str(detok.detokenize(b"\xab\xcd")), "This token is 16 bits")
    ...
    def test_empty_db(self) -> None:
        ...
        self.assertIn("unknown token", detok.detokenize(b"1234").error_message())
    ...
    def test_unknown_shorter_token(self) -> None:
        ...
        self.assertIn("unknown token 00030201", repr(detok.detokenize(b"\1\2\3")))
    ...
    def test_decode_from_elf_data(self) -> None:
        ...
        self.assertEqual(str(detok.detokenize(JELLO_WORLD_TOKEN)), "Jello, world!")
...
class NestedMessageParserTest(unittest.TestCase):
    ...
    def test_transform_io(self) -> None:
        for data, expected, title, transform in self.TRANSFORM_TEST_CASES:
            self.assertEqual(
                expected,
                b"".join(self.decoder.transform_io(io.BytesIO(data), transform)),
                f"{title}: {data!r}",
            )
    ...
    def test_transform_bytes_sequential(self) -> None:
        ...
        self.assertEqual(self.decoder.transform(b"bc", transform, flush=True), b"*ABC")
    ...
    def test_read_messages_flush(self) -> None:
        self.assertEqual(list(self.decoder.read_messages(b"123$a")), [(False, b"123")])
...
class DetokenizeNestedDomains(unittest.TestCase):
    ...
    def test_nested_hashed_arg_with_two_domain_match(self) -> None:
        detok = detokenize.Detokenizer(
            tokens.Database(
                [
                    tokens.TokenizedStringEntry(0xA, "and this is domain1", "D1"),
                    tokens.TokenizedStringEntry(
        ...
        detok = detokenize.Detokenizer(
            tokens.Database(
                [
                    ...
                    tokens.TokenizedStringEntry(4, "nested " + "${D2}#00000003", "D5"),
                ]
            )
        )
        self.assertEqual(
    ...
    def test_nested_hashed_arg_with_domain_whitespace(self) -> None:
        detok = detokenize.Detokenizer(
            tokens.Database(
                [
                    tokens.TokenizedStringEntry(0xA, "and this is domain1", "Domain1"),
                    tokens.TokenizedStringEntry(
