...
# Servod Test Plan: firmware.FWAutoupdate.ac_ro

This test plan emulates the hardware-level interactions of the `firmware.FWAutoupdate.ac_ro` Tast test.
...
dut-control dut_pd_data_role:DFP
dut-control charger_attached
dut-control gsc_uart_regexp:"['RW_[AB]: *\\* *(\\d+).(\\d+).(\\d+)/(\\S+)[\\r\\n]']"
