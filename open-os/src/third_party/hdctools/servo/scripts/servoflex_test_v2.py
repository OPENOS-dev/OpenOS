...
FLASHROM_PASS = ["probe_spi_rems: id1 0xbf, id2 0x48", "Found Generic flash chip"]
FLASHROM_FAIL = None
        ...
        for row_idx in range(3):
            ...
            for set_val in [GPIO_MAPS[ctrls[kbd_col]], ctrls[kbd_col]]:
                ...
                if ctrls[kbd_col] != set_val:
                    logging.error(
                        "After setting %s, %s != %s", kbd_row, kbd_col, set_val
                    )
                    errors += 1
        ...
        for set_name, get_name in gpios_to_test.items():
            ...
            if not retval:
                ...
                if ctrl[get_name] != set_val:
                    logging.error(
                        "GPIO %s from %s -> %s", set_name, all_ctrls[set_name], set_val
                    )
                    errors += 1
                else:
                    logging.debug(
                        "Done GPIO %s from %s -> %s",
                        set_name,
                        all_ctrls[set_name],
                        set_val,
                    )
