...
def find_type_in_file(
    proto_file: FileDescriptorProto, type_name: str
) -> Optional[Union[DescriptorProto, EnumDescriptorProto]]:
...
) -> Tuple[FileDescriptorProto, Union[DescriptorProto, EnumDescriptorProto]]:
...
def import_type(
    imports: List[str], type: str, local: Optional[FileDescriptorProto]
) -> Tuple[str, Union[DescriptorProto, EnumDescriptorProto], str]:
...
def collect_type(
    ...
    local: Optional[FileDescriptorProto],
) -> Tuple[str, str, str]:
    dft: str
    dft_import: str = ""
        ...
        if len(parts) == 2:
            ...
            for nested_type in parent.nested_type:
                if nested_type.name == type:
                    ...
                    key_type, _, _ = collect_type(
                        imports, nested_type, nested_type.field[0], local
                    )
                    val_type, _, _ = collect_type(
                        imports, nested_type, nested_type.field[1], local
                    )
...
def generate_enum(
    ...
    res: List[CodeGeneratorResponse.File],
) -> List[str]:
    res.append(
...
def generate_message(
    ...
    if args:
        args = ", " + args
    message_lines.extend([f"", f"  def __init__(self{args}) -> None: ...", f""])
    ...
    for oneof_name, oneof_fields in oneof.items():
        ...
        message_lines.extend(
            [
                f"  @property",
                f"  def {oneof_name}(self) -> {type}: ..." f"",
                f"  def {oneof_name}_variant(self) -> Union[{literals}, None]: ..." f"",
        ...
        return_variant = "\n  ".join(
            [
                f"if variant == '{name}': return unwrap(self.{name})"
                for name, _ in oneof_fields
            ]
        )
        return_asdict = "\n  ".join(
            [
                f"if variant == '{name}': return {{'{name}': unwrap(self.{name})}}  # type: ignore"
                for name, _ in oneof_fields
            ]
        )
        if return_variant:
...
def generate_service_method(
    ...
    if output_mode == "stream":
        if input_mode == "stream":
            ...
            if sync:
                ...
                add_import(
                    imports, f"from ._utils import AioStreamStream as StreamStream"
                )
        else:
        ...
        if not sync:
            add_import(imports, f"from typing import Awaitable")
    ...
    if input_mode == "stream":
        ...
        args = ", ".join(
            [f"{name}: {type} = {dft}" for name, type, dft in input_fields]
        )
...
def generate_service(
    ...
    methods = "\n\n    ".join(
        [
            "\n    ".join(generate_service_method(imports, file, service, method, sync))
...
def generate_servicer_method(
    imports: List[str], method: MethodDescriptorProto, sync: bool = True
) -> List[str]:
    input_mode = "stream" if method.client_streaming else "unary"
    output_mode = "stream" if method.server_streaming else "unary"
...
def generate_servicer(
    ...
    methods = "\n\n    ".join(
        [
            "\n    ".join(generate_servicer_method(imports, method, sync))
            for method in service.method
        ]
    )
    if not methods:
        methods = "pass"
    return (f"class {service.name}Servicer:\n" f"    {methods}\n").split("\n")


def generate_rpc_method_handler(
    imports: List[str], method: MethodDescriptorProto
) -> List[str]:
    input_mode = "stream" if method.client_streaming else "unary"
    output_mode = "stream" if method.server_streaming else "unary"
...
def generate_add_servicer_to_server_method(
    imports: List[str],
    file: FileDescriptorProto,
    service: ServiceDescriptorProto,
    sync: bool = True,
    ...
    method_handlers = "    ".join(
        [
            "\n    ".join(generate_rpc_method_handler(imports, method))
            for method in service.method
        ]
    )
...
for file_name in _REQUEST.file_to_generate:
    file: FileDescriptorProto = next(
        filter(lambda x: x.name == file_name, _REQUEST.proto_file)
    )
    ...
    enums = "\n".join(
        sum(
            [generate_enum(pyi_imports, file, enum, _FILES) for enum in file.enum_type],
            [],
        )
    )
    messages = "\n".join(
        sum(
            [
                generate_message(pyi_imports, file, message, _FILES)
    ...
    services = "\n".join(
        sum(
            [generate_service(grpc_imports, file, service) for service in file.service],
            [],
        )
    )
    aio_services = "\n".join(
        sum(
            [
                generate_service(grpc_aio_imports, file, service, False)
    ...
    servicers = "\n".join(
        sum(
            [
                generate_servicer(grpc_imports, file, service)
                for service in file.service
            ],
            [],
        )
    ...
    aio_servicers = "\n".join(
        sum(
            [
                generate_servicer(grpc_aio_imports, file, service, False)
                for service in file.service
            ],
            [],
        )
    ...
    add_servicer_methods = "\n".join(
        sum(
            [
                generate_add_servicer_to_server_method(grpc_imports, file, service)
                for service in file.service
            ],
            [],
        )
    ...
    aio_add_servicer_methods = "\n".join(
        sum(
            [
                generate_add_servicer_to_server_method(
                    grpc_aio_imports, file, service, False
                )
                for service in file.service
            ],
            [],
