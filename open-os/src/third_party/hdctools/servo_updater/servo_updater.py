...
def print_versions(outfile, boards, file, channel):
    ...
    for i, board in enumerate(boards):
        _unused, binfile, newvers = get_files_and_version(board, file, channel)
        if i:
            print(file=outfile)
...
def main(cmdline=sys.argv[1:]):
    ...
    parser.add_argument(
        ...
        "(human friendly, do not parse, format subject to change)",
    )
    parser.add_argument(
        "--json",
        dest="json_only",
        ...
        "(stable data structure when parsed as JSON, "
        "raw printed formatting is subject to change)",
    )
    parser.add_argument(
        "-s",
        "--serialname",
