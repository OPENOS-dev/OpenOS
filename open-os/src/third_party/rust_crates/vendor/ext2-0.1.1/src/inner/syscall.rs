...
use crate::inner::body::*;
use crate::inner::DirectoryEntryType;
use crate::inner::Ext2Filesystem;
use crate::inner::{DirectoryEntry, Inode};
use crate::IoResult;

use libc::{gid_t, uid_t, utimbuf};
use nix::errno::Errno;
...
impl<T: Read + Write + Seek> Ext2Filesystem<T> {
    ...
    pub fn utime(
        ...
        if let Some(times) = times {
            ...
            inode.last_modification_time = times.modtime as u32;
        } else {
    ...
    pub fn chown(&mut self, inode_nbr: u32, owner: uid_t, group: gid_t) -> IoResult<()> {
        ...
        if owner != uid_t::max_value() {
            inode.user_id = owner as u16;
        }

        if group != gid_t::max_value() {
            inode.group_id = group as u16;
        }
    ...
    /// bits of the file
    pub fn chmod(&mut self, inode_nbr: u32, mut mode: Mode) -> IoResult<()> {
    ...
    pub fn symlink(
        ...
        let inode_nbr = self.alloc_inode().ok_or(Errno::ENOSPC)?;
        let (_, inode_addr) = self.get_inode(inode_nbr)?;
        let mode = Mode::S_IRWXO | Mode::S_IRWXG | Mode::S_IRWXU;
        let access_mode = TypePerm((mode.bits() | SFlag::S_IFLNK.bits()) as u16);
