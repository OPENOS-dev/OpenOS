...
#define MAX_FLD_LEN              (26)   // 11pts(22)/rop_degree/rip_degree/fld_cv/eye_open_level
...
struct result
{
    ...
    int NumofSkinMap;
    double  TfmtxMap2TG[6]; // transformation matrix from skin map to TG
    unsigned char SkinMap[1500]; //30x50
};
...
struct MTKFDFTInitInfo
{
    ...
    MINT8    CamID;                            // the id of this camera, for multi-cam usage
    ...
    MBOOL    ParallelRGB565Conversion;
    MUINT32  LandmarkEnableCnt;
...
struct FdOptions
{
    ...
    MINT32 fd_scale_count;          //by frame set how many scales should GFD do detection
    MINT32 fd_scale_start_position; //by frame set which scale should GFD start detection
    MINT32 gfd_fast_mode;
    MBOOL ae_stable;
    MBOOL af_stable;
    MUINT8 LV;
    MINT32 curr_gtype;
    MINT32 inputPlaneCount; // originally used for SW input buffer, After FD531, it means # of multi-cam in current frame
    MBOOL doPhase2 ;        // for HAL to decide whether calling HW & phase2 or not.
    MBOOL doGender;
    MBOOL doPose;
    MBOOL doHWFLD;          // FDversion >= 531, HW support detect FLD
    MBOOL doROIFD;          // FDversion >= 52, HW support detect face in ROI
    MINT32 ROIFDinfo[8];    // FDversion >= 52,
    MUINT8 pyramid_num;     // adaptive pyramid number for UW camera: 2020/03/30, Savan
    MUINT16 pyramid_size;   // adaptive pyramid resolution for UW camera: 2020/03/30, Savan
    union{
    ...
    union{
    MUINT8 *ImageBufferUV20;
        MTEE_INT64 TEMP64_12;
    };
    ...
    MINT8  MainCamID;               // the id of maincam, -1 means turn off multi-cam usage
    MUINT8 MainCamFov;              // the fov of main cam, for multi-cam usage
    MUINT8 MainCamFOVRatio[2];      // 0~100, 100x of the main cam fov percentage [W, H] after cropping
    MUINT8 ThisCamFOVRatio[2];      // 0~100, 100x of the this cam fov percentage [W, H] after cropping
    MUINT16 MainCamFaceSize;        // the size of face in main cam, 0 means no face in main cam
    MINT32 TouchID;                 //-1: no touch, >=0: touched face id
    MINT32 magicNo;                 // magic number for frame sync. (from MW)
    MUINT16 TGWidthHeight[2];       // original TG size
    MUINT16 TGCropOffsetXY[2];      // TG fov-> FD fov crop offset (x, y)
    MUINT16 TGCropWidthHeight[2];   // size after cropping TG fov -> FD fov

    FdOptions()
    {
        TouchID = -1;                    // default value
        DynamicLandmarkCnt = 0;
        startW = 0;                        // initialize padding width
        startH = 0;                        // initialize padding height
        MainCamID = -1;
        MainCamFov = 80;
        MainCamFaceSize = 0;
        MainCamFOVRatio[0] = 100;
        MainCamFOVRatio[1] = 100;
        ThisCamFOVRatio[0] = 100;
        ThisCamFOVRatio[1] = 100;
        TGCropOffsetXY[0] = 0;
        TGCropOffsetXY[1] = 0;
        TGWidthHeight[0] = 4000;
        TGWidthHeight[1] = 3000;
        TGCropWidthHeight[0] = 4000;
        TGCropWidthHeight[1] = 3000;
    }
};
...
public:
    ...
    virtual void FDVTMainROIforHWFLD(unsigned short ROIRIP[][6], int* nFLD);
    ...
    virtual void FDVTMainJoinHWFLD(MINT16 nHWFLD, MINT16 FLDresult[][MAX_FLD_LEN]);
