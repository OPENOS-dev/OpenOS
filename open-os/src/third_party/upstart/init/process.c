...
process_from_name (const char *process)
{
	...
	} else if (! strcmp (process, "tmpfiles")) {
		return PROCESS_TMPFILES;
	} else {
		return -1;
	}
}
...
const char *
process_type_enum_to_str_int (int raw_type)
{
	return process_type_enum_to_str (raw_type);
}
...
process_type_enum_to_str (ProcessType type)
{
	...
	state_enum_to_str (PROCESS_TMPFILES, type);

	return NULL;
}

int
process_type_str_to_enum_int (const char *type)
{
	return process_type_str_to_enum (type);
}
...
process_type_str_to_enum (const char *type)
{
	...
	state_str_to_enum (PROCESS_TMPFILES, type);
