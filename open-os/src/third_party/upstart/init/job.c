...
static const char *
job_goal_enum_to_str (int raw_goal)
	__attribute__ ((warn_unused_result));

static int job_goal_str_to_enum (const char *goal)
	__attribute__ ((warn_unused_result));

static const char *
job_state_enum_to_str_int (int raw_state)
	__attribute__ ((warn_unused_result));
...
static const char *
job_trace_state_enum_to_str_int (int raw_state)
	__attribute__ ((warn_unused_result));
...
static int
job_trace_state_str_to_enum_int (const char *state)
	__attribute__ ((warn_unused_result));
		 ...
		 JobGoal  goal)
{
	 ...
	 *
	 * The exceptions are:
	 *  - the natural rest states of waiting and a running process; these
	 *    need induction to get them moving.
	 *  - applying 'kill timeout' for pre- and post-start with a JOB_STOP
	 *    goal; normally we let these run as long as they need, but in
	 *    the event of a hung process, we still want to complete the
	 *    teardown (e.g., shutdown?) process eventually.
	 */
	switch (goal) {
	case JOB_START:
	...
	case JOB_STOP:
		if (job->state == JOB_RUNNING)
			job_change_state (job, job_next_state (job));
		else if (job->state == JOB_PRE_START ||
			 job->state == JOB_POST_START) {
			ProcessType process;

			if (job->state == JOB_PRE_START) {
				process = PROCESS_PRE_START;
			} else {
				process = PROCESS_POST_START;
			}

			/* Put a SIGTERM+SIGKILL timeout on a (pre|post)-start
			 * process.
			 */
			if (job->pid[process]) {
		  ...
		  JobState  state)
{
	...
	while (job->state != state) {
		...
		case JOB_SECURITY:
			...
			break;
		case JOB_TMPFILES_SPAWNING:
			nih_assert (job->goal == JOB_START);
			nih_assert (old_state == JOB_SECURITY);

			/* Apply any registered tmpfiles.d configurations. */
			if (job->class->process[PROCESS_TMPFILES]) {
				job_process_start (job, PROCESS_TMPFILES);
			}
			state = job_next_state (job);
			break;
		case JOB_TMPFILES:
			nih_assert (job->goal == JOB_START);
			nih_assert (old_state == JOB_TMPFILES_SPAWNING);
		...
		case JOB_PRE_STARTING:
			nih_assert (job->goal == JOB_START);
			nih_assert (old_state == JOB_TMPFILES);
...
job_state_from_name (const char *state)
{
	...
	if (! strcmp (state, "waiting")) {
		...
		return JOB_SECURITY;
	} else if (! strcmp (state, "tmpfiles-spawning")) {
		return JOB_TMPFILES_SPAWNING;
	} else if (! strcmp (state, "tmpfiles")) {
		return JOB_TMPFILES;
...
job_serialise (const Job *job)
{
	...
	if (! state_set_json_enum_var (json,
				job_trace_state_enum_to_str_int,
...
static int
job_state_str_to_enum_int (const char *state)
{
	return job_state_str_to_enum (state);
}
...
job_deserialise (JobClass *parent, json_object *json)
{
	...
	if (! state_get_json_enum_var (json,
				job_trace_state_str_to_enum_int,
				"trace_state", job->trace_state))
		goto error;
...
job_state_enum_to_str_int (int raw_state)
{
	return job_state_enum_to_str (raw_state);
}
...
job_trace_state_enum_to_str_int (int raw_state)
{
	return job_trace_state_enum_to_str (raw_state);
}
...
job_trace_state_str_to_enum_int (const char *state)
{
	return job_trace_state_str_to_enum (state);
}
