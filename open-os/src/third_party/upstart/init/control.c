			      ...
			      int              file)
{
	...
	/*
	 * TODO(b/257962606)
	 * Partial revert of commit abdbf1defcf46a1ea4723a9b526dbd7828f4697d
	 * to allow non root device to emit message.
	 */
	if (! control_check_permission (message)) {
		nih_dbus_error_raise_printf (
			DBUS_INTERFACE_UPSTART ".Error.PermissionDenied",
...
/**
 * control_disable_respawn:
 *
 * @data: not used,
 * @message: D-Bus connection and message received.
 *
 * Implements the DisableRespawn method of the com.ubuntu.Upstart
 * interface.
 *
 * Disables job respawns.
 *
 * Returns: zero on success.
 **/
int
control_disable_respawn(void             *data,
			NihDBusMessage   *message)
{
	nih_info (_("Disabling job respawn"));
