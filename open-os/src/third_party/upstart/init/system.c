	      ...
	      const char *options)
{
	...
	if (mount (type, dir, type, flags, options) < 0)
		nih_return_system_error (-1);
 ...
 * @linkpath: path of symlink.
 *
 * Create specified symlink.
 **/
void
system_symlink (const char *target, const char *linkpath)
{
	nih_assert (target);
	nih_assert (linkpath);

	if (symlink (target, linkpath) < 0 && errno != EEXIST)
		nih_error ("%s: %s -> %s", _("Unable to create symlink"), target, linkpath);
}
...
/**
 * system_seccomp:
 * @operation: SECCOMP_SET_MODE_FILTER (we do not allow SECCOMP_SET_MODE_STRICT
 *             here)
 * @flags: MODE_FILTER flags,
 * @args: pointer to a filter program (struct sock_fprog).
 *
 * Configure the Secure Computing (seccomp) state of the calling process.
 *
 * Returns: zero on success, negative value on raised error.
 **/
int
system_seccomp (unsigned int operation,
		unsigned int flags,
		void *args)
{
	nih_assert (operation == SECCOMP_SET_MODE_FILTER);
