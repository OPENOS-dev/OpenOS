...
static void job_process_kill_timer_callback   (void *job, NihTimer *timer);
		   ...
		   ProcessType             process)
{
	...
	/* Apply 'kill timeout' to (pre|post)-stop processes -- first with
	 * TERM, and then re-armed with KILL if necessary.
	 */
	if (process == PROCESS_PRE_STOP || process == PROCESS_POST_STOP) {
		job_process_set_term_timer (job, process,
				job->class->kill_timeout);
	}
}
...
/**
 * job_process_set_term_timer:
 * @job: job to set SIGTERM timer for,
 * @process: process to be killed,
 * @timeout: timeout to apply for timer.
 *
 * Set SIGTERM kill timer for specified @job @process with timeout @timeout.
 **/
void
job_process_set_term_timer (Job          *job,
			    ProcessType   process,
			    time_t        timeout)
{
	nih_assert (job);
 ...
 *
 * This callback is called to terminate with either TERM or KILL signal. KILL
 * is used for cases where the job already failed to terminate within the
 * allowed timeout.
 **/
static void
			...
			NihTimer *timer)
{
	...
	/* Log unexpected termination signals with a bit higher log level:
	 *  - pre- or post-stop: These are likely because of missing or
	 *    incorrect timeout configurations.
	 *  - KILL signal: This means the job didn't handle the first
	 *    termination signal in time, and is likely a sign of a misbehaving
	 *    service.
	 */
	if (process == PROCESS_PRE_STOP || process == PROCESS_POST_STOP ||
			!job->kill_sigterm) {
		nih_warn (_("Sending %s signal to %s %s process (%d)"),
			  (job->kill_sigterm ? "TERM" : "KILL"),
			  job_name (job), process_name (process), job->pid[process]);
	} else {
		nih_info (_("Sending %s signal to %s %s process (%d)"),
			  (job->kill_sigterm ? "TERM" : "KILL"),
	...
	if (system_kill (job->pid[process],
			 job->kill_sigterm ? SIGTERM : SIGKILL) < 0) {
		NihError *err;
	...
	if (job->kill_sigterm) {
		/* Arm for KILL, in case TERM didn't work. */
		job_process_set_kill_timer (job, process,
					    job->class->kill_timeout);
	}
}

/* job_process_kill_timer, but matching NihTimerCb's type. */
static void
job_process_kill_timer_callback (void      *job,
			NihTimer *timer)
{
	job_process_kill_timer ((Job *)job, timer);
}
			...
			int          state_only)
{
	...
	case PROCESS_SECURITY:
		...
		break;
	case PROCESS_TMPFILES:
		nih_assert (job->state == JOB_TMPFILES);

		/* If the tmpfiles process is killed or exits with a status
			  ...
			  size_t           len)
{
	...
	/* Now call the handler registered via job_process_start() to deal with
	 * the error condition. Note that the job and associated process data
	 * might be destroyed after this.
	 */
