...
static void   start_reply_handler          (char **job_path,
				   NihDBusMessage *message,
				   const char *instance);
static void   start_reply_handler_callback (void *job_path,
				   NihDBusMessage *message,
				   const char *instance);
static void   reply_handler                (int *ret, NihDBusMessage *message);
static void   reply_handler_callback       (void *ret, NihDBusMessage *message);
...
list_sessions_action (NihCommand *command, char * const *args)
{
	...
	while ((ent = readdir (dir))) {
		...
		_contents = nih_file_read (NULL, path, &len);
		...
		contents = nih_strndup (NULL, _contents, len);
		if (! contents)
			continue;
...
#ifndef TEST
/**
 * disable_respawn_action:
 * @command: NihCommand invoked,
 * @args: command-line arguments.
 *
 * This function is called for the "disable-respawn" command.
 *
 * Returns: command exit status.
 */
static int
disable_respawn_action (NihCommand *command, char * const *args)
{
	nih_local NihDBusProxy *upstart = NULL;
	NihError *              err;
	...
	if (upstart_disable_respawn_sync (NULL, upstart) < 0)
		goto error;
...
static void
start_reply_handler_callback (void *         job_path,
		     NihDBusMessage *message,
		     const char *    instance)
{
	start_reply_handler ((char **)job_path, message, instance);
}
...
static void
reply_handler_callback (void *           ret,
	       NihDBusMessage *message)
{
	reply_handler ((int *)ret, message);
}
...
static NihCommand commands[] = {
	...
	{ "disable-respawn", NULL,
	  N_("Disable respawning of jobs."),
	  N_("Disable respawning of jobs."),
	  NULL, NULL, disable_respawn_action },
