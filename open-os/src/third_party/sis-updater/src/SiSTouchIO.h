...
#ifndef SRC_SISTOUCHIO_H_
#define SRC_SISTOUCHIO_H_
...
class SiSTouchIO {
 public:
  ...
  enum ERR_NO {
    ...
    ERROR_COPY_FROM_USER =
        -14,             // copy data from user memory to kernel memory fail
    ERROR_CLIENT = -13,  // client not created
    ERROR_COPY_FROM_KERNEL =
        -19,  // copy data from kernel memory to user memory fail
  ...
  // virtual int call_00( unsigned char *data, int size ) = 0;  HERE
  ...
  // 0; virtual int call_89( int *status ) = 0; virtual int call_90( int*
  // powermode ) = 0;

  // virtual int call_command( int cmd, int size, unsigned char *data, int
  // addcrc ) = 0; virtual int call_simple_command( int cmd, unsigned char
  // *data ) = 0;
  ...
  // 20120925
  // virtual int call_simple_command_with_wrong_crc( int cmd, unsigned char
  // *data ) = 0;
  ...
  static void setCustomDevice(bool isCustom, int devType, char devName[],
                              int devNameLen);
  ...
  static int getDeviceType(char *device_name, int max_retry, int retry_delay,
 ...
 protected:
  ...
  // ); virtual int make_90_buffer( unsigned char powermode ); virtual int
  // make_command_buffer( int cmd, int size, unsigned char *data, int addcrc
  // );

  virtual void dbg_print_buffer_hex(const unsigned char *data, int size);
  virtual void dbg_print_buffer_hex(int cmd, const unsigned char *data,
                                    int size);

  virtual int parse_syscall_return_value(int ret,
                                         int type = ONLY_RETURN_ZERO_FLAG);

  unsigned char m_buffer[BUFFER_SIZE];

  uint16_t m_crc;  // save crc current value
  virtual void AppendCRC(char data);
  // 20120925
  // virtual void AppendWrongCRC( char data );

  virtual inline unsigned int get_MAX_SIZE() { return ZEUS_I2C_MAX_SIZE; }
};
...
class SiSTouch_Zeus_USB : public SiSTouchIO {
 ...
 protected:
  ...
  virtual int sis_usb_io(unsigned char request, unsigned char dir,
...
class SiSTouch_Zeus_InterUSB : public SiSTouchIO {
 ...
 protected:
  ...
  //    virtual int make_88_buffer( int* addr, int *data, int* mask, int number
  //    ); virtual int make_command_buffer( int cmd, int size, unsigned char
  //    *data, int addcrc );
  virtual int sis_usb_start();
  virtual int sis_usb_stop();
  virtual int sis_usb_io(unsigned char request, unsigned char dir,
...
class SiSTouch_Aegis_I2C : public SiSTouchIO {
 public:
  SiSTouch_Aegis_I2C(int max_retry, int delay, int verbose, int ioDelay,
                     int changeDiagMode, int changePWRMode)
      : SiSTouchIO(max_retry, delay, verbose, 1, ioDelay, changeDiagMode,
                   changePWRMode) {}
 ...
 protected:
  ...
  virtual int make_83_buffer(int base, int pack_num);
  virtual int make_84_buffer(const int *data, int size);
  using SiSTouchIO::make_85_buffer;
  virtual int make_85_buffer(int data);
  virtual int make_86_buffer(int addr);
  //    virtual int make_88_buffer( int *addr, int *data, int *mask, int number
  //    ); virtual int make_90_buffer( int powermode ); virtual int
  //    make_command_buffer( int cmd, int size, unsigned char *data, int addcrc
  //    );
 ...
 private:
  ...
  virtual inline unsigned int get_MAX_SIZE() { return AEGIS_I2C_MAX_SIZE; }
  // 20120926
...
class SiSTouch_Aegis_I2C_FOR_NEW_DRIVER : public SiSTouch_Aegis_I2C {
 public:
  SiSTouch_Aegis_I2C_FOR_NEW_DRIVER(int max_retry, int delay, int verbose,
                                    int ioDelay, int changeDiagMode,
                                    int changePWRMode)
      : SiSTouch_Aegis_I2C(max_retry, delay, verbose, ioDelay, changeDiagMode,
                           changePWRMode) {}
...
class SiSTouch_Aegis_USB : public SiSTouchIO {
 public:
  ...
  SiSTouch_Aegis_USB(int max_retry, int delay, int verbose, int ioDelay,
                     int changeDiagMode, int changePWRMode)
      : SiSTouchIO(max_retry, delay, verbose, 1, ioDelay, changeDiagMode,
                   changePWRMode) {}
...
class SiSTouch_Aegis_Multi : public SiSTouchIO {
 public:
  ...
  SiSTouch_Aegis_Multi(int max_retry, int delay, int verbose, int ioDelay,
                       int changeDiagMode, int changePWRMode)
      : SiSTouchIO(max_retry, delay, verbose, 1, ioDelay, changeDiagMode,
                   changePWRMode),
        m_SlaveAddr(0) {}
  ...
  SiSTouch_Aegis_Multi(int max_retry, int delay, int verbose, int ioDelay,
                       int changeDiagMode, int changePWRMode, int slaveAddr)
      : SiSTouchIO(max_retry, delay, verbose, 1, ioDelay, changeDiagMode,
                   changePWRMode),
  ...
  // virtual int call_00( unsigned char *data, int size ); HERE
  virtual int call_04();
  // virtual int call_81();
  virtual int call_82();
  // virtual int call_82_without_retry();
  ...
  // virtual int call_87();
  // virtual int call_88( int *address, int *data, int *mask, int number );
  // virtual int call_88_master( int *address, int *data, int *mask, int
  // number ); virtual int call_89( int *status ); virtual int call_90( int
  // *powermode ); virtual int call_command( int cmd, int size, unsigned char
  // *data, int addcrc ); virtual int call_simple_command( int cmd, unsigned
  // char *data );
  virtual int is_noneed_retry_error(int ret);
  virtual void log_out_error_message(int ret, int cmd);
 ...
 protected:
  ...
  // virtual int make_88_buffer( int *addr, int *data, int *mask, int number
  // ); virtual int make_88_buffer_master( int *addr, int *data, int *mask,
  // int number ); virtual int make_88_buffer_slave( int *addr, int *data,
  // int *mask, int number ); virtual int make_90_buffer( int powermode );
  // virtual int make_90_buffer_master( int powermode );
  // virtual int make_90_buffer_slave( int powermode );
  // virtual int make_command_buffer( int cmd, int size, unsigned char *data,
  // int addcrc ); virtual int make_command_buffer_master( int cmd, int size,
  // unsigned char *data, int addcrc ); virtual int
  // make_command_buffer_slave( int cmd, int size, unsigned char *data, int
  // addcrc );
 ...
 private:
  ...
  int simple_io(int cmd, int sleepms = 0, int wTimeout = DEFAULT_TIMEOUT,
                int rTimeout = DEFAULT_TIMEOUT);

  int simple_io_slave(int cmd, int sleepms = 0, int wTimeout = DEFAULT_TIMEOUT,
                      int rTimeout = DEFAULT_TIMEOUT);
  int io_command(unsigned char *buf, int size, int sleepms = 0,
...
class SiSTouch_Aegis_Multi_FOR_NEW_DRIVER : public SiSTouch_Aegis_Multi {
 public:
  SiSTouch_Aegis_Multi_FOR_NEW_DRIVER(int max_retry, int delay, int verbose,
                                      int ioDelay, int changeDiagMode,
                                      int changePWRMode)
      : SiSTouch_Aegis_Multi(max_retry, delay, verbose, ioDelay, changeDiagMode,
                             changePWRMode) {}
...
class SiSTouch_Aegis_InterUSB : public SiSTouch_Aegis_Multi {
 public:
  SiSTouch_Aegis_InterUSB(int max_retry, int delay, int verbose, int ioDelay,
                          int changeDiagMode, int changePWRMode)
      : SiSTouch_Aegis_Multi(max_retry, delay, verbose, ioDelay, changeDiagMode,
                             changePWRMode, MASTER_ADDR) {}
...
class SiSTouch_Aegis_Slave : public SiSTouch_Aegis_Multi {
 public:
  SiSTouch_Aegis_Slave(int max_retry, int delay, int verbose, int ioDelay,
                       int changeDiagMode, int changePWRMode)
      : SiSTouch_Aegis_Multi(max_retry, delay, verbose, ioDelay, changeDiagMode,
                             changePWRMode, SLAVE0_ADDR) {}

  static int device_Exist();
  static const char *get_device_name();
};

#endif  // SRC_SISTOUCHIO_H_
