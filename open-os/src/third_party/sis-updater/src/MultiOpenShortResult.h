...
#ifndef SRC_MULTIOPENSHORTRESULT_H_
#define SRC_MULTIOPENSHORTRESULT_H_
...
class MultiOpenShortResult {
 ...
 public:
  ...
  explicit MultiOpenShortResult(int chipNum);
  ...
  // MutualShortExtraCheckByTP, begin
  void init(MultiOpenShortData*
                deviceData);  // do after init each single-chip (mResults)
  ...
  static void fprintMultiData(FILE* fp2D, MultiOpenShortData* data,
                              MultiOpenShortResult* result,
                              OpenShortConfig* config, int tableID);

  static bool saveMultiOpenShortResult(FILE* file,
                                       MultiOpenShortData* multiData,
                                       MultiOpenShortResult* multiResult,
                                       OpenShortConfig* config);

 private:
  // copy constructor
  MultiOpenShortResult(const MultiOpenShortResult& rhs)
      : mChipNum(0), mResults(0), mChipOrderIndexs(0) {}
  // assignment operator
  MultiOpenShortResult& operator=(const MultiOpenShortResult& rhs) {
    return *this;
  }
};

#endif  // SRC_MULTIOPENSHORTRESULT_H_
