 ...
 * type of character. Some of the common ones use spaces, tabs, commas and
 * semi-colons.
 *
...
#ifndef SRC_CSV_PARSER_HPP_

#define SRC_CSV_PARSER_HPP_
 ...
 *
 * @li ENCLOSURE_NONE (1) means the CSV file does not use any
 * enclosure characters for the fields
 * @li ENCLOSURE_REQUIRED (2) means the CSV file requires enclosure
 * characters for all the fields
 * @li ENCLOSURE_OPTIONAL (3) means the use of enclosure characters for
 * the fields is optional
 *
 * The ENCLOSURE_TYPE_BEGIN and ENCLOSURE_TYPE_END members of this enum
 * definition are never to be used.
 */
typedef enum {
 ...
 *
 * @li The parser can handle documents where fields are always enclosed, not
 * enclosed at all or optionally enclosed.
 * @li When fields are strictly all enclosed, there is an assumption that any
 * enclosure characters within the field are escaped by placing a backslash in
 * front of the enclosure character.
 *
 ...
 *
 * For option (c) when the enclosure character is optional, if an enclosure
 * character is spotted at either the beginning or the end of the string, it is
 * assumed that the field is enclosed.
 *
 * The csv_parser::init() method can accept a character array as the path to the
 * CSV file. Since it is overloaded, it can also accept a FILE pointer to a
 * stream that is already open for reading.
 *
 * The set_enclosed_char() method accepts the field enclosure character as the
 * first parameter and the enclosure mode as the second parameter which controls
 * how the text file is going to be parsed.
 *
 * @see csv_parser::set_enclosed_char()
 * @see enclosure_type_t
 *
 * @todo Add ability to parse files where fields/columns are terminated by
 * strings instead of just one char.
 * @todo Add ability to set strings where lines start by. Currently lines do not
 * have any starting char or string.
 * @todo Add ability to set strings where line end by. Currently lines can only
 * end with a single char.
 * @todo Add ability to accept other escape characters besides the backslash
 * character 0x5C.
...
class csv_parser {
 public:
   ...
   * @li The record delimiter character is initialized to a new line character
   * 0x0A.
   *
   * @li The lengths of all the above-mentioned fields are initialized to 0,1,1
   * and 1 respectively.
   ...
   * Class destructor
   *
   * In the class destructor the file pointer to the input CSV file is closed
   * and the buffer to the input file name is also deallocated.
   *
   ...
   *
   * This init method accepts a pointer to the CSV file that has been opened for
   * reading
   *
   ...
   *
   * If the enclosure is optional, there could be fields that are enclosed, and
   * fields that are not enclosed within the same line/record.
   *
   ...
   *
   * Call this method to see if there are more rows to retrieve before invoking
   * csv_parser::get_row()
   *
   * @see csv_parser::get_row()
   * @see csv_parser::more_rows
   *
   * @return bool Returns true if there are still more rows and false if there
   * is not.
   */
  bool has_more_rows(void) { return more_rows; }

  /**
   * Defines the number of records to discard
   *
   * The number of records specified will be discarded during the parsing
   * process.
   *
  ...
  /**
   * Returns the number of times the csv_parser::get_row() method has been
   * invoked
   *
   * @see csv_parser::reset_record_count()
   * @return unsigned int The number of times the csv_parser::get_row() method
   * has been invoked.
   */
  unsigned int get_record_count(void) { return record_count; }
   ...
   * @return void
   */
  void reset_record_count(void) { record_count = 0U; }
 ...
 private:
   ...
   * @param[out] buffer_len A pointer to an integer storing the length of the
   * buffer.
   * @return void
   */
   ...
   * @param[in] line The character array buffer containing the current
   * record/line
   * @param[in] line_length The length of the buffer
   */
  void _get_fields_without_enclosure(csv_row_ptr row, const char* line,
   ...
   * @param[in] line_length The length of the buffer
   */
  void _get_fields_with_enclosure(csv_row_ptr row, const char* line,
                                  const unsigned int* line_length);
   ...
   * @param[in] line_length The length of the buffer
   */
  void _get_fields_with_optional_enclosure(csv_row_ptr row, const char* line,
                                           const unsigned int* line_length);
 ...
 protected:
   ...
   *
   * If present or used for a field it is assumed that both ends of the fields
   * are wrapped.
   *
   ...
   *
   * For now the only valid escape character allowed is the backslash character
   * 0x5C
   *
   * This is only important when the enclosure character is required or
   * optional.
   *
   * This is the backslash character used to escape enclosure characters found
   * within the fields.
   *
   * @see csv_parser::_get_fields_with_enclosure()
   * @see csv_parser::_get_fields_with_optional_enclosure()
   * @todo Update the code to accept other escape characters besides the
   * backslash
   *
   * @var escaped_char
   */
   ...
   *
   * This is the single character used to mark the end of a column in the text
   * file.
   *
   ...
   *
   * This is the single character used to mark the end of a record in the text
   * file.
   *
   * The most popular one is the new line character however it is possible to
   * use others as well.
   *
   ...
   *
   * For now this is not being used. It will be used in future versions of the
   * object.
   *
   ...
   *
   * This variable controls how many records in the file are skipped before
   * parsing begins.
   *
   * @see csv_parser::_skip_lines()
   * @see csv_parser::set_skip_lines()
   ...
   *
   * This boolean property is an internal indicator of whether there are still
   * records in the file to be parsed.
   *
   * @see csv_parser::has_more_rows()
   * @var more_rows
   */
 ...
 private:
  ...
  csv_parser(const csv_parser& rhs)
      : enclosed_char(0x00),
        ...
        more_rows(false) {}
  // assignment operator
  csv_parser& operator=(const csv_parser& rhs) { return *this; }
}; /* class csv_parser */

#endif  // SRC_CSV_PARSER_HPP_
