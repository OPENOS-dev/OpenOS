...
#ifndef SRC_SISTOUCHADAPTER_H_
#define SRC_SISTOUCHADAPTER_H_
...
class SiSTouchAdapter {
 public:
  ...
  int doUpdate(FILE* file, bool update_bootloader, bool update_bootloader_auto,
               bool reserve_RODATA, bool update_parameter, bool force_update,
  ...
  bool checkContinueNG(OpenShortConfig* config, OpenShortResult* result, int x,
                       int y);
  void checkSecondThreshold(OpenShortConfig* config, OpenShortResult* result);
  int doOpenShort(OpenShortData* deviceData, OpenShortConfig* config,
                  OpenShortResult* result, bool bCheckSecondThreshold = false);
  int doOpenShort(OpenShortData* deviceData, OpenShortData* sampleData,
                  OpenShortConfig* config, OpenShortResult* result,
                  bool bCheckSecondThreshold = false);
  int doCheckFWState();
  ...
  static SiSTouchAdapter* prepareAdapter(Parameter param);
 ...
 protected:
  SiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
  ...
  SiSTouchAdapter(SiSTouchIO* io, int max_retry, int delay, int verbose,
  ...
  virtual void prepareBuffer(int* buf, int buf_length,
                             int *device_firmware_id) = 0;
  ...
  //    *deviceData, OpenShortResult *result) = 0; virtual int
  //    calculateCycleRxDiff(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result) = 0; virtual int
  ...
  //    OpenShortData *sampleData, OpenShortResult *result) = 0; virtual int
  //    testLineOpenShort(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result) = 0; virtual int
  //    testLineOpenShort(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortData *sampleData, OpenShortResult *result) = 0;
  ...
  bool read_from_buffer(int* buf, int buf_length, unsigned int offset, int* id,
                        int size);
  bool read_from_address(unsigned int addr, int* buffer, int size);
  int read_file_to_buffer(FILE* file, int32_t offset, int size, int* buf);
  bool write_buffer_to_address(int* buf, int offset, int buf_length, int length,
                               unsigned int addr);
  int32_t getFileLength(FILE* file);
  bool callReset();
