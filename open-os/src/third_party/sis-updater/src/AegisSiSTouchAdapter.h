...
#ifndef SRC_AEGISSISTOUCHADAPTER_H_
#define SRC_AEGISSISTOUCHADAPTER_H_
...
class AegisSiSTouchAdapter : public SiSTouchAdapter {
 protected:
  AegisSiSTouchAdapter(SiSTouchIO* io, int max_retry, int delay, int verbose,
                       int ioDelay, int changeDiagMode, int changePWRMode)
      : SiSTouchAdapter(io, max_retry, delay, verbose, ioDelay, changeDiagMode,
                        changePWRMode) {}
 ...
 protected:
  ...
  virtual inline unsigned int getFirmwareIdBase() { return FIRMWARE_ID_BASE; }
  ...
  virtual inline unsigned int getCycleAddress() { return CYCLE_DATA_ADDRESS; }

  virtual inline unsigned int getLoopAddress() { return LOOP_DATA_ADDRESS; }

  virtual unsigned int getBootFlagBase() { return BOOT_FLAG_BASE; }

  virtual unsigned int getBootFlagAddr() { return BOOT_FLAG_ADDRESS; }

  virtual inline unsigned int getBootFlagValue() { return BOOT_FLAG_VALUE; }
  ...
  virtual inline int get_READ_LENGTH() { return READ_LENGTH; }

  virtual inline int get_BUFFER_SIZE() { return BUFFER_SIZE; }

  virtual inline int getMainCodeCrcBase() { return MAINCDOE_CRC_BASE; }

  virtual inline int getBootloaderCrcBase() { return BOOTLOADER_CRC_BASE; }

  virtual unsigned int getLayoutOffsetAddress() {
    return LAYOUT_OFFSET_ADDRESS;
  }

  virtual unsigned int getFwModeAddr() { return FW_MODE_ADDRESS; }
  ...
  //   virtual bool loadBaseVoltage(float *dataBuf, int buf_width, int
  //   buf_height); virtual bool loadRawVoltage(float *dataBuf, int buf_width,
  //   int buf_height); virtual bool loadCycle(float *dataBuf, int buf_width,
  //   int buf_height); virtual bool loadLoop(float *dataBuf, int buf_width, int
  //   buf_height); virtual bool loadDiff(float *dataBuf, int buf_width, int
  //   buf_height);

  // MutualShortExtraCheck, begin
  //    virtual int calculateCycleTxDiff(OpenShortConfig* config, OpenShortData
  //    *deviceData, OpenShortResult *result); virtual int
  //    calculateCycleRxDiff(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result); virtual int
  //    JudgeTxShortResult(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result); virtual int
  //    JudgeRxShortResult(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result); virtual int
  //    checkMutualShortExtra(OpenShortConfig* config, OpenShortData*
  //    deviceData, OpenShortResult *result);
  // MutualShortExtraCheck, end

  // MutualNearFarCheck, begin
  //    virtual int checkMutualNearFar(OpenShortConfig* config, OpenShortData*
  //    deviceData, OpenShortResult *result); virtual void
  //    checkMutualNearFarTx(OpenShortConfig* config, OpenShortData* deviceData,
  //    OpenShortResult *result); virtual void
  //    checkMutualNearFarRx(OpenShortConfig* config, OpenShortData* deviceData,
  //    OpenShortResult *result);
  // MutualNearFarCheck, end

  //    virtual int testPointOpenShort(OpenShortConfig* config, OpenShortData
  //    *deviceData, OpenShortResult *result); virtual int
  //    testPointOpenShort(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortData *sampleData, OpenShortResult *result); virtual int
  //    testLineOpenShort(OpenShortConfig* config, OpenShortData *deviceData,
  //    OpenShortResult *result); virtual int testLineOpenShort(OpenShortConfig*
  //    config, OpenShortData *deviceData, OpenShortData *sampleData,
  //    OpenShortResult *result);

  virtual int updateFlow(int* buf, int buf_length, int file_length,
                         bool update_bootloader, bool isNewBootloader,
                         bool reserve_RODATA, bool update_parameter,
                         bool restart = true);
  // virtual int updateCalibrationFlow(OpenShortData* data);
  // virtual int eraseCalibrationFlag(bool checkBootLoader = false);
  virtual void prepareBuffer(int* buf, int buf_length, int *device_firmware_id);
  virtual int isSiSFWFile(int* buf, int buf_length);

  virtual int parameterCheck(bool* update_bootloader,
                             const bool update_bootloader_auto,
                             bool* isNewBootloader, bool* reserve_RODATA,
                             bool* update_parameter, bool force_update,
                             const bool isRecoveryDevice, int* buf,
                             int file_Length);
  ...
  virtual inline int get_actual_pending_byte_length(int pending_length) {
    ...
    return (pending_length < max_pending_length) ? pending_length
                                                 : max_pending_length;
  }
...
class AegisI2CSiSTouchAdapter : public AegisSiSTouchAdapter {
 public:
  AegisI2CSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                          int changeDiagMode, int changePWRMode)
      : AegisSiSTouchAdapter(
            new SiSTouch_Aegis_I2C(max_retry, delay, verbose, ioDelay,
                                   changeDiagMode, changePWRMode),
            max_retry, delay, verbose, ioDelay, changeDiagMode, changePWRMode) {
  }

  AegisI2CSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                          int changeDiagMode, int changePWRMode,
                          int detectHidrawFlag)
      : AegisSiSTouchAdapter(new SiSTouch_Aegis_I2C_FOR_NEW_DRIVER(
                                 max_retry, delay, verbose, ioDelay,
                                 changeDiagMode, changePWRMode),
                             max_retry, delay, verbose, ioDelay, changeDiagMode,
                             changePWRMode) {}
...
class AegisUSBSiSTouchAdapter : public AegisSiSTouchAdapter {
 public:
  AegisUSBSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                          int changeDiagMode, int changePWRMode)
      : AegisSiSTouchAdapter(
            new SiSTouch_Aegis_USB(max_retry, delay, verbose, ioDelay,
                                   changeDiagMode, changePWRMode),
            max_retry, delay, verbose, ioDelay, changeDiagMode, changePWRMode) {
  }
...
class AegisMultiSiSTouchAdapter : public AegisSiSTouchAdapter {
 protected:
  AegisMultiSiSTouchAdapter(SiSTouchIO* io, int max_retry, int delay,
                            int verbose, int ioDelay, int changeDiagMode,
                            int changePWRMode)
      : AegisSiSTouchAdapter(io, max_retry, delay, verbose, ioDelay,
                             changeDiagMode, changePWRMode) {}
 ...
 public:
  ...
  AegisMultiSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                            int changeDiagMode, int changePWRMode)
      : AegisSiSTouchAdapter(
            new SiSTouch_Aegis_Multi(max_retry, delay, verbose, ioDelay,
                                     changeDiagMode, changePWRMode),
            max_retry, delay, verbose, ioDelay, changeDiagMode, changePWRMode) {
  }
  ...
  AegisMultiSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                            int changeDiagMode, int changePWRMode,
                            int detectHidrawFlag)
      : AegisSiSTouchAdapter(new SiSTouch_Aegis_Multi_FOR_NEW_DRIVER(
                                 max_retry, delay, verbose, ioDelay,
                                 changeDiagMode, changePWRMode),
                             max_retry, delay, verbose, ioDelay, changeDiagMode,
                             changePWRMode) {}
  ...
  int doUpdate(FILE** multi_files, int multi_Num,
               const UpdateFWParameter& param, const bool isRecoveryDevice);
  int doEraseCalibrationFlag(int multi_Num);

  int getMultiDeviceOpenShortData(MultiOpenShortData* data);

  bool checkContinueNG(OpenShortConfig* config, MultiOpenShortResult* result,
                       int x, int y);
  void checkSecondThreshold(OpenShortConfig* config,
                            MultiOpenShortResult* result);
  int doMultiOpenShort(MultiOpenShortData* deviceData, OpenShortConfig* config,
                       MultiOpenShortResult* result);
  int doMultiOpenShort(MultiOpenShortData* deviceData,
                       MultiOpenShortData* sampleData, OpenShortConfig* config,
                       MultiOpenShortResult* result);

  // MutualShortExtraCheckByTP, begin
  int testLineOpenShortByTP(OpenShortConfig* config,
                            MultiOpenShortData* deviceData,
                            MultiOpenShortResult* result);
  float getAbsDiffPercentage(float numerator, float denominator);
  int JudgeTxShortResultByTP(OpenShortConfig* config,
                             MultiOpenShortData* deviceData,
                             MultiOpenShortResult* result);
  int JudgeRxShortResultByTP(OpenShortConfig* config,
                             MultiOpenShortData* deviceData,
                             MultiOpenShortResult* result);
  int checkMutualShortExtraByTP(OpenShortConfig* config,
                                MultiOpenShortData* deviceData,
                                MultiOpenShortResult* result);
  // MutualShortExtraCheckByTP, end

  // MutualNearFarCheckByTP, begin
  int checkMutualNearFarByTP(OpenShortConfig* config,
                             MultiOpenShortData* deviceData,
                             MultiOpenShortResult* result);
  void checkMutualNearFarTxByTP(OpenShortConfig* config,
                                MultiOpenShortData* deviceData,
                                MultiOpenShortResult* result);
  void checkMutualNearFarRxByTP(OpenShortConfig* config,
                                MultiOpenShortData* deviceData,
                                MultiOpenShortResult* result);
  ...
  static bool ioStaticDetectIsMulti(int max_retry, int retryDelay, int verbose,
                                    int ioDelay, int changeDiagMode,
                                    int changePWRMode);
};

class AegisInterUSBSiSTouchAdapter : public AegisMultiSiSTouchAdapter {
 public:
  AegisInterUSBSiSTouchAdapter(int max_retry, int delay, int verbose,
                               int ioDelay, int changeDiagMode,
                               int changePWRMode)
      : AegisMultiSiSTouchAdapter(
            new SiSTouch_Aegis_InterUSB(max_retry, delay, verbose, ioDelay,
                                        changeDiagMode, changePWRMode),
            max_retry, delay, verbose, ioDelay, changeDiagMode, changePWRMode) {
  }
...
class AegisSlaveSiSTouchAdapter : public AegisMultiSiSTouchAdapter {
 public:
  AegisSlaveSiSTouchAdapter(int max_retry, int delay, int verbose, int ioDelay,
                            int changeDiagMode, int changePWRMode)
      : AegisMultiSiSTouchAdapter(
            new SiSTouch_Aegis_Slave(max_retry, delay, verbose, ioDelay,
                                     changeDiagMode, changePWRMode),
            max_retry, delay, verbose, ioDelay, changeDiagMode, changePWRMode) {
  }
...
#endif  // SRC_AEGISSISTOUCHADAPTER_H_
