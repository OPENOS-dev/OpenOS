...
class RTC_EXPORT EnvironmentFactory final {
 public:
  ...
  void Set( std::unique_ptr<const FieldTrialsView> utility);
  void Set( std::unique_ptr<Clock> utility);
  void Set( std::unique_ptr<TaskQueueFactory> utility);
  void Set( std::unique_ptr<RtcEventLog> utility);

  void Set(const FieldTrialsView*  utility);
  void Set(Clock*  utility);
  void Set(TaskQueueFactory*  utility);
  void Set(RtcEventLog*  utility);
...
inline void EnvironmentFactory::Set(
    const FieldTrialsView*  utility) {
  if (utility != nullptr) {
    field_trials_ = utility;
  }
}

inline void EnvironmentFactory::Set(Clock*  utility) {
  if (utility != nullptr) {
    clock_ = utility;
  }
}

inline void EnvironmentFactory::Set(TaskQueueFactory*  utility) {
  if (utility != nullptr) {
    task_queue_factory_ = utility;
  }
}

inline void EnvironmentFactory::Set(RtcEventLog*  utility) {
  if (utility != nullptr) {
    event_log_ = utility;
  }
}
