...
void EnvironmentFactory::Set(
     std::unique_ptr<const FieldTrialsView> utility) {
  if (utility != nullptr) {
...
void EnvironmentFactory::Set( std::unique_ptr<Clock> utility) {
  if (utility != nullptr) {
    clock_ = utility.get();
    Store(std::move(utility), leaf_);
  }
...
void EnvironmentFactory::Set(
     std::unique_ptr<TaskQueueFactory> utility) {
  if (utility != nullptr) {
...
void EnvironmentFactory::Set(
     std::unique_ptr<RtcEventLog> utility) {
  if (utility != nullptr) {
