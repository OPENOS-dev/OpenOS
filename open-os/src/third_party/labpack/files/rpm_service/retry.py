...
def install_sigalarm_handler(new_handler):
    ...
    if (new_handler is None
                or (not callable(new_handler) and new_handler != signal.SIG_IGN
                    and new_handler != signal.SIG_DFL)):
...
def timeout(func, args=(), kwargs={}, timeout_sec=60.0, default_result=None):
    ...
    old_timeout_time = (time.time() +
                        old_alarm_sec) if old_alarm_sec > 0 else 0
