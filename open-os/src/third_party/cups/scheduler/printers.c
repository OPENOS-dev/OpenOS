    ...
    ppd_file_t *ppd)  /* I  - Printer PPD */
{
  ...
  /*** Sharp ***/
  else if (((option = ppdFindOption(ppd, "JCLRetentionSetting")) != NULL) &&
      (choice = ppdFindChoice(option, "Before")) &&
      ((coption = ppdFindCustomOption(ppd, "JCLRetentionPassword")) != NULL) &&
      ((cparam = ppdFirstCustomParam(coption)) != NULL))
  {
    mapping = ippNew();

    /*
     * JCLRetentionSetting = Before
     */
    ippAddString(mapping, IPP_TAG_ZERO, IPP_TAG_TEXT, option->keyword, NULL,
                 choice->choice);

    /*
     * JCLRetentionPassword = Custom.1234
     */
    ippAddString(mapping, IPP_TAG_ZERO, IPP_TAG_TEXT, coption->keyword, NULL,
                 "Custom.%s");
  }
  ...
  if (resolution) {
    ...
    */
	...
	continue;
      ...
      if (!mapping)
        mapping = ippNew();
      res_x[res_cnt] = res.resolution.xres;
      res_y[res_cnt] = res.resolution.yres;
      res_cnt++;
    ...
    */
    if (mapping) {
      ippAddResolutions(ppd_attrs, IPP_TAG_PRINTER,
                        "pwg-raster-document-resolution-supported",
                        res_cnt, IPP_RES_PER_INCH, res_x, res_y);
      ippAddResolutions(ppd_attrs, IPP_TAG_PRINTER,
