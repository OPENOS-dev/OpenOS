...
std::optional<HttpHeaders> ParseHttpHeaders(const std::string& headers) {
...
// static
std::optional<HttpRequest> HttpRequest::Deserialize(SmartBuffer* message) {
  ...
  std::optional<HttpHeaders> headers = ParseHttpHeaders(request_headers);
  if (!headers) {
