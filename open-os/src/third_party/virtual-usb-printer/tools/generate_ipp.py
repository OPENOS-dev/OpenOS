...
"""This script is used to convert an encoded IPP message into a human-readable
JSON form.
...
def add_attribute(attributes, group, value_type, value_name, value):
    attributes[group].append(
        {"type": value_type, "name": value_name, "value": value}
    )
...
def main():
    ...
    while i < len(ascii_values):
        ...
        if is_ipp_delimiter(ascii_val):
            ...
            i += 1
            # Following the value tag, extract the name length and the name
            # value.
            ...
            (value, i) = extract_value(
                ascii_values, cur_value_type, i, value_length
            )

            if cur_value_type == prev_value_type and name_length == 0:
                # This means that the current attribute has more than one value.
                # So we must store each value in an array.
                extend_attribute_values(
                    ipp_attributes, current_attributes_group, value
                )
            else:
                add_attribute(
                    ipp_attributes,
                    current_attributes_group,
                    cur_value_type,
                    name,
                    value,
                )
