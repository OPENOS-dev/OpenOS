...
#ifndef VIRTUAL_USB_DEVICE_USB_DEVICE_SCANNER_ESCL_MANAGER_H_
#define VIRTUAL_USB_DEVICE_USB_DEVICE_SCANNER_ESCL_MANAGER_H_
...
struct JobInfo {
  ...
  JobState state;
  // The ScanSettings for the job.
  ScanSettings settings;
  // The number of remaining pages for the job.
  uint8_t num_remaining_pages;
  // The delay in providing the job's image, if applicable.
...
class EsclManager {
 ...
 private:
  ...
  bool LogSettings(const ScanSettings& settings);
  base::DictValue SettingsToDict(const ScanSettings& settings);

  ScannerCapabilities scanner_capabilities_;
  ScannerStatus status_;
  base::FilePath output_log_dir_;
  int log_counter_;
};

#endif  // VIRTUAL_USB_DEVICE_USB_DEVICE_SCANNER_ESCL_MANAGER_H_
