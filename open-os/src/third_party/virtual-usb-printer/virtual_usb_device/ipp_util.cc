...
#include "virtual-usb-printer/virtual_usb_device/cups_constants.h"
...
std::vector<IppAttribute> GetAttributes(const base::ListValue& attributes) {
  ...
  for (std::size_t i = 0; i < attributes.size(); ++i) {
    const base::Value& attributes_dictionary = attributes[i];
    if (!attributes_dictionary.is_dict()) {
      LOG(ERROR)
          << "Failed to extract dictionary value from attributes list at index "
          << i;
    }
    ipp_attributes.push_back(GetAttribute(attributes_dictionary.GetDict()));
  }
...
void AddName(const std::string& name, bool include_name, SmartBuffer* buf) {
...
void AddBooleanAttribute(bool value,
                         IppTag tag,
...
void AddIntAttribute(int value,
                     IppTag tag,
                     const std::string& name,
                     bool include_name,
                     SmartBuffer* buf) {
...
std::optional<uint16_t> ReadShort(const std::vector<uint8_t>& bytes,
                                  size_t start) {
  uint16_t value;
...
std::optional<size_t> GetAttributeLength(const std::vector<uint8_t>& bytes,
                                         size_t start) {
  size_t i = start;
  std::optional<uint16_t> name_length = ReadShort(bytes, i);
  if (!name_length) {
  ...
  i += 2;
  i += name_length.value();
  std::optional<uint16_t> value_length = ReadShort(bytes, i);
  if (!value_length) {
...
std::optional<size_t> GetGroupLength(const std::vector<uint8_t>& bytes,
                                     size_t start) {
  if (start >= bytes.size())
    return std::nullopt;
  ...
  while (i < bytes.size()) {
    ...
    // Skip the attribute.
    std::optional<size_t> length = GetAttributeLength(bytes, i);
    if (!length) {
...
std::optional<size_t> GetAttributesLength(const std::vector<uint8_t>& bytes) {
  size_t i = 0;
  while (i < bytes.size()) {
    uint8_t tag = bytes[i++];
    if (tag == static_cast<uint8_t>(IppTag::END)) {
      ...
      std::optional<size_t> length = GetGroupLength(bytes, i);
      if (!length) {
...
// static
std::optional<IppHeader> IppHeader::Deserialize(SmartBuffer* message) {
...
bool RemoveIppAttributes(SmartBuffer* buf) {
  std::optional<size_t> length = GetAttributesLength(buf->contents());
  if (!length) {
...
IppAttribute GetAttribute(const base::DictValue& attribute) {
  const std::string* type = attribute.FindString(kTypeKey);
  const std::string* name = attribute.FindString(kNameKey);
  const base::Value* value = attribute.Find(kValueKey);
...
std::vector<IppAttribute> GetAttributes(const base::DictValue& attributes,
                                        const std::string& key) {
  const base::ListValue* attributes_list = attributes.FindList(key);
...
void AddPrinterAttributes(const std::vector<IppAttribute>& ipp_attributes,
                          ...
                          SmartBuffer* buf) {
  std::map<std::string, std::function<void(const IppAttribute&, SmartBuffer*)>>
