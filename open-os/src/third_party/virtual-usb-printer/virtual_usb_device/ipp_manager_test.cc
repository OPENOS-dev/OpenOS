...
class IppManagerTest : public testing::Test {
 protected:
  ...
  // Used to setup IppAttribute vectors to be returned from commands.
  ...
  // Pre-serialized versions of the various attribute member variables.
  // These will be used to build up expected response SmartBuffers.
...
TEST_F(IppManagerTest, InvalidOperation) {
  ...
  SmartBuffer response = ipp_manager_.HandleIppRequest(header, SmartBuffer());
...
TEST_F(IppManagerTest, HandleValidateJob) {
  ...
  SmartBuffer response = ipp_manager_.HandleIppRequest(header, SmartBuffer());
  std::optional<IppHeader> response_header = IppHeader::Deserialize(&response);
