...
/** Current transfer request in an endpoint. */
typedef uint32_t (*CUSBD_reqQueueSize)(void* pD, struct CUSBD_Ep* ep);
/**
...
/** Called when device receiver out transfer. */
typedef void (*CUSBD_CbEPxOut)(void* pD, uint16_t EpNum);

/** Called when device receiver in transfer. */
typedef void (*CUSBD_CbEPxIn)(void* pD, uint16_t EpNum);
...
typedef struct CUSBD_Dev
{
    ...
    */
    ...
    uint8_t aHnpSupport;
    /** informs if OTG device in host mode supports HNP on alternative port */
    uint8_t aAltHnpSupport;
...
typedef struct CUSBD_epOps
{
    ...
    CUSBD_reqQueueSize reqQueueSize;
...
typedef struct CUSBD_OBJ
{
     ...
     * @param[in] ep endpoint associated with the request
     * @param[in] pD driver state info specific to this instance
     * @return number of requests in the endpoint
     */
    uint32_t (*reqQueueSize)(void *pD, struct CUSBD_Ep *ep);
    ...
    /**
     * Returns SOF status
     * @param[out] isActive if 1 device connected, if 0 device is disconnected from host
     * @param[in]  timeout argument specifies the number of milliseconds
     * @param[in] pD driver state info specific to this instance
     */
    void (*dGetSofStatus)(void* pD, uint8_t *isActive, uint32_t timeout);
