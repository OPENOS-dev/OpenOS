...
CROS_WORKON_COMMIT="214931119a1a790912a43314d1192e11603d2139"
CROS_WORKON_TREE=("46cfbf5b634705d744b131c948633ec4c89c6cc6" "40871706c95b79b57bc69248a7e6304ae7ed3c86")
...
# This package is architected to never use Cargo (except for consistency
# checking in src_test), and shouldn't act as anything but a bundle of sources.
# Hence, dependencies on the Rust toolchain aren't necessary.
# shellcheck disable=SC2034 # used by cros-rust
CROS_RUST_INSTALL_SOURCES_ONLY=1
...
src_compile() {
	...
	if [[ "${S}" != "${WORKDIR}"/* ]]; then
		...
		(
			cd "${S}" &&
			find vendor -type f -name 'LICENSE*' -exec cp --parents --target-directory="${targ}" {} +
		) || die
	fi
...
src_test() {
	...
	sed -i -e 's~directory = .*~directory = "'"${S}/vendor"'"~' "${ECARGO_HOME}/config.toml" || die
