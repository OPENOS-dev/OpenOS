...
declare -A git_repos=(
    ...
    [nvidia-video-samples]=https://github.com/Igalia/vk_video_samples/archive
)
...
last_tag=$(git -C "${deqp_repo}" describe --tags --abbrev=0 --match "vulkan-cts-*" | sed 's/vulkan-cts-//')
commit_date=$(git -C "${deqp_repo}" log -1 --pretty='format:%cd' --date=format:'%Y%m%d')
ebuild_version="${last_tag}.${commit_date}"
old_ebuild=$(git ls-files ./*.ebuild)

echo old_ebuild: "${old_ebuild}"

if [ "${old_ebuild%-r*.ebuild}" = "deqp-${ebuild_version}" ]; then
    r=${old_ebuild//deqp-.*-r\([0-9]*\).ebuild/\1/}
    ebuild="deqp-${ebuild_version}-r$((r + 1)).ebuild"
    echo bump ebuild revision to: "${ebuild}"
else
    ebuild="deqp-${ebuild_version}-r1.ebuild"
    echo bump ebuild to: "${ebuild}"
fi
