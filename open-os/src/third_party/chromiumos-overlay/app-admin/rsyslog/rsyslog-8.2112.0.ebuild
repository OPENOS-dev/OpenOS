...
IUSE+=" impcap imtcp imudp jemalloc kafka kerberos kubernetes mdblookup"
...
src_install() {
	...
	# Unneeded/unwanted files.
	local module_prefix="${ED}/usr/$(get_libdir)/rsyslog"
	local unwanted_files=(
		"${ED}"/etc/rsyslog.d/50-default.conf

		"${module_prefix}"/imdiag.so
		"${module_prefix}"/imfile.so
		"${module_prefix}"/impstats.so
		"${module_prefix}"/imptcp.so
		"${module_prefix}"/ommail.so
		"${module_prefix}"/omprog.so
		"${module_prefix}"/omruleset.so
		"${module_prefix}"/omstdout.so
		"${module_prefix}"/omtesting.so
		"${module_prefix}"/omuxsock.so
		"${module_prefix}"/pmaixforwardedfrom.so
		"${module_prefix}"/pmciscoios.so
		"${module_prefix}"/pmcisconames.so
		"${module_prefix}"/pmlastmsg.so
		"${module_prefix}"/pmsnare.so
	)
	if ! use imtcp; then
		unwanted_files+=("${module_prefix}"/imtcp.so)
	fi
	if ! use imudp; then
		unwanted_files+=("${module_prefix}"/imudp.so)
	fi
