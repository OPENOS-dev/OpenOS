...
inherit bash-completion-r1 flag-o-matic linux-info meson-multilib ninja-utils python-any-r1 cros-toolchain-funcs udev usr-ldscript
...
src_prepare() {
	local PATCHES=(
		...
		"${FILESDIR}"/CHROMIUM-Drop-sgx-group-rule.patch
		...
		"${FILESDIR}"/BACKPORT-udev-Add-dev-media-by-path-symlinks-for-media-contro.patch
		"${FILESDIR}"/BACKPORT-udev-permanent-symlinks-with-USB-revision-for-dev-me.patch
		"${FILESDIR}"/BACKPORT-udev-Fix-constness-issues-with-newer-glibc.patch
	)
...
src_configure() {
	...
	# This can be dropped after upgrading libudev >= 252.7 because upstream
	# reworked buffer management to comply with newer glibc fortify rules
	if ver_test -lt 252.7; then
		# Override FORTIFY_SOURCE to avoid false-positives
		append-cflags -U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=1
	else
		die "FORTIFY_SOURCE=1 should only be used for libudev < 252.7"
	fi
