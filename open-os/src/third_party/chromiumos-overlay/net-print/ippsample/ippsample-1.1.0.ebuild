...
inherit cros-toolchain-funcs cros-sanitizers
...
PATCHES=(
	...
	"${FILESDIR}/ippsample-1.1.0-test-path.patch"
	"${FILESDIR}/ippsample-1.1.0-relax-media-size.patch"
)
...
src_install() {
	...
	doins -r "${S}"/test
	insinto /usr/local/share/ippsample/test/print
	doins "${FILESDIR}/ipp-everywhere-pdf.conf"
	doins "${FILESDIR}/ipp-everywhere-raster.conf"
