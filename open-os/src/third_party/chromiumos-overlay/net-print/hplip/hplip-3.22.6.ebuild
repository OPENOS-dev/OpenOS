...
PATCHES=(
	...
	"${FILESDIR}/${PN}-3.22.6-load-plugin-from-dlc.patch"
	"${FILESDIR}/${PN}-3.22.6-fix-detect-invalid-raster.patch"
)
...
src_install() {
	...
	if use fuzzer ; then
		...
		fuzzer_install "${FILESDIR}"/OWNERS.fuzzer hpcups_fuzzer --comp "${fuzzer_component_id}"
		fuzzer_install "${FILESDIR}"/OWNERS.fuzzer hpps_fuzzer --comp "${fuzzer_component_id}"
	fi
}
