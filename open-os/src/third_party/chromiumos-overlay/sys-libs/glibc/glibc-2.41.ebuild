...
inherit python-any-r1 prefix preserve-libs cros-toolchain-funcs flag-o-matic gnuconfig \
...
SLOT="2.2"
IUSE="cros_host audit caps cet clone3 compile-locales custom-cflags doc gd hash-sysv-compat headers-only +multiarch multilib multilib-bootstrap nscd perl profile selinux +ssp stack-realign +static-libs static-pie suid systemd systemtap test vanilla crosscompile_opts_headers-only +nsswitch loong"
...
do_compile_test() {
	...
	# cros: Most of the time CC is already set, but not in early sanity checks.
...
setup_env() {
	...
	# * gold (bug #269274)
	# * mold (bug #860900)
	# CrOS: root cause why & fix the upstream tc-ld-force-bfd which causes bazel failures
	# because we should be using tc-ld-force-bfd instead of tc-ld-disable-gold
	tc-ld-disable-gold
...
die_if_installing_on_board() {
	# Quick heuristic to ensure this installation request is conceptually
	# sound. If it's not, the user will get an error message mid-build about
	# using the wrong toolchain, and that's really hard to map to the actual
	# problem.
	[[ "${CHOST}" == *-pc-linux-* ]] && return
	die "For non-host sysroots, glibc is supposed to be installed instead" \
		"by cross-*/glibc. If you're seeing this message, it could be" \
		"that glibc was upgraded recently. Try re-running setup_board" \
		"(or a run of 'cros build-packages' that happens to run that)?"
}
...
src_unpack() {
	die_if_installing_on_board
	# CrOS: For building cross toolchain: Initially, CC is set to ${CHOST}-clang. setup_env will set
	# CC by calling tc-getCC ${CTARGET}, which returns the value in CC if it is already set.
	# Calling cros_use_gcc won't suffice, since that sets CC=${CHOST}-gcc, and what we need is
	# CC=${CTARGET}-gcc.
	unset CC
	setup_env
...
src_prepare() {
	...
	EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" eapply "${FILESDIR}"/patches
...
glibc_do_configure() {
	...
	# b/320487146: coreboot-sdk's toolchain prebuilts can't deal with relr
	# relocations. Disable relr for the host.
	export cros_force_disable_relr=
	is_crosscompile || cros_force_disable_relr=1
...
glibc_do_src_install() {
	...
	# CrOS: Explicitly create /usr/lib -> lib64 symlink for bazel builds.
	# The glibc ebuild is currently creating the /lib -> lib64 symlink when
	# SYMLINK_LIBS=yes. We then rely on create_sdk_board_root to create the
	# /usr/lib -> lib64 symlink. This change adds the symlink creation into
	# the glibc ebuild so that we don't have the logic bifurcated.
