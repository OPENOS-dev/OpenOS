...
# Keep only below locales in the serial image to save space for developer features.
SERIAL_LOCALES="en|es|ja|pl|zh-CN|zh-TW"
...
sign_image() {
	local fw_image=$1
	local keydir="${BROOT}/usr/share/vboot/devkeys"
...
add_ec() {
	...
	if is_early_ec_sync_enabled "${coreboot_config}"; then
	...
	einfo "Padding ec{ro,rw} ${pad} byte."

	local ecrw
	if use zephyr_ec; then
		ecrw="${T}/ecrw"
		do_futility dump_fmap -x "${ecroot}/ec.bin" "RW_FW:${ecrw}"
	else
		ecrw="${ecroot}/ec.RW.bin"
	fi

	local ecrw_hash="${T}/ecrw.hash"
	openssl dgst -sha256 -binary "${ecrw}" > "${ecrw_hash}" || \
		die "Unable to compute RW hash"

	local ecrw_version
	if use wilco_ec; then
		ecrw_version="${ecroot}/ec.RW.version"
	else
		ecrw_version="${T}/ecrw.version"
		do_futility dump_fmap -x "${ecroot}/ec.bin" \
			"RW_FWID:${ecrw_version}"
	fi

	local ecrw_config="${ecroot}/ec.config"
	# TODO: b/307788351 - Check for ecrw_config in ec.bin if it doesn't exist

	do_cbfstool "${image}" add -r FW_MAIN_A,FW_MAIN_B -t raw -c "${comp_type}" \
		-f "${ecrw}" -n "ecrw" -p "${pad}"
	do_cbfstool "${image}" add -r FW_MAIN_A,FW_MAIN_B -t raw -c none \
		-f "${ecrw_hash}" -n "ecrw.hash"
	do_cbfstool "${image}" add -r FW_MAIN_A,FW_MAIN_B -t raw -c none \
		-f "${ecrw_version}" -n "ecrw.version"
	if [[ -f "${ecrw_config}" ]]; then
		do_cbfstool "${image}" add -r FW_MAIN_A,FW_MAIN_B -t raw -c lzma \
			-f "${ecrw_config}" -n "ecrw.config"
	else
		ewarn "Missing EC RW config, skip packing ecrw.config"
	fi

	if ! use ec_ro_sync; then
		einfo "Skip packing EC RO."
	elif [[ -f "${ecroot}/ec.RO.bin" ]]; then
		do_cbfstool "${image}" add -r COREBOOT -t raw -c "${comp_type}" \
			-f "${ecroot}/ec.RO.bin" -n "ecro" -p "${pad}"
		do_cbfstool "${image}" add -r COREBOOT -t raw -c none \
...
# Copy the source image and add payloads to it.
# Args:
#   $1: Source image to start from.
#   $2: Payload to add to read-only image portion.
#   $3: Payload to add to read-write image portion.
#   $4: Destination image to be created.
build_image() {
	local src_image="$1"
	local ro_payload="$2"
	local rw_payload="$3"
...
add_compressed_assets() {
	...
	if [ -n "${build_name}" ]; then
		while IFS= read -r -d '' file; do
			if [[ "${rom}" == *.serial ]] && [[ "${file}" =~ locale_.*.bin ]] && ! [[ "${file}" =~ locale_(${SERIAL_LOCALES}).bin ]]; then
				# Serial images are bulkier than other images due to extra debug features it has.
				# Skip adding locales other than the above locales to serial image in order to
				# save space for these exclusive debug features.
				einfo Skipping "${file}" for "${rom}"
				continue
			fi
			do_cbfstool "${rom}" add -r "${cbfs_regions}" -f "${file}" \
				-n "$(basename "${file}")" -t raw -c precompression
...
build_images() {
	...
	local image_name
	local image_file
	local image_file_dev
	local image_file_net
	local image_files=()
	...
	image_name="image-${build_name}"
	image_file="${outdir}${image_name}.bin"
	image_file_dev="${outdir}${image_name}.serial.bin"
	image_file_net="${outdir}${image_name}.net.bin"
	...
	check_assets "${coreboot_file}" "${depthcharge}"
	add_assets "${coreboot_file}"
	add_assets "${coreboot_file}.serial"

	build_image "${coreboot_file}" \
		"${depthcharge}" "${depthcharge}" "${image_file}"
	...
	sed -i -E "/(${SERIAL_LOCALES}),/!d" locales.serial || die "Failed to update locales.serial"
	do_cbfstool "${coreboot_file}.serial" add -r COREBOOT -f "locales.serial" -n "locales" -t raw -c lzma
	build_image "${coreboot_file}.serial" \
		"${depthcharge_dev}" "${depthcharge_dev}" "${image_file_dev}"
	image_files+=("${image_file_dev}")
	...
	build_image "${coreboot_file}.serial" \
		"${depthcharge}" "${netboot}" "${image_file_net}"
	image_files+=("${image_file_net}")

	# Add EC files to all the images.
	# b/325886850: EC files are added to CBFS as the last things to make
	# best use of CBFS space.
	...
	# ME and GPR0 are only available on Intel platform, and
	# GPR0 is only available for platforms starting from CML (hatch).
	if use intel_cpu; then
		...
		local locked_fw="${image_file_net}"
		local unlocked_fw="${image_file_net}.unlocked"
		ifdtool -p "${ifd_chipset}" -u -O "${unlocked_fw}" "${locked_fw}" ||
			die "Failed to unlock ME via ifdtool."
		ifdtool -p "${ifd_chipset}" -g -O "${unlocked_fw}" "${unlocked_fw}" ||
			die "Failed to disable GPR0 via ifdtool."
	...
	local bootfile="${PORTAGE_USERNAME}/${build_name}/vmlinuz"
	local argsfile="${PORTAGE_USERNAME}/${build_name}/cmdline"
