...
src_install() {
	...
	# Some miscellaneous files are needed to run tests. We do not know how
	# to enumerate them all yet. So we have to add them manually for now.
	doins "${BINARY_DIR}/icudtl.dat"
	doins "${BINARY_DIR}/resources.pak"
	...
	doexe "${BINARY_DIR}/cast_receiver"
	...
	doexe "${BINARY_DIR}/screen_ai_ocr_perf_test"
	...
	if use v4l2_codec; then
		...
		doexe "${BINARY_DIR}/vulkan_overlay_adaptor_test"
	fi
}
