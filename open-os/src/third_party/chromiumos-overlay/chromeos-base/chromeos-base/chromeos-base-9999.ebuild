...
inherit cros-workon user udev tmpfiles eapi8-dosym
...
SLOT="0"
KEYWORDS="~*"
IUSE="arcpp chromeless_tty cros_embedded cros_host kdump pam vtconsole"
...
src_install() {
	...
	if ! use cros_host ; then
		...
		# System with ARC container does not support pid space bigger than or equals
		# to 65536.
		if ! use arcpp ; then
			doins "${FILESDIR}"/11-pid-max.conf
		fi
		...
		# Preserve existing kmod files as we transition to kmod 32-r1
		# which moves its files from /sbin to /usr/bin
		dosym8 -r /usr/bin/kmod /sbin/depmod
		dosym8 -r /usr/bin/kmod /sbin/insmod
		dosym8 -r /usr/bin/kmod /sbin/kmod
		dosym8 -r /usr/bin/kmod /sbin/lsmod
		dosym8 -r /usr/bin/kmod /sbin/modinfo
		dosym8 -r /usr/bin/kmod /sbin/modprobe
		dosym8 -r /usr/bin/kmod /sbin/rmmod
		...
		doins "${FILESDIR}"/cursor.sh "${FILESDIR}"/editor.sh "${FILESDIR}"/pager.sh \
			"${FILESDIR}"/openos-welcome-profile.sh

		# OPENOS first-boot TUI welcome animation
		exeinto /usr/bin
		doexe "${FILESDIR}"/openos-welcome.sh

		# OPENOS first-boot device model setup (Upstart)
		insinto /etc/init
		...
		insopts -m 440
		echo "${SHARED_USER_NAME} ALL=(ALL) ALL" | newins - 95_cros_base
	fi
}
