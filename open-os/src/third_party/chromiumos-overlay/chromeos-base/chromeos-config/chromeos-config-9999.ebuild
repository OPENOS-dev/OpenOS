...
# Modified by Open-OS Project (2026-07-04)
...
DESCRIPTION="Open-OS system configuration (based on chromiumos-config)"
...
run_cros_config_tool() {
	...
	PYTHONPATH="${S}/chromeos-config" "${EPYTHON}" -m \
		"cros_config_host.${tool}" "$@"
}
...
src_compile() {
	...
	# Protobuf based configs generate JSON directly with no YAML.
	if [[ -f "${SYSROOT}${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}/project-config.json" ]]; then
		einfo "Using project-config.json located at ${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}"
		yaml_files=( "${SYSROOT}${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}/project-config.json" )
	else
		einfo "Did not find project-config.json in ${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}."
	fi
