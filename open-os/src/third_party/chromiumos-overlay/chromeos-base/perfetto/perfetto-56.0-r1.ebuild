...
CROS_WORKON_COMMIT="8b65138050c38379e489e4a323e0472c08497449"
CROS_WORKON_TREE="d2a78086f87c80edc5349072a5d693f8ac3b4fca"
...
CROS_WORKON_LOCALNAME="third_party/perfetto"
CROS_WORKON_PROJECT="external/github.com/google/perfetto"
CROS_WORKON_REPO="${CROS_GIT_HOST_URL}"
CROS_WORKON_EGIT_BRANCH="main"

inherit cros-workon ninja-utils cros-protobuf tmpfiles toolchain-funcs user
...
src_configure() {
	tc-export CC CXX AR STRIP BUILD_CC BUILD_CXX BUILD_AR BUILD_STRIP PKG_CONFIG BUILD_PKG_CONFIG
...
strip=\"${BUILD_STRIP}\"
linker=\"${linker_name}\"
host_pkg_config=\"${BUILD_PKG_CONFIG}\"
target_ar=\"${AR}\"
target_cc=\"${CC}\"
target_cxx=\"${CXX}\"
...
enable_perfetto_trace_processor_mac_instruments=false
...
"
	...
	# Save ${GN_ARGS} before adding sanitizer gn args because SDK source
	# generation doesn't work well with sanitizer.
...
is_ubsan=true"
	...
	# * Disable watching socket creation using inotify syscalls in the
	#   SDK.
	local sdk_gn_args+="
...
extra_target_cflags=\"${CFLAGS}\"
extra_target_cxxflags=\"${CXXFLAGS}\"
enable_perfetto_sock_inotify=false
"
...
src_compile() {
	eninja -C "${BUILD_OUTPUT}" traced traced_probes perfetto trace_processor_shell libtrace_processor.a
...
src_install() {
	...
	dolib.a "${BUILD_OUTPUT}/libtrace_processor.a"
}
