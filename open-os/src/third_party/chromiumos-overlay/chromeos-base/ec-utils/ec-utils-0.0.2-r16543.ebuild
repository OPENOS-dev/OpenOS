...
EAPI=7
CROS_WORKON_COMMIT="b0d8684f0ac19b032f5ead7acef195fd5e206dcd"
CROS_WORKON_TREE="bd8f5365a2bff4701a161b4ae281c0cecbbb9661"
...
inherit cros-workon libchrome user udev tmpfiles cros-sanitizers
...
pkg_preinst() {
	enewgroup "dialout"
	# Ensure that this group exists so that ec_coredump can access /dev/cros_ec.
	enewgroup "cros_ec-access"
	# Create user and group for ec_coredump.
	enewuser "ec_coredump"
	enewgroup "ec_coredump"
}
...
src_install_cros_ec_utils() {
	if use cros_host; then
		...
		dosbin "build/host/util/ec_coredump"
		dotmpfiles "${FILESDIR}/tmpfiles.d/cros-ec.conf"
		udev_dorules "${FILESDIR}/99-ec-coredump.rules"
		udev_dorules "${FILESDIR}/99-ec-panic-sync.rules"
	fi
}
