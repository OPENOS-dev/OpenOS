...
src_install() {
	...
	# Install tmpfiles script for generating the necessary directory and clearing
	# content on device renewal.
	dotmpfiles "${FILESDIR}"/tmpfiles.d/tee-supplicant.conf
	insinto /usr/lib/tmpfiles.d/on-demand
	doins "${FILESDIR}"/tmpfiles.d/on-demand/tee-supplicant-wipe.conf
