...
src_prepare() {
	...
	# OPENOS: Overwrite original files with modified OOBE files (device naming step).
	# Using direct file replacement instead of patches.
	local oobe_dir="${FILESDIR}/oobe-device-name"
	cp "${oobe_dir}/chrome/browser/ash/login/screens/welcome_screen.h" \
	   "chrome/browser/ash/login/screens/welcome_screen.h" || die
	cp "${oobe_dir}/chrome/browser/ash/login/screens/welcome_screen.cc" \
	   "chrome/browser/ash/login/screens/welcome_screen.cc" || die
	cp "${oobe_dir}/chrome/browser/resources/chromeos/login/screens/oobe/welcome.ts" \
	   "chrome/browser/resources/chromeos/login/screens/oobe/welcome.ts" || die
	cp "${oobe_dir}/chrome/browser/resources/chromeos/login/screens/oobe/welcome.html" \
	   "chrome/browser/resources/chromeos/login/screens/oobe/welcome.html" || die
	einfo "OPENOS: Applied OOBE device naming step modifications"
...
pkg_preinst() {
	...
	if [[ ${chrome_size} -ge 330000000 && -z "${KEEP_CHROME_DEBUG_SYMBOLS}" ]] && use chrome_internal && ! use chrome_dcheck; then
