...
constexpr const char kUserActionSetDeviceName[] = "setDeviceName";
...
void WelcomeScreen::OnUserAction(const base::ListValue& args) {
  ...
  if (action_id == kUserActionSetDeviceName) {
    CHECK_EQ(args.size(), 2u);
    OnDeviceNameEntered(args[1].GetString());
    return;
  }
...
void WelcomeScreen::OnDeviceNameEntered(const std::string& device_name) {
  // Write device name to /var/lib/openos/device-name
  base::FilePath path("/var/lib/openos/device-name");
  if (!base::PathExists(path.DirName())) {
    base::CreateDirectory(path.DirName());
  }
  base::WriteFile(path, device_name);
  OnContinueButtonPressed();
}
