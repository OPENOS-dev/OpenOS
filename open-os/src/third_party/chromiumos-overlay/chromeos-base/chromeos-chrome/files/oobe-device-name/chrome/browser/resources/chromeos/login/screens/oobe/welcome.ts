...
export class OobeWelcomeScreen extends OobeWelcomeScreenBase {
  ...
  static get properties(): PolymerElementProperties {
    return {
      ...
      /**
       * Device name entered by user during OOBE.
       */
      deviceName: {
        type: String,
        value: '',
      ...
      /**
       * Error message for device name validation.
       */
      deviceNameError: {
        type: String,
        value: '',
  ...
  private deviceNameError: string;
  ...
  private cancel(): void {
    ...
    if (this.uiStep === WelcomeScreenState.DEVICE_NAME) {
      this.closeDeviceNameSection();
      return;
    }
  }
   ...
   * Shows the device name step before proceeding.
   */
  private onWelcomeNextButtonClicked(): void {
    this.cancelChromeVoxHint();
    this.deviceName = '';
    this.deviceNameError = '';
    this.setUIStep(WelcomeScreenState.DEVICE_NAME);
  }
  ...
  private getDeviceNameInput(): CrInputElement {
    const input = this.shadowRoot?.querySelector('#deviceNameInput');
    assert(input instanceof CrInputElement);
    return input;
  }

  /**
   * Handle input change in device name field.
   */
  private onDeviceNameInputChanged_(): void {
    const name = this.getDeviceNameInput().value.trim();
    this.deviceName = name;
    this.deviceNameError = name.length === 0 ? 'Please enter a device name' : '';
  }

  /**
   * Handle "Continue" button on device name step.
   */
  private onDeviceNameContinueClicked_(): void {
    const name = this.deviceName.trim();
    if (name.length === 0) {
      this.deviceNameError = 'Please enter a device name';
      return;
    }
    this.userActed(['setDeviceName', name]);
  }

  /**
   * Handle "Back" button on device name step.
   */
  private closeDeviceNameSection(): void {
    this.setUIStep(WelcomeScreenState.GREETING);
  }
}
