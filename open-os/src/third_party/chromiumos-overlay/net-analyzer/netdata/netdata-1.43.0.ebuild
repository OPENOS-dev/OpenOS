...
inherit autotools linux-info python-single-r1 systemd cros-sanitizers
...
PATCHES=(
	"${FILESDIR}/${P}-use-char-type-with-char_traits.patch"
	"${FILESDIR}/${P}-dlib-global_optimization-add-template-argument-list.patch"
)
...
src_install() {
	...
	doins "${FILESDIR}"/user_config/netdata.conf

	insinto /usr/local/etc/netdata/
	doins "${FILESDIR}"/user_config/.user_plugins.lst
