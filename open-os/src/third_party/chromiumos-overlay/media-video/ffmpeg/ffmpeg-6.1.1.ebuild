...
inherit flag-o-matic multilib multilib-minimal cros-toolchain-funcs ${SCM}
...
multilib_src_configure() {
	...
	set -- "${S}/configure" \
		...
		--libdir="${EPREFIX}/usr/local/$(get_libdir)" \
		--docdir="${EPREFIX}/usr/local/share/doc/${PF}/html" \
		--mandir="${EPREFIX}/usr/local/share/man" \
		--cc="$(tc-getCC)" \
		--cxx="$(tc-getCXX)" \
		--ar="$(tc-getAR)" \
		--nm="$(tc-getNM)" \
