...
inherit eutils cros-toolchain-funcs
...
src_unpack() {
	...
	mv go "go-${PV}"
	unpack "go${BOOTSTRAP_VER}.src.tar.gz"
	mv go "go${BOOTSTRAP_VER}"
}

src_configure() {
	export GOROOT_BOOTSTRAP="${WORKDIR}/go${BOOTSTRAP_VER}"
...
src_install() {
	...
	# The src directory contains testdata, which we don't need and which
	# can cause problems if installed (see b/313514253). Remove it before
	# installing.
	find src -name testdata -exec rm -r {} + || die
	...
	doexe "${tooldir}/"{asm,cgo,compile,link}
	doexe "${tooldir}/"vet
	doexe "${tooldir}/"cover
	...
	# Go pkg archives are not ELF files and cannot be stripped.
	dostrip -x "$(get_goroot)/pkg/linux_$(get_goarch "${CBUILD}")"
}
