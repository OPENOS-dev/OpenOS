...
inherit autotools cros-fuzzer cros-sanitizers flag-o-matic multilib cros-toolchain-funcs
...
# Temporary fix for the ghostscript 10.04.0 uprev.
# The removed patch from the patch archive has merge
# conflict in ghostscript 10.04.0. We recreate this patch
# in files/ghostscript-gpl-10.04.0-Don-t-force-libstdc.patch.
#
# This temporary fix, together with the added patch, should
# be removed once gentoo upgrades the archive at
# https://gitweb.gentoo.org/proj/codec/ghostscript-gpl-patches.git/
src_unpack() {
    default
    rm ghostscript-gpl-10.04.0-patches/0004-Don-t-force-libstdc-fix-build-w-libc.patch
}
...
src_prepare() {
	...
	if use crosfonts; then
		# Keep the D050000L font (used for ZapfDingbats in our Fontmap.cros
		# file) but remove all the others.
		cp "${S}/Resource/Font/D050000L" "${S}/Resource/D050000L" || die
		rm -r "${S}/Resource/Font/"* || die
		mv "${S}/Resource/D050000L" "${S}/Resource/Font/D050000L" || die
		cp "${FILESDIR}/Fontmap.cros" "${S}/Resource/Init/Fontmap.GS" || die
	fi
...
src_configure() {
	...
	local devices=(
		...
		ppmraw ps2write pxlcolor pxlmono r4081 sj48 stcolor t4693d4 tek4696
		uniprint
