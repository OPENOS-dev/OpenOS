...
inherit cros-constants cros-toolchain-funcs python-any-r1
...
autotest_src_compile() {
	...
	if [ "${AUTOTEST_PROFILERS_LIST}" = "*" ]; then
		AUTOTEST_PROFILERS_LIST=$(\
			print_test_dirs client/profilers yes | sort -u)
	fi
	...
	if [ -n "${test_opt}" -o -n "${dep_opt}" -o -n "${prof_opt}" ]; then
		...
		mkdir -p packages
		local logfile="packages/${CATEGORY}_${PN}.log"
		"${EPYTHON}" utils/packager.py \
			-r packages \
...
autotest_src_install() {
	...
	# Install packages.
	if [[ -d "${AUTOTEST_WORKDIR}/packages" ]]; then
		insinto "${AUTOTEST_BASE}/packages"
		doins "${AUTOTEST_WORKDIR}/packages/"*.tar.bz2
	fi
...
autotest_pkg_postinst() {
	# TODO: Remove this function once there is no caller.
	:
}
