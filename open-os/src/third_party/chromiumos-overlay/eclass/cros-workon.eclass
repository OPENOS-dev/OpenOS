...
CROS_SOURCE_REPO_FOLDER="${CROS_WORKON_SRCROOT}/.repo"
CROS_SOURCE_SUPERMANIFEST="${CROS_WORKON_SRCROOT}/.supermanifest"

WORKSPACE_TYPE="repo"
if [[ ! -d "${CROS_SOURCE_REPO_FOLDER}" && -f "${CROS_SOURCE_SUPERMANIFEST}" ]]; then
	WORKSPACE_TYPE="cog"
fi
...
local_copy_cp() {
	...
	for sl in "${CROS_WORKON_SUBDIRS_TO_COPY[@]}"; do
		...
		pushd "${src}/${sl}" >/dev/null || die "cd ${src}/${sl}"

		# Construct a find expression to exclude the files
		# ignored by .gitignore.
		local -a git_ignore_expressions
		readarray -t git_ignore_expressions < <(
		...
		local -a find_cmd=(
			find .
			# List all the exclude patterns.
			\(
				# build actions shouldn't be interacting with git.
				-path ./.git

				# Python compiled objects are a pain.
				-o -name '*.py[co]'

				# Add the git ignored file expressions.
				"${git_ignore_expressions[@]}"

			# Prune all the excluded files.
			\) -prune

			# Print the remaining files. We exclude directories because we only want
			# files. cp creates the parent directories.
			-o ! -type d -print0
		)
		...
		local -a cp_cmd=(
			...
			cp
			# Preserve the src parent directory structure.
			--parents
			# Copy symlinks unchanged.
			--no-dereference
			# Set the target.
			--target-directory "${dst}/${sl}/"
		)

		# Use `find`, `xargs`, and `cp` to clone the src. It's a lot faster than `rsync`
		# due to the parallelism.
		"${find_cmd[@]}" | "${cp_cmd[@]}" || die "${find_cmd[*]} | ${cp_cmd[*]}"
		...
		local fix_chmod_cmd=(
			find "${dst}/${sl}/"
			# Any dirs we copy should have 755 perms.
			-type d '!' -perm 755 -exec chmod 755 {} + ','
			# Any +x files we copy should have 755 perms.
			-type f -perm /a+x '!' -perm 755 -exec chmod 755 {} + ','
			# Any -x files we copy should have 644 perms.
			-type f '!' -perm /a+x '!' -perm 644 -exec chmod 644 {} +
		)

		"${fix_chmod_cmd[@]}" || die "${fix_chmod_cmd[*]}"
	done
}
...
cros-workon_src_unpack() {
	...
	# CoG workspaces do not contain git repositories, so any git clone
	# operations cannot reference a local path. Since this is the case,
	# manually overwrite the path attribute with the upstream git repository,
	# so that git clone is always cloning from the remote.
	old_path=("${path[@]}")
	if [[ ${WORKSPACE_TYPE} == "cog" ]]; then
		path=( )
		for i in ${!CROS_WORKON_REPO[@]}; do
			path+=( "${CROS_WORKON_REPO}/${CROS_WORKON_PROJECT}" )
		done
	fi

	einfo "repo(s): ${repo}\nproject(s): ${project[@]}\ndestdir(s): ${destdir[@]}"
	einfo "subtree(s): ${subtree[@]}\nbranch(es): ${branch[@]}\npath(s): ${path}"
		...
		for (( i = 0; i < project_count; ++i )); do
			...
			if [[ -d "${path[i]}" ]]; then
				...
				if [[ "${CROS_WORKON_COMMIT[i]}" == "main" ||
					...
					if ! git clone -sn "${path[i]}" "${destdir[i]}"; then
						die "Can't clone ${path[i]} to ${destdir[i]}."
					elif ! (cd "${destdir[i]}" && git checkout -q "${CROS_WORKON_COMMIT[i]}") ; then
	...
	if [[ "${fetch_method}" == "local" && ! -d ${path} ]] ; then
		einfo "workspace type: ${WORKSPACE_TYPE}"
		# For Cog workspaces, we expect the ${path} variable to be a remote URI
		# instead of a local path, so the warnings don't matter in this case.
		if [[ "${WORKSPACE_TYPE}" != "cog" ]]; then
	...
	# We do want to unilaterally reset the path variable if it was overwritten in
	# a Cog workspace.
	if [[ ${WORKSPACE_TYPE} == "cog" ]]; then
		path=("${old_path[@]}")
	fi
...
cros-workon_enforce_subtrees() {
	...
	local ignore_dirs=()
	for (( i = 0; i < project_count; ++i )); do
		# Prevent src/platform/dev/contrib from being used in ebuilds.
		if [[ "${CROS_WORKON_PROJECT[i]}" == "chromiumos/platform/dev-util" ]] \
			&& ! has "${destdir[i]}/contrib" "${ignore_dirs[@]}"; then
			ignore_dirs+=( "${destdir[i]}/contrib" )
		fi
	...
	# Error if any keep_dirs is also in ignore_dirs.
	local dir
	for dir in "${ignore_dirs[@]}"; do
		if has "${dir}" "${keep_dirs[@]}"; then
			die "Subtree ${dir} is not allowed."
		fi
	done
	...
	# If the directory to keep is only ${S} and there are no ignore_dirs, then
	# there is nothing we need to do.
	if [[ "${#keep_dirs[@]}" == 1 && "${keep_dirs}" == "${S}" \
		&& "${#ignore_dirs[@]}" == 0 ]]; then
		return
	fi
...
cros-workon_pkg_setup() {
	if [[ ${MERGE_TYPE} != "binary" && ${CROS_WORKON_INCREMENTAL_BUILD} == "1" ]]; then
		...
		# Check for dependency changes that necessitate clearing the
		# incremental cache.
		if [[ -n "${_ECLASS_CROS_PROTOBUF}" ]]; then
			local protoc_version="$(protoc --version || die)"
			# A generic name is used for this file, so it can be
			# used to cover other generators / versions in the
			# future.
			local versions_file="${out}/.version-check"
			if ! cmp -s <(echo "${protoc_version}") "${versions_file}" ; then
				einfo "Clearing incremental cache."
				rm -rf "${out}/"*
			fi
			echo "${protoc_version}" > "${versions_file}" || die
			chown ${PORTAGE_USERNAME}:${PORTAGE_GRPNAME} "${versions_file}" || die
		fi
