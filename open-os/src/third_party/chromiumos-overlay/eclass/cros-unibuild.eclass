...
# @ECLASS-VARIABLE: UNIBOARD_JSON_CONFIG_DIR
# @DESCRIPTION:
#  This is the installation directory of project-config.json, which is read and
#  subsequently written to UNIBOARD_YAML_CONFIG. tmpdir is used to prevent
#  duplicating file contents.
UNIBOARD_JSON_TO_YAML_CONFIG_DIR="/usr/local/share/chromeos-config"
...
platform_json_compile() {
	...
	cros_config_schema \
		-o "${WORKDIR}/project-config.json" --minify \
		-m "${files[@]}" \
...
platform_json_install() {
	...
	einfo "Install location: ${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}/project-config.json"
	insinto "${UNIBOARD_JSON_TO_YAML_CONFIG_DIR}"
	doins "${WORKDIR}/project-config.json"
}

# @FUNCTION: platform_config_proto_install
# @USAGE:
# @DESCRIPTION:
# Install project and program config.jsonproto files.
# Find all the files in ${WORKDIR}, zip them, and install into
# ${UNIBOARD_CROS_CONFIG_DIR}.
# These files will be available to the Build API as artifacts.
platform_config_proto_install() {
	[[ $# -eq 0 ]] || die "${FUNCNAME}: takes no arguments"
	einfo "Installing project and program config.jsonproto files."

	local zip_name=config_protos.zip

	find "${WORKDIR}" -name "config.jsonproto" -exec zip "${zip_name}" {} + || die
