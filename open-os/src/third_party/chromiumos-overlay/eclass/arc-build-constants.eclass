...
arc-build-constants-configure() {
	...
	# To avoid the limited rootfs size problem on the reven board,
	# install the ARC image on a temporary path before packaging it
	# into the DLC image. /build/rootfs is a temporary rootfs path
	# that will not be copied to the actual root partition.
	if use arcvm_dlc; then
		ARC_VM_PREFIX="/build/rootfs/android_tmp"
	fi
