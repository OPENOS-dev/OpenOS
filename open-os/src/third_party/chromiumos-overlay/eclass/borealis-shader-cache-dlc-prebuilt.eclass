...
# WARNING: `BOREALIS_` variables are set and parsed via regex to extract
# values out in Kabuto. If you are updating the variable names, update
# here and in Kabuto at the same time with CQ depends.
...
src_install() {
	...
	if [[ "${pkg_mesa_build_id}" != "${local_mesa_build_id}" ]]; then
		eerror "${PN} (${BOREALIS_SHADER_CACHE_DEVICE_ID}) Mesa build id mismatch:"
		eerror "  local:    ${local_mesa_build_id}"
		eerror "  packaged: ${pkg_mesa_build_id}"
		if [[ "${BOREALIS_SHADER_CACHE_USED_IN_TESTS}" == "true" ]]; then
			# If the DLC is used for tast, always install regardless of Mesa build id mismatch
			eerror "BOREALIS_SHADER_CACHE_USED_IN_TESTS=true, installing DLC despite mismatch."
			dlc-prebuilt_src_install
		else
			# If Kabuto automation failed to build the correct shader cache, do not install the DLC
			eerror "DLC not installed."
		fi
	else
		# Continue installation without any error logs
		dlc-prebuilt_src_install
	fi
}
