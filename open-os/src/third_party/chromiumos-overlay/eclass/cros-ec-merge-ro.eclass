...
# Some firmware build packages use a file called revision_bump to trigger a
# modification. For example, see the following file:
# sys-firmware/chromeos-fpmcu-release-buccaneer/files/revision_bump
...
# @FUNCTION: cros-ec-merge-ro_find_ro_fw
# @USAGE: <board> <firmware-bin path>
# @DESCRIPTION:
# Find appropriate RO firmware in <firmware-bin path> for <board>
cros-ec-merge-ro_find_ro_fw() {
	local fw_target="${1%_fp}"
	local firmware_bin_dir="$2"
...
cros-ec-merge-ro_src_install() {
	...
	if [[ -z "${FIRMWARE_EC_BOARD}" ]]; then
		eerror "FIRMWARE_EC_BOARD is not defined"
		die
	fi
	...
	local ro_fw="$(cros-ec-merge-ro_find_ro_fw "${target}" "${firmware_bin_dir}")"
