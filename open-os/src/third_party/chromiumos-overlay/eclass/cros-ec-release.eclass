...
# `sys-firmware/chromeos-fpmcu-release-*/files/revision_bump' to cause
...
cros-ec-release_src_prepare() {
	...
	local private_directories=("ec-private" "fingerprint")

	# Link the private sources in the private/ sub-directory.
	local dir
	for dir in "${private_directories[@]}"; do
		ln -sfT "${SYSROOT}/firmware/${FIRMWARE_EC_BOARD}/release/${dir}" \
			"${S}/${dir}" || die
	done
...
cros-ec-release_src_install() {
	...
	# Do not strip debug information from ec.RO.elf, ec.RW.elf and ec.obj.
	dostrip -x "/firmware/${target}/release"/ec.{RO,RW}.elf \
		"/firmware/${target}/release"/ec.obj
