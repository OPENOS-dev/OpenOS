...
# @ECLASS-VARIABLE: CROS_GO_COVER_PACKAGES
# @DESCRIPTION:
# Go packages to include in code coverage.
# Default is to include only the exact packages in CROS_GO_TEST.
...
cros-go_gopath() {
	printf "%s:%s" "$(cros-go_workspace)" "${SYSROOT}/usr/lib/gopath"
...
go_list() {
	if ! GO111MODULE=${GO111MODULE:-off} GOPATH="$(cros-go_gopath)" $(tc-getGO) list "$@"; then
		[[ -z "${CROS_GO_SKIP_DEP_CHECK}" ]] && die
	fi
}
...
# @FUNCTION: go_tool
# @DESCRIPTION:
# Wrapper function for running tools.
# Tools are always run locally on host.
go_tool() {
	GO111MODULE=${GO111MODULE:-off} GOPATH="$(cros-go_gopath)" $(tc-getBUILD_GO) tool "$@" || die
}
...
# @FUNCTION: go_lint
# @DESCRIPTION:
# Wrapper function for running "staticcheck"
go_lint() {
	# shellcheck disable=SC2154
	local staticcheck_output_base="${CROS_ARTIFACTS_TMP_DIR}/linting-output/staticcheck"
	mkdir -p "${staticcheck_output_base}"

	local file_name="${1//'/...'/}"
	file_name="${staticcheck_output_base}/${file_name//'/'/-}-$(date +%s)"

	# b/417772434: `-fail ''` is set here, since a build failure in a linter
	# prevents lints from being produced. That's counterproductive.
	GO111MODULE="${GO111MODULE:-off}" GOPATH="$(cros-go_gopath)" \
		staticcheck -checks "inherit,-SA1019" -fail '' "$@" \
			>> "${file_name}.txt" || die
}
...
# Arguments are forwarded directly to 'go build' command.
cros-go_src_compile() {
...
cros-go_src_test() {
	...
	if use coverage; then
		local coverpkg=()
		# shellcheck disable=SC2154
		if (( ${#CROS_GO_COVER_PACKAGES[@]} != 0 )); then
			coverpkg=("-coverpkg" "$(IFS=, ; echo "${CROS_GO_COVER_PACKAGES[*]}")")
		fi
		local coverage_path="${CROS_ARTIFACTS_TMP_DIR}/coverage_logs"
		mkdir -p "${coverage_path}"
		local pkg_cover="${PN}_cover.out"
		local pkg_report="${PN}.html"
		go_test -covermode=atomic -short \
			-coverprofile="${coverage_path}/${pkg_cover}" \
			"${coverpkg[@]}" "${pkglist[@]}"
		go_tool cover -html="${coverage_path}/${pkg_cover}" \
			-o "${coverage_path}/${pkg_report}"
	else
...
cros-go_pkg_postinst() {
	...
	local CROS_GO_WORKSPACE=(
		"${SYSROOT}/usr/lib/gopath"
	)

	if ! tc-is-cross-compiler; then
		# BROOT will have SYSROOT != /, but can use packages from /.
		CROS_GO_WORKSPACE+=( "/usr/lib/gopath" )
	fi
