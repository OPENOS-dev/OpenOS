...
# @ECLASS-VARIABLE: CROS_SANITIZER_DISABLE_UBSAN_FUNCTION
# @PRE_INHERIT
# @DESCRIPTION:
# Disables `-fsanitize=function` if set to a nonempty value.
#
# If set in a first-party ebuild, please file a bug to clean the usage up.
: "${CROS_SANITIZER_DISABLE_UBSAN_FUNCTION:=}"

# @ECLASS-VARIABLE: CROS_SANITIZER_DISABLE_UBSAN_VPTR
# @PRE_INHERIT
# @DESCRIPTION:
# Disables `-fsanitize=vptr` if set to a nonempty value.
#
# If set in a first-party ebuild, please file a bug to clean the usage up.
: "${CROS_SANITIZER_DISABLE_UBSAN_VPTR:=}"
...
ubsan-setup-env() {
	...
	use ubsan || return 0
	local flags=(
		"-fsanitize=alignment,bounds,pointer-overflow"
		"-fsanitize=integer-divide-by-zero,float-divide-by-zero"
		"-fsanitize=signed-integer-overflow,shift,vla-bound"
		"-fno-sanitize-recover=all"
		"-frtti"
	)
	if [[ -z "${CROS_SANITIZER_DISABLE_UBSAN_FUNCTION:-}" ]]; then
		flags+=( "-fsanitize=function" )
	fi
	append-flags "${flags[@]}"
	append-ldflags "${flags[@]}"
	append-cppflags -DCHROMEOS_UBSAN_BUILD
	# If vptr checks are disabled, we need to not set -fsanitize=vptr on C/C++
	# builds. _However_, some ebuilds link to libraries from other ebuilds that
	# have `-fsanitize=vptr` enabled, so the linker needs to be prompted to
	# consider those libraries regardless.
	if [[ -z "${CROS_SANITIZER_DISABLE_UBSAN_VPTR:-}" ]]; then
		append-cppflags "-fsanitize=vptr"
	fi
	append-ldflags "-fsanitize=vptr"
...
cros-rust-setup-sanitizers() {
	...
	if use asan || use lsan || use msan || use tsan; then
		...
		if ver_test "${rustc_ver}" -ge "1.92.0"; then
			rust_san_flags+=( -Cunsafe-allow-abi-mismatch=sanitizer )
		fi
	fi
