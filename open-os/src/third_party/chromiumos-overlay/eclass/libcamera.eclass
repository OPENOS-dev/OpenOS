...
# individually, LIBCAMERA_PIPELINES to be used, and the only LIBCAMERA_IPA (as
# well as the potential LIBCAMREA_IPA_GPU) be connected via IPC.
...
# TODO(b/313639713): Drop libchrome dependency after cleaning up cros_camera_hal.
inherit libchrome meson
...
# @ECLASS-VARIABLE: LIBCAMERA_IPA
# @DESCRIPTION:
# The name of ipa that will be connected via IPC. If non-empty, a symbolic link
# will be created to link to the ipa_proxy.
: "${LIBCAMERA_IPA:=}"

# @ECLASS-VARIABLE: LIBCAMERA_IPA_GPU
# @DESCRIPTION:
# The name of ipa of gpu that will be connected via IPC. If non-empty, a
# symbolic link will be created to link to the ipa_proxy.
: "${LIBCAMERA_IPA_GPU:=}"
...
libcamera_src_install() {
	...
	if [[ -n "${LIBCAMERA_IPA}" ]]; then
		dosym "${LIBCAMERA_IPA}_ipa_proxy.so" "/usr/$(get_libdir)/libcamera_ipa_proxy.so"
	fi

	if [[ -n "${LIBCAMERA_IPA_GPU}" ]]; then
		dosym "${LIBCAMERA_IPA_GPU}_ipa_proxy.so" "/usr/$(get_libdir)/libcamera_ipa_gpu_proxy.so"
	fi
