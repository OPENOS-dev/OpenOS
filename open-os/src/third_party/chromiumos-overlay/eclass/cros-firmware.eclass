...
# from source".
IUSE="bootimage cros_ish test tot_firmware unibuild zephyr_ec fw_lzma_zip"
...
# `futility` and `cbfstool` are needed when building the firmware updater.
BDEPEND="
	${PYTHON_DEPS}
	app-arch/p7zip
	app-arch/zip
...
PINNED_FIRMWARE_DIR="pinned"
EC_COMPONENT_MANIFESTS_DIR="cme"
...
cros-firmware_src_compile() {
	...
	_add_param ext_cmd --image_output "${PINNED_FIRMWARE_DIR}"
	_add_param ext_cmd \
		--ec_component_manifest_output "${EC_COMPONENT_MANIFESTS_DIR}"
	if use fw_lzma_zip; then
		_append_var ext_cmd --lzma_zip
	fi
	...
	if use tot_firmware; then
		...
		einfo "Build BCS firmware updater to ${UPDATE_SCRIPT}:" \
			"${ext_cmd[*]}"
	...
	if [[ ! -f "${UPDATE_SCRIPT}" ]]; then
		einfo "Build local updater to ${UPDATE_SCRIPT}:" \
...
cros-firmware_src_install() {
	...
	if [[ -d "${PINNED_FIRMWARE_DIR}" ]]; then
		insinto /build/firmware
		doins -r "${PINNED_FIRMWARE_DIR}"
	fi

	if [[ -d "${EC_COMPONENT_MANIFESTS_DIR}" ]]; then
		insinto /usr/share
		doins -r "${EC_COMPONENT_MANIFESTS_DIR}"
	fi
}
