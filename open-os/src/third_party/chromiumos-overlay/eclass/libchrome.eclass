...
# Compiling libchrome with the ipcz flag enabled causes it to export the
# ENABLE_IPCZ_ON_CHROMEOS define. This define is used by other packages
# in order to conditionally support ipcz. We add the ipcz flag to IUSE
# to ensure that packages that depend on libchrome are always built
# against the appropriate variant of libchrome (ipcz enabled/disabled).
IUSE="ipcz"

# Require a recent one from time to time to help keep people up-to-date.
# shellcheck disable=SC2154
RDEPEND="
	>=chromeos-base/libchrome-0.0.1-r${REQUIRED_LIBCHROME_EBUILD_VERSION}:0=[cros-debug=,ipcz=]
"

# libcore-base.so includes the perfetto SDK. Perfetto users needs to
# rebuild when perfetto uprevs to avoid using different implementations
# from including perfetto.h and in libbase-core.so.
DEPEND="${RDEPEND}
