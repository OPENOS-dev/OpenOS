...
src_prepare() {
	...
	# See b/474167829 and https://bugs.gentoo.org/968239.
	export WarningsNotAsErrors="CS7035"

	# dotnet looks at special variables to check for paths to compilers.
	# See: https://github.com/dotnet/runtime/issues/3972#issuecomment-595489386
	export CLR_CC="$(which "$(tc-getCC)")"
	export CLR_CXX="$(which "$(tc-getCXX)")"
	export CLR_LINK="$(which "$(tc-getLD)")"
	export PKG_CONFIG="$(which "$(tc-getPKG_CONFIG)")"
...
src_compile() {
	...
	local -a buildopts=(
		...
		# Use mono runtime for now as it is easier to build
		--use-mono-runtime
...
src_install() {
	...
	fperms 0755 "${dest}"
	dosym "${EPREFIX}/${dest}/dotnet" "/usr/bin/dotnet-${SDK_SLOT}"
}
