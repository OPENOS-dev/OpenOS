...
inherit flag-o-matic python-single-r1 strip-linguas cros-toolchain-funcs
...
SLOT="0"
IUSE="amd64 cet debuginfod guile lzma multitarget nls +python +server sim source-highlight test vanilla xml xxhash zstd"
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-15.1-python.patch
	"${FILESDIR}"/${PN}-15.1-inlined-unwind.patch
)
...
src_install() {
	...
	if use python ; then
		...
		# ChromeOS: install auto-load script for libc++ pretty printers
		if [[ -f "${ESYSROOT}/usr/share/libcxx/gdb/printers.py" ]] ; then
			insinto "/usr/share/gdb/auto-load/usr/$(get_libdir)"
			doins "${FILESDIR}/libc++.so.1.0-gdb.gdb"
		fi
	fi
}
