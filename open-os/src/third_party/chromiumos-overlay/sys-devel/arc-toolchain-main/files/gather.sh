...
# main-arc-dev. Before running it, please modify the following 4 variables
#   - ANDROID_TREE
#   - ARTIFACTS_DIR_ARM64
...
#
# Go to go/ab, select branch git_main-arc-dev. Pick a -userdebug build for all
...
#
# 3. Build bionic/libc, deapexer, and their dependencies
#
# Go to $ANDROID_TREE and run:
# $ . build/rbesetup.sh
# $ lunch bertha_x86_64-trunk_staging-userdebug
# $ mmm bionic/libc
# $ m deapexer
#
# $ lunch bertha_arm64-trunk_staging-userdebug
# $ mmm bionic/libc
#
# Add a comment to save the sha of the artifact git repositories to the
# ebuild so that we can reproduce this if necessary. You should be able to
# find them using git log in the following directories:
#   bionic/libc: ${ANDROID_TREE}/bionic
#   deapexer:    ${ANDROID_TREE}/system/apex
#   debugfs:     ${ANDROID_TREE}/external/e2fsprogs
#   fsck.erofs:  ${ANDROID_TREE}/external/erofs-utils
#
# 4. Run the script!
#
# $ ./gather.sh
...
# 1. Location of the android main-arc-dev branch tree.
...
ARC_ANDROID_CPU=('goldmont-without-sha-xsaves' 'armv8-a_cortex-a72' \
  'goldmont-without-sha-xsaves' 'armv8-a_cortex-a72')
...
TO_DIR_BASE="${TO_DIR_BASE:-"${ANDROID_TREE}/arc-toolchain-main-dir"}"
...
# 4. Path to deapexer, debugfs, and fsck.erofs binaries.
: "${DEAPEXER:="${ANDROID_TREE}/out/host/linux-x86/bin/deapexer"}"
: "${DEBUGFS:="${ANDROID_TREE}/out/host/linux-x86/bin/debugfs_static"}"
: "${FSCKEROFS:="${ANDROID_TREE}/out/host/linux-x86/bin/fsck.erofs"}"

if [[ ! -f "${DEAPEXER}" ]] || \
	[[ ! -f "${DEBUGFS}" ]] || \
	[[ ! -f "${FSCKEROFS}" ]] ; then
	echo "Please set \$DEAPEXER, \$DEBUGFS, and \$FSCKEROFS before running."
	exit 1
fi
...
for (( a = 0; a < len; ++a )); do
	...
	if [[ -f "${apex_path}" ]] &&
		...
		"${DEAPEXER}" \
			...
			--fsckerofs_path="${FSCKEROFS}" \
			extract "${apex_path}" "${apex_dir_path}"
	fi
	...
	for f in "${BINARY_FILES[@]}"; do
		...
		# prebuilds.
		mapfile -t paths < <(find "${artifacts_system_dir}/${lib}" -name "${f}" 2>/dev/null \
		  | grep -v /bootstrap/ | grep -v /arm/ | grep -v /arm64/ | grep -v /hwasan/)
		...
		0)
			...
			;;
		*)
			# If multiple files are found, check if symlinks point to
			# the same underlying file
			file=""
			for p in "${paths[@]}"; do
				if [[ -L "${p}" && ! -f "${p}" ]]; then
					# If symlink, add prefix to the path
					path="${artifacts_system_dir}"$(readlink "${p}")
				else
		...
		# Resolve symlink if not exists. Special case: ART mainline module
		# has different package name on debuggable build.
		if [ ! -f "${file}" ]; then
	...
	runcmd \
		cp -pPR \
		"${ANDROID_TREE}/bionic/libc/include"/* \
...
runcmd cp -pPr \
	"${ANDROID_TREE}/prebuilts/clang/host/linux-x86/clang-r510928/"/* \
...
TARBALL="${TO_DIR_BASE}/../arc-toolchain-main-${PACKET_VERSION}.tar.gz"
...
echo "Or you try this command: gsutil cp -a public-read ${TARBALL} gs://chromeos-localmirror/distfiles/"
