...
if [[ "${CATEGORY}" == cross-*-bootstrap ]]; then
	...
	# b/405168125: gcc fails to build with clang when cross-compiling for RISCV,
	# so build with gcc instead in that case.
...
PATCHES=(
	...
	"${FILESDIR}/0004-Fix-fixincludes.patch"
	"${FILESDIR}/0005-fix-libcpp-setlocale.patch"
...
src_prepare() {
	...
	if [[ ${CTARGET} == riscv* ]]; then
		eapply "${FILESDIR}/libcpp-enable-nls.patch"
	fi
...
src_configure() {
	...
	local confgcc=(
		...
		# No need build all three stages of gcc.
		--disable-bootstrap
	)
