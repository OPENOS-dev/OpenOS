...
LICENSE="GPL-3+"
IUSE="cet debuginfod doc cros_host hardened gprofng multitarget nls pgo +plugins static-libs test vanilla xxhash zstd"
...
PATCHES=(
	# CVE-2025-11082
	"${FILESDIR}/binutils-2.45-elf-Don-t-read-beyond-.eh_frame-section-size.patch"
	# CVE-2025-11083
	"${FILESDIR}/binutils-2.45-elf-Don-t-match-corrupt-section-header-in-linker-inp.patch"
	# CVE-2025-69649 (needed for binutils 2.45 and 2.46)
	"${FILESDIR}/binutils-PR-33697-fuzzer-segfault.patch"
	# NOTE: If upgrading to 2.46, please be sure to cherrypick upstream's
	# 81e90cf63a10ad11772c2437c8f2a88f1a00c739 for CVE-2025-69650.
	# 2.47+ should not need the cherrypick.
)
...
# ChromiumOS: Use ${CTARGET} as slot name when we're building a cross toolchain.
if is_cross ; then
	SLOT="${CTARGET}"
else
	SLOT="0"
...
"
# CrOS: bazel builds have problems with transient dependencies so we need to add perl/texinfo
# TODO(b/448074562): bazel builds require perl and texinfo unconditionally
# TODO(b/448173486): fix bison&flex bdepend divergence to use app-alternatives
BDEPEND="
	dev-lang/perl
	sys-apps/texinfo
	pgo? (
...
# ChromiumOS: Disable split debug for cross-<abi>/binutils because
...
toolchain-binutils_bugurl() {
	# ChromiumOS: Bugs against this package should be reported in
	# the ChromiumOS bug tracker instead of Gentoo's.
	printf "https://crbug.com/"
}
toolchain-binutils_pkgversion() {
	printf "ChromiumOS %s" "${PV}"
	[[ -n ${PATCH_VER} ]] && printf " p%s" "${PATCH_VER}"
}
...
src_configure() {
	...
	myconf+=(
		...
		# TODO(b/448077099): The following line causes a build regression in the amd64-generic CQ
		...
		# CrOS: We don't want the ./configure script to probe for ada support
		# since it invokes an unprefixed `gcc`
		acx_cv_cc_gcc_supports_ada=no

		# CrOS: We want to build binutils with clang. GCC_FOR_TARGET isn't used
		# by the build, but the configure script probes for it. We set it to false
		# to prevent the compiler wrappers from killing the build.
...
src_install() {
	...
	# ChromiumOS: Install wrappers.
	# TODO(b/448077101): See how much of this block is needed (CrOS-specific) or upstreamable
	{
		if use hardened ; then
