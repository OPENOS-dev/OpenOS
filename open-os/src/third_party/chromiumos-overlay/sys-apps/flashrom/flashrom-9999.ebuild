...
inherit cros-workon flag-o-matic cros-toolchain-funcs meson cros-sanitizers
...
src_configure() {
	...
	# b/404353398#comment20: eclasses like `platform.eclass` set -march=x86-64-v2
	# for `use test`. Ensure to do that here; otherwise, this ebuild might emit
	# unittests with instructions that are too new for some builders.
	if use test && [[ "${ARCH}" == amd64 ]]; then
		append-cppflags -march=x86-64-v2
	fi
