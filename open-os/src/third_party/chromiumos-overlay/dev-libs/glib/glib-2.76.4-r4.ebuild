...
inherit gnome.org gnome2-utils linux-info meson-multilib multilib python-any-r1 cros-toolchain-funcs xdg
...
multilib_src_configure() {
	...
	# b/404353398: Clang is generating gfni instructions here, which breaks tests
	# on builders. Disable that feature for amd64 builds until a longer-term fix
	# is landed.
	if [[ "${ARCH}" == amd64 ]]; then
		append-cppflags -Xclang -target-feature -Xclang -gfni
	fi
