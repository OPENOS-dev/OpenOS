...
DESCRIPTION="RTS5453-VB Firmware GOOG0L01 Binary"
...
#    ex: tar -cJf realtek-rts5453vb-GOOG0L01-firmware-${PV}.tar.xz \
#          realtek-rts5453vb-GOOG0L01-firmware-${PV}/rts5453_V${PV}.bin
...
# 4) Finally run `ebuild realtek-rts5453vb-GOOG0L01-firmware-${PV}.ebuild manifest`
src_install() {
	...
	local bf=rts5453vb_GOOG0L01.bin
	local hf=rts5453vb_GOOG0L01.hash

	echo -ne '\x'"${fw_major_ver_hex}" > "${hf}"
	echo -ne '\x'"${fw_minor_ver_hex}" >> "${hf}"
	echo -ne '\x'"${fw_patch_ver_hex}" >> "${hf}"

	if [[ -f "${P}/config_name.bin" ]]; then
