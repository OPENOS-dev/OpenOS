...
DESCRIPTION="RTS5453-VB Firmware GOOG0O02 Binary"
...
#    ex: tar -cJf realtek-rts5453vb-GOOG0O02-firmware-${PV}.tar.xz \
#          realtek-rts5453vb-GOOG0O02-firmware-${PV}/rts5453_V${PV}.bin
...
# 4) Finally run `ebuild realtek-rts5453vb-GOOG0O02-firmware-${PV}.ebuild manifest`
src_install() {
	...
	local bf=rts5453vb_GOOG0O02.bin
	local hf=rts5453vb_GOOG0O02.hash
	...
	if [[ -f "${P}/config_name.bin" && $(stat -c %s "${P}/config_name.bin") -eq CONFIG_NAME_LEN ]]; then
		...
		die "Unexpected config name length. Expect to be 8."
	fi
