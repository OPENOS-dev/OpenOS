...
CROS_WORKON_COMMIT="919819eded2a955f7437a1daca0cddca21a03407"
CROS_WORKON_TREE="af934c118a89459e3806570effe730b3c57b2915"
...
inherit meson multilib-minimal flag-o-matic cros-toolchain-funcs cros-workon arc-build cros-sanitizers python-any-r1
...
IUSE="${IUSE_VIDEO_CARDS}
	...
	cheets_user cheets_user_64 ovis rex"
...
src_prepare() {
	...
	if use video_cards_msm; then
		einfo "Disable timeline sempahore for msm for ARC-R."
		eapply "${FILESDIR}/CHROMIUM-disable-timeline-semaphore-for-QC-platforms.patch"
	fi

	if use ovis || use rex; then
		einfo "Exclude special memory types on ovis and rex."
		eapply "${FILESDIR}/CHROMIUM-venus-workaround-to-exclude-special-memory-.patch"
	fi
...
multilib_src_configure() {
	...
	if use cheets; then
		...
		# libbacktrace was replaced with libunwindstack in VNDK 34
		if (( ARC_PLATFORM_SDK_VERSION >= 34 )); then
			USE_LIBBACKTRACE="disabled"
		fi
	...
	emesonargs+=(
		...
		-Dandroid-libbacktrace="${USE_LIBBACKTRACE}"
...
multilib_src_install() {
	...
	# Required since Mesa 24.2
	dosym dri/virtio_gpu_dri.so "${ARC_VM_PREFIX}/vendor/$(get_libdir)/libgallium_dri.so"
}
...
multilib_src_install_all() {
	...
	if use vulkan; then
		...
		if (( ARC_PLATFORM_SDK_VERSION < 33 )); then
