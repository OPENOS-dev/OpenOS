...
inherit flag-o-matic java-pkg-opt-2 java-ant-2 cmake-multilib python-r1 cros-toolchain-funcs
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-4.7.0-chromeos-fix-proto-cmake.patch
)
...
src_prepare() {
	...
	if use contrib; then
		# Remove wechat_qrcode b/283136823
		rm -r "${WORKDIR}/${PN}_contrib-${PV}"/modules/wechat_qrcode || die
...
python_module_compile() {
	...
	local numpy_include_dir="${SYSROOT}$(python_get_sitedir)/numpy/core/include"
