...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-1.28.1-ignore-roi-compliant.patch
	"${FILESDIR}"/${PN}-1.28.1-add-fallback-libv4l2tracer-location.patch
	"${FILESDIR}"/${PN}-1.28.1-V4L_FALLTHROUGH.patch
	"${FILESDIR}"/${PN}-1.28.1-v4l2-compliance-fix-pix_array-control-test.patch
)
...
multilib_src_configure() {
	# ChromeOS: v4l2-tracer requires exceptions to be enabled.
	use tracer && cros_enable_cxx_exceptions
