...
CROS_WORKON_COMMIT=("6f1494a0044d455da3170a01f626bfe1ae99535b" "ea7c3cd2b0e004f52c2135cc850e684a92ca4cb1")
CROS_WORKON_TREE=("4d01f51ecc356bf9ab29468aab5ff0e4daa656e5" "bba0751dc3ddf5742989ed786f617b55efb64f36")
CROS_WORKON_PROJECT=(
...
LLVM_FOLDER="llvm-project-665984f5b3271b54d4e97797fa7f8ad598b2106a"
LLVM_ARCHIVE="${LLVM_FOLDER}.zip"

LIBCLC_FOLDER="libclc-clspv-665984f5b3271b54d4e97797fa7f8ad598b2106a"
LIBCLC_ARCHIVE="${LIBCLC_FOLDER}.zip"

SRC_URI="
gs://chromeos-localmirror/distfiles/${LLVM_ARCHIVE}
gs://chromeos-localmirror/distfiles/${LIBCLC_ARCHIVE}
"

LICENSE="Apache-2.0"
SLOT="0"
KEYWORDS="*"
IUSE="debug +perfetto clvk-dp4a"

VK_SPV_VERSION="1.3.283"
...
DEPEND="
	...
	>=dev-util/spirv-headers-${VK_SPV_VERSION}-r3
...
[[ ${PV} != "9999" ]] && PATCHES=(
	"${FILESDIR}/clvk-02-GITHUB-PR-808-timeline_semaphore.patch"
	"${FILESDIR}/clvk-03-GITHUB-PR-807-cl_khr_command_buffer.patch"
	"${FILESDIR}/clvk-90-chromeos_specific_device_properties.patch"
	"${FILESDIR}/clspv-00-REVERT-704d4a52-SPV_KHR_fma.patch"
)

src_unpack() {
	unpack "${LLVM_ARCHIVE}"
	# Remove llvm-cov to avoid interferences with post_install task looking
	# for coverage files.
	rm -rf "${WORKDIR}/${LLVM_FOLDER}/llvm/test/tools/llvm-cov"
	unpack "${LIBCLC_ARCHIVE}"
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DCLSPV_EXTERNAL_LIBCLC_DIR="${WORKDIR}"
...
src_install() {
	...
	insinto "/etc"
	if "$(usex clvk-dp4a true false)"
	then
		cp "${FILESDIR}/clvk-dp4a.conf" "${BUILD_DIR}/clvk.conf"
	else
		cp "${FILESDIR}/clvk.conf" "${BUILD_DIR}/clvk.conf"
	fi
	doins "${BUILD_DIR}/clvk.conf"
}
