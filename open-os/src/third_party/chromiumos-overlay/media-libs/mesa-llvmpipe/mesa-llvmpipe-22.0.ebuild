...
inherit multilib flag-o-matic meson cros-toolchain-funcs ${GIT_ECLASS} cros-workon

# libgcc is required for video_cards_llvmpipe.
inherit cros-gcc
...
PATCHES=(
	"${FILESDIR}"/BACKPORT-c11-threads-fix-build-on-c23.patch
	"${FILESDIR}"/UPSTREAM-util-blob-Fix-const-correctness-warning-in-blob_read.patch
	"${FILESDIR}"/UPSTREAM-intel-parse-intel_measure-environment-without-side-e.patch
	"${FILESDIR}"/BACKPORT-mesa-retain-const-qualifier-from-pointer.patch
)
...
src_prepare() {
	...
	eapply "${FILESDIR}"/UPSTREAM-vulkan-wsi-Add-dep_libudev-to-idep-dependencies.patch
