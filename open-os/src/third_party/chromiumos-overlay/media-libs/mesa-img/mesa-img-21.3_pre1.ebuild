...
PATCHES=(
	...
	"${FILESDIR}"/UPSTREAM-egl-reorder-code-in-_eglQueryDevicesEXT-add-swrast-v.patch
	"${FILESDIR}"/UPSTREAM-egl-don-t-expose-swrast-device-if-swrast-is-not-buil.patch
	"${FILESDIR}"/UPSTREAM-util-blob-Fix-const-correctness-warning-in-blob_read_string.patch
	"${FILESDIR}"/UPSTREAM-glx-fix-const-qualifier-warnings-found-with-C23-glibc-support.patch
	"${FILESDIR}"/UPSTREAM-mesa-retain-const-qualifier-from-pointer.patch
	"${FILESDIR}"/UPSTREAM-c11-threads-fix-build-on-C23.patch
	"${FILESDIR}"/CHROMIUM-dri-cast-strrchr-result-to-char-in-megadriver_stub.c.patch
)
