...
def fake_build_query_overlays(tmp_path):
    ...
    openos_overlay = portage_testables.Overlay(
        root_path=tmp_path / "src" / "third_party" / "openos-overlay",
        name="openos",
    )
    ...
    overlay_fake.add_package(
        portage_testables.Package(
            category="openos-base",
            package="openos-bsp-fake",
            version="0.0.1-r256",
            IUSE="another internal +static",
            inherit=["cros-workon", "openos-bsp"],
            keywords="*",
        )
    )
    overlay_fake.add_package(
    ...
    overlay_fake_private.add_package(
        portage_testables.Package(
            category="openos-base",
            package="openos-bsp-fake-private",
            version="0.0.1",
            IUSE="another internal +static",
            inherit=["cros-workon", "openos-bsp"],
            keywords="-* ~amd64",
