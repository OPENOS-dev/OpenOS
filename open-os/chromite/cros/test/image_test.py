...
class LinkageTest(image_test_lib.ImageTestCase):
    ...
    def TestLinkage(self) -> None:
        ...
        if self._IsPackageMerged("openos-base/openos-login"):
        ...
        if not self._IsPackageMerged(
            "openos-base/openos-chrome[pgo_generate]"
        ):
            if self._IsPackageMerged("openos-base/openos-chrome"):
...
class FactoryScriptTest(image_test_lib.ImageTestCase):
    ...
    Please contact
    https://git.example.com/openos/platform/factory/+/main/DIR_METADATA
    or
    openos-factoy-eng@example.com
    ...
    def TestFinalizeBundle_ExtractFirmwareInfo(self) -> None:
        ...
        if self._board and not portage_util.PortageqHasVersion(
            "openos-base/openos-config", self._board
        ):
            logging.info(
                "Board %s doesn't have openos-config. Skip the test.",
                self._board,
            )
            return

        fw_update = root / "usr/sbin/openos-firmwareupdate"
