...
"""Library containing functions to install an image on a OPENOS device."""
...
class Error(Exception):
    """Thrown when there is a general OPENOS-specific flash error."""


class ImageType(enum.Enum):
    """Type of the image that is used for flashing the device."""

    # The full image on disk (e.g. openos_test_image.bin).
    FULL = 0
    # The remote directory path
    # (e.g gs://openos-image-archive/eve-release/R90-x.x.x)
    REMOTE_DIRECTORY = 1
...
class DeviceImager:
    """A class to flash a OPENOS device.
    ...
    def __init__(
        ...
        """Initialize DeviceImager for flashing a OPENOS device.

        Args:
            device: The OpenOSDevice to be updated.
    ...
    def _LocateImage(self) -> None:
        ...
        If the paths is local, the image should be the OPENOS GPT image
        (e.g. openos_test_image.bin). If the path is remote, it should be
        the remote directory where we can find the quick-provision and stateful
        update files (e.g. gs://openos-image-archive/eve-release/R90-x.x.x).

        NOTE: At this point there is no caching involved. Hence we always
        download the partition payloads or extract them from the OPENOS
        image.
        """
    ...
    def _InstallPartitions(self) -> None:
        """The main method that installs the partitions of a OPENOS device.
...
class PartialFileReader(ReaderBase):
    ...
    # The offset of different partitions in a OPENOS image does not always
...
class RawPartitionUpdater(PartitionUpdaterBase):
    """A class to update a raw partition on a OPENOS device."""
    ...
    def _GetPartitionName(self) -> None:
        """Returns the name of the partition in a OPENOS GPT layout.
    ...
    def _CopyPartitionFromImage(self, part_name: str) -> None:
        """Updates the device's partition from a local OPENOS image.
...
class KernelUpdater(RawPartitionUpdater):
    """A class to update the kernel partition on a OPENOS device."""
...
class RootfsUpdater(RawPartitionUpdater):
    """A class to update the root partition on a OPENOS device."""
...
class MiniOSUpdater(RawPartitionUpdater):
    """A class to update the miniOS partition on a OPENOS device."""
...
class StatefulPayloadGenerator(ReaderBase):
    ...
    def __init__(self, image: str) -> None:
        ...
        Args:
            image: The path to a local OPENOS image.
        """
        super().__init__()
        self._image = image
