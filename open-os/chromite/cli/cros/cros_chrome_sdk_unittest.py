    ...
    FAKE_METADATA = """
{
  ...
  "metadata-version": "1",
  "bot-hostname": "build82-m2.example.com",
...
class RunThroughTest(
    ...
    def SetupCommandMock(
        ...
        # --no-shell drops gni files in //build/args/openos/. reclient configs
        # are also dropped here regardless of --no-shell or not.
        osutils.SafeMakedirs(
            os.path.join(self.chrome_root, "src", "build", "args", "openos")
        )
    ...
    def testManyBoards(self) -> None:
        ...
        for board in SDKFetcherMock.BOARDS:
            board_arg_file = os.path.join(
                self.chrome_src_dir, "build/args/openos/%s.gni" % board
            )
            self.assertExists(board_arg_file)
            board_crostoolchain_arg_file = os.path.join(
                self.chrome_src_dir,
                "build/args/openos/%s-crostoolchain.gni" % board,
            )
    ...
    def testInternalGclientSpec(self, mock_gclient_load) -> None:
        ...
        mock_gclient_load.return_value = [
            {
                "url": "https://git.example.com/chromium/src.git",
                "custom_deps": {},
                "custom_vars": {},
            }
        ]
    ...
    def testSymlinkCache(self) -> None:
        ...
        sysroot_dir = os.path.join(
            self.tempdir,
            "chrome-sdk/tarballs/sysroot_openos-base_openos-chrome.tar.xz/"
            "some-fake-hash",
        )
        ...
        sysroot_link = os.path.join(
            self.tempdir,
            "chrome-sdk/symlinks/%s+%s+sysroot_openos-base_openos-"
...
class VersionTest(gs_unittest.AbstractGSContextTest, cros_test_lib.TestCase):
    ...
    VERSION_BASE = "gs://openos-image-archive/%s-release/LATEST-%s" % (
        BOARD,
        VERSION,
    )
