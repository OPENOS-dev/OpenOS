...
class SDKFetcher:
    ...
    For the version of OPENOS specified, the class downloads and caches
    SDK components.
    """
    ...
    def __init__(
        ...
        self.version_finder = chrome_lkgmOpenOSVersionFinder(
            cache_dir,
            self.board,
    ...
    def FixCachePermissions(cls, cache_dir) -> None:
        ...
        example.com/c/3905759 added read-only directories into the sysroot that
    ...
    def GetFullVersion(self, version: str):
        """Get the full OPENOS version.

        Add the release branch and build number to a OPENOS platform version.
        This will specify where you can get the latest build for the given
        version for the current board.

        Args:
            version: A OPENOS platform number of the form XXXX.XX.XX, i.e.,
        ...
        """
        if openos_version.IsFullVersion(version):
    ...
    def _GetVersionInfo(self, version: str) -> str:
        """Get the full OPENOS version and the GS path of artifacts.

        Args:
            version: either of the followings:
              - A OPENOS platform number of the form XXXX.XX.XX, i.e.,
                12345.0.0
              - A OPENOS platform number + snapshot identifier:
        ...
        Returns:
            Full OPENOS version (eg. R123-12345.0.0)
        """
        if openos_version.IsFullVersion(version):
            # Assuming the given version is full version.
            return version

        if not openos_version.IsPlatformVersion(version):
...
class ChromeSDKCommand(command.CliCommand):
    """Set up an environment for building Chrome on OPENOS.

    Pulls down SDK components for building and testing Chrome for OPENOS,
    ...
    _BUILD_ARGS_DIR = "build/args/openos/"
    ...
    def ValidateVersion(version):
        ...
        if not openos_version.IsPlatformVersion(
            version
        ) and not openos_version.IsFullVersion(version):
    ...
    def AddParser(cls, parser) -> None:
        ...
        parser.add_argument(
            "--chroot",
            type="str_path",
            help="Path to a OPENOS chroot to use. If set, "
            "<chroot>/build/<board> will be used as the sysroot that Chrome "
            "is built against. If openos-chrome was built, the build "
    ...
    def _SetupEnvironment(self, board, sdk_ctx, options):
        ...
        if options.chroot:
            ...
            # build_packages or emerge openos-chrome has been run for
            # |board|).
        ...
        # openos-chrome ebuild are only relevant for builds done within
        ...
        # openos and android from shared source.
