...
class Shellball:
    ...
    https://example.com/docs/firmware-updater
    """
...
def _MountImagePartition(
    ...
    Args:
        image_file: A path to the image file (openos_base_image.bin).
...
def ResignImageFirmware(image_file, keyset) -> None:
    ...
    with osutils.TempDir() as rootfs_dir:
        ...
        with _MountImagePartition(image_file, "ROOT-A", rootfs_dir):
            sb_file = os.path.join(rootfs_dir, "usr/sbin/openos-firmware")
