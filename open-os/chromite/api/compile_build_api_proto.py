...
class SubdirectorySet(enum.Enum):
    ...
    def get_source_dirs(
        self, source: Path, openos_config_path: Path
    ) -> Iterable[Path]:
        """Get the directories for the given subdirectory set."""
        if self is self.ALL:
            return [
                source,
                openos_config_path / "proto" / "openos",
            ]
...
def _GenerateFiles(
    ...
    openos_config_path = _CHROMEOS_CONFIG_PATH

    with tempfile.TemporaryDirectory() as tempdir:
        if not openos_config_path.exists():
            openos_config_path = Path(tempdir) / "config"

            logging.info("Creating shallow clone of openos/config")
            git.Clone(
                openos_config_path,
                "%s/openos/config" % constants.EXTERNAL_GOB_URL,
                depth=1,
            )

        for src_dir in dir_subset.get_source_dirs(source, openos_config_path):
