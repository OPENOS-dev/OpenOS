...
def _BundleOPENOSConfigResponse(request, response, _config) -> None:
...
@faux.success(_BundleOPENOSConfigResponse)
@faux.empty_error
@validate.require("sysroot.path")
@validate.exists("result_path.path.path")
@validate.validation_complete
def BundleOPENOSConfig(
    ...
    """Output the OPENOS Config payload for a build target."""
    output_dir = request.result_path.path.path
    chroot = controller_util.ParseChroot(request.chroot)
    sysroot = controller_util.ParseSysroot(request.sysroot)

    configs = artifacts.BundleOPENOSConfig(chroot, sysroot, output_dir)

    if not configs:
        logging.warning(
            "Could not create OPENOS Config for %s.", sysroot.path
        )
        return
