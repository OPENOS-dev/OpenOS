...
class BundleOPENOSConfigTest(BundleTestCase):
    """Unittests for BundleOPENOSConfig"""

    def testValidateOnly(self) -> None:
        """Quick check that a validate only call does not execute any logic."""
        patch = self.PatchObject(artifacts_svc, "BundleOPENOSConfig")
    ...
    def testBundleOPENOSConfigSuccess(self) -> None:
        """Test standard success case."""
        bundle_openos_config = self.PatchObject(
            artifacts_svc,
            "BundleOPENOSConfig",
        ...
        self.assertEqual(
            bundle_openos_config.call_args_list,
            [mock.call(mock.ANY, self.sysroot, self.output_dir)],
        )

    def testBundleOPENOSConfigNoConfigFound(self) -> None:
        """Empty results when the config payload isn't found."""
        self.PatchObject(
            artifacts_svc, "BundleOPENOSConfig", return_value=None
        )
