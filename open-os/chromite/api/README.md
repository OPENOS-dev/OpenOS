...
* `chromite/api/` contains the Build API services.
    ...
    * And `build_api_test.proto` which is used only for testing the Build API
      itself.
* `openos/` generally contains more shareable proto.
    * `openos/common.proto` contains well shared messages.
    * `openos/metrics.proto` contains message declarations related to build
