...
"""Unit tests for the chrome_openos_lkgm program."""
...
from chromite.scripts import chrome_openos_lkgm
...
class ChromeLKGMCommitterTester(
    ...
    """Test cros_openos_lkgm.Committer."""

    def testCommitNewLKGM(self) -> None:
        """Tests that we can commit a new LKGM file."""
        committer = chrome_openos_lkgm.ChromeLKGMCommitter(
            "1001.0.0",
            "main",
            openos_version.VersionInfo("999.0.0"),
            dryrun=False,
        )
                ...
                with mock.patch.object(
                    ...
                    with mock.patch.object(
                        ...
                        ce.assert_called_once_with(
                            123456, "openos/CHROMEOS_LKGM", "1001.0.0"
                        )
                        bc.assert_called_once_with(
                            ...
                            ready=True,
                            reviewers=[
                                "chrome-os-gardeners-reviews@example.com"
                            ],
                        )
    ...
    def testOlderLKGMFails(self) -> None:
        ...
        committer = chrome_openos_lkgm.ChromeLKGMCommitter(
            "1001.0.0",
            "main",
            openos_version.VersionInfo("1002.0.0"),
            dryrun=False,
        )
        with mock.patch.object(committer._gerrit_helper, "CreateChange") as cg:
            cg.return_value = mock.MagicMock(gerrit_number=123456)
            with mock.patch.object(
                ...
                self.assertRaises(
                    chrome_openos_lkgm.LKGMNotValid, committer.UpdateLKGM
                )
    ...
    def testOlderLKGMWithPromotingFromSnapshotToRelease(self) -> None:
        ...
        committer = chrome_openos_lkgm.ChromeLKGMCommitter(
            "1001.0.0",
            "main",
            openos_version.VersionInfo("1002.0.0-12345"),
            dryrun=False,
    ...
    def testAbandonObsoleteLKGMs(self) -> None:
        ...
        cleaner = chrome_openos_lkgm.ChromeLKGMCleaner(
            "main", openos_version.VersionInfo("10002.0.0"), "USER_EMAIL"
        )
            ...
            with mock.patch.object(
                ...
                with mock.patch.object(
                    ...
                    with mock.patch.object(
                        ...
                        ce.assert_called_once_with(
                            123456, "openos/CHROMEOS_LKGM", "1003.0.0"
                        )
                        bc.assert_called_once_with(
                            123456,
                            labels={"Bot-Commit": 1, "Commit-Queue": 2},
    ...
    def testVersionWithSnapshotNumber(self) -> None:
        ...
        committer = chrome_openos_lkgm.ChromeLKGMCommitter(
            "1003.0.0-2222222",
            branch,
            openos_version.VersionInfo("1002.0.0-1111111"),
            False,
        )
            ...
            with mock.patch.object(
                ...
                with mock.patch.object(
                    ...
                    with mock.patch.object(
                        ...
                        ce.assert_called_once_with(
                            123456, "openos/CHROMEOS_LKGM", "1003.0.0-2222222"
                        )
                        bc.assert_called_once_with(
                            123456,
                            labels={"Bot-Commit": 1, "Commit-Queue": 2},
    ...
    def testCommitMsg(self) -> None:
        ...
        previous_lkgm_uprev_cls = [
            StubGerritChange(
                ...
                footers=[
                    (
                        chrome_openos_lkgm.GIT_FOOTER_EXTERNAL_MANIFEST_POS,
                        "22221",
                    ),
                ],
            )
        ...
        self.assertIn(
            "Automated Commit: LKGM 1001.0.0 for openos.", commit_msg_lines
        )
        ...
        self.assertIn(
            "Uploaded by https://example.com/b/some-build-id",
            commit_msg_lines,
        )
