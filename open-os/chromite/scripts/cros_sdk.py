...
def _ReExecuteIfNeeded(argv, opts) -> None:
    ...
    On Linux: Re-exec via sudo for chroot privileges.
    On macOS: Skip; QEMU user-mode doesn't need root.
    ...
    if platform_enter.is_macos():
        logging.debug("macOS: Skipping sudo re-exec (QEMU user-mode)")
        return
...
def CreateParser(
    ...
    group = parser.add_argument_group(
        "Internal OPENOS Build Team Options",
        "Caution: these are for meant for the OPENOS build team only",
    )
...
def main(argv) -> None:
    ...
    try:
        ...
        cros_build_lib.die(
            ...
            "Are you in a Chromium source tree instead of OPENOS?\n\n"
            "Please change to a directory inside your OPENOS source tree\n"
            "and retry.  If you need to setup a OPENOS source tree, see\n"
            "  https://example.com/chromium-os/developer-guide"
        )
    ...
    host = os.uname()[4]
    # OPENOS: Multi-platform support - allow x86_64, aarch64 (Linux),
    # arm64/arm64e (macOS Apple Silicon), amd64 (macOS Intel)
    supported_arches = {"x86_64", "aarch64", "arm64", "arm64e", "amd64"}
    if host not in supported_arches:
        cros_build_lib.die(
            "cros_sdk is currently supported on x86_64 and arm64/aarch64; "
            "you're running %s. Supported: %s"
            % (host, ", ".join(sorted(supported_arches)))
        )
    logging.info("Host architecture: %s (platform: %s)", host, sys.platform)
    ...
    # On macOS: Skip namespace unshare (kernel namespaces are Linux-only;
    # QEMU user-mode provides isolation).
    if not platform_enter.is_macos():
