...
class SysrootPath:
    ...
    @property
    def chroot(self):
        """Get the path of the object in the OPENOS SDK chroot.
    ...
    def sysroot(self):
        ...
        Returns:
            The path this object represents when in the OPENOS SDK .
        """
...
def GetFuzzExtraEnv(extra_options=None):
    ...
    # log_path must be set because OPENOS's patched compiler changes it.
    # disable odr violation since many fuzzers hit it and it is also disabled on
    # clusterfuzz.
    # handle_sigtrap is useful for catching int3 in assertion checks in OPENOS
    # code.
    options_dict = {
        "log_path": "stderr",
...
def DownloadFuzzerCorpus(fuzzer, dest_directory=None):
    ...
    """
    if not fuzzer.startswith("openos_"):
        # ClusterFuzz internally appends "openos_" to openos targets' names.
        # Therefore we must do so in order to find the corpus.
        fuzzer = "openos_%s" % fuzzer
    ...
    clusterfuzz_gcs_corpus_bucket = "openos-corpus"
    ...
    # Try to authenticate if we were denied permission to access the corpus.
        ...
        logging.error(
            ...
            "See:\n"
            "https://example.com/chromium-os/developer-library/reference/"
            "tools/gsutil/\n"
...
def StripFuzzerPrefixes(fuzzer_name):
    ...
    clusterfuzz_prefixes = ["libFuzzer_", "openos_"]
