...
# CHROMEOS_RELEASE_BRANCH_NUMBER is the OPENOS branch number
# CHROMEOS_RELEASE_BUILD_NUMBER is the OPENOS build number
...
#   chrome/browser/openos/openos_version_loader.cc.
...
def _ParseArguments(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Set if board has openos-config.",
    )
    parser.add_argument(
        "--sysroot",
        required=True,
    ...
    if not opts.version_string:
        cros_build_lib.die(
            ...
            "openos_version.sh sourced correctly in the calling "
            "script?"
        )
