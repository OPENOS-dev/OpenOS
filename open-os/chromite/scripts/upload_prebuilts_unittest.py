...
class TestPrebuilt(cros_test_lib.MockTestCase):
    ...
    def setUp(self) -> None:
        ...
        self._gs_bucket_path = "gs://openos-prebuilt/host/version"
...
class TestSyncPrebuilts(cros_test_lib.MockTestCase):
    ...
    def setUp(self) -> None:
        ...
        def mock_rev(_filename, _data, report, *_args, **_kwargs) -> None:
            report.setdefault("created_cls", []).append(
                f"https://example.com/unittest/{clnum[0]}"
            )
            clnum[0] += 1
    ...
    def _testSyncHostPrebuilts(self, chroot, out_dir) -> None:
        ...
        self.assertEqual(
            report,
            {
                "created_cls": ["https://example.com/unittest/1"],
            },
        )
        ...
        self.rev_mock.assert_has_calls(
            [
                mock.call(
                    ...
                    {
                        "created_cls": [
                            "https://example.com/unittest/1",
                            "https://example.com/unittest/2",
                        ],
                    },
                    dryrun=False,
                ),
...
class TestSdk(cros_test_lib.MockTestCase):
    ...
    def testSdkUpload(
        ...
        upload_calls = [
            mock.call(
                "%s.Manifest" % tar, "gs://openos-sdk/%s.Manifest" % vtar
            ),
            mock.call(tar, "gs://openos-sdk/%s" % vtar),
        ]
        ...
        for tc in tc_tarballs:
            ...
            upload_calls.append(
                mock.call(
                    tc[1],
                    ("gs://openos-sdk/" + tc_upload_path)
                    % {"target": tc[0]},
                )
            )
        upload_calls.append(
            mock.call(mock.ANY, "gs://openos-sdk/cros-sdk-latest.conf")
        )
