...
class Upgrader:
    ...
    def _VerifyEbuildOverlay(
        ...
        is still in another overlay (e.g. openos-overlay).  In this case,
    ...
    def _GiveMaskedError(self, upgraded_cpv, emerge_output) -> None:
        ...
        # # /.../openos/src/third_party/openos-overlay/profiles\
        # /targets/openos/package.mask:
        # >=upgraded_cp
        package_mask = None
    ...
    def _FixPythonCompat(self, ebuild_path) -> None:
        """Edit PYTHON_COMPAT to cover the python versions used by OPENOS."""
        ...
        # Set PYTHON_COMPAT to versions we care about.
        content = re.sub(python_regexp, r"\1 python3_{11..14} \2", content)
    ...
    def _FinalizeLocalPInfolist(self, orig_pinfolist):
        ...
        for pinfo in orig_pinfolist:
            # No need to report or try to upgrade openos-base packages.
            if pinfo.category == "openos-base":
                continue
    ...
    def PrepareToRun(self) -> None:
        ...
        # TODO(b/242758296) Remove this when OPENOS supports EAPI 8.
        self._DowngradeEAPI()
    ...
    def Commit(self) -> None:
        ...
        for pkg in sorted(self._upgraded_packages, key=lambda x: x.package):
            ...
            # is not a Portage overlay (e.g. openos-overlay).
