...
http://goto/openos-prebuilts

Example of uploading prebuilt amd64 host files to Google Storage:
upload_prebuilts -p /b/cbuild/build -s -u gs://openos-prebuilt

Example of uploading x86-dogfood binhosts to Google Storage:
upload_prebuilts -b x86-dogfood -p /b/cbuild/build/ -u gs://openos-prebuilt -g
"""
...
_BINHOST_BASE_URL = "gs://openos-prebuilt"
_PREBUILT_BASE_DIR = "src/third_party/openos-overlay/openos/config/"
    ...
    def SyncBoardPrebuilts(
        ...
        Args:
            ...
            git_sync: If set, update make.conf of target to reference the latest
                prebuilt packages generated here.
            sync_binhost_conf: If set, update binhost config file in
                openos-overlay for the current board.
        ...
        for target in self._GetTargets():
            ...
            if self._target == target and not self._skip_upload:
                ...
                # the --prepend-version argument (e.g. by openos-sdk bot).
...
def ParseOptions(argv) -> Tuple[argparse.Namespace, Optional[BuildTarget]]:
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Update binhost.conf in openos-overlay or "
        "openos-overlay. Commit the changes, but don't "
