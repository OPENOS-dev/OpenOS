...
class ChromeLKGMCleaner:
    ...
    def __init__(
        self,
        branch: str,
        current_lkgm: openos_version.VersionInfo,
        user_email: str,
        dryrun: bool = False,
    ...
    def ProcessObsoleteLKGMRolls(self) -> None:
        ...
        if self._buildbucket_id:
            build_link = (
                "\nUpdated by"
                f" https://example.com/b/{self._buildbucket_id}\n"
            )

        for change in open_changes:
            logging.info(
                "Found a open LKGM roll CL: %s (example.com/c/%s).",
                change.subject,
            ...
            roll_to = openos_version.VersionInfo(roll_to_string)
            ...
            if not mergeable:
                ...
                roll_from = openos_version.VersionInfo(
...
class ChromeLKGMCommitter:
    ...
    # https://example.com/chromium/chromium/src/+/main:infra/config/subprojects/chrome/try.star.
    _PRESUBMIT_BOTS = {
        "luci.chrome.try": (
            "openos-betty-chrome",
            "openos-brya-chrome",
            "openos-jacuzzi-chrome",
            "openos-reven-chrome",
            "openos-trogdor-chrome",
            "openos-volteer-chrome",
        ),
    }
    ...
    _COMMIT_MSG_HEADER = "Automated Commit: LKGM %(lkgm)s for openos."
    ...
    def __init__(
        ...
        # Strip any chrome branch from the lkgm version.
        self._lkgm = openos_version.VersionInfo(lkgm).VersionString()
    ...
    def UpdateLKGM(self) -> None:
        ...
        new_lkgm = openos_version.VersionInfo(self._lkgm)
        ...
        self._gerrit_helper.ChangeEdit(
            change.gerrit_number, "openos/CHROMEOS_LKGM", self._lkgm
        )

        if self._dryrun:
            logging.info(
                "Would have applied CQ+2 to example.com/c/%s",
                change.gerrit_number,
            )
            self._gerrit_helper.AbandonChange(
                change,
        ...
        logging.info(
            "Applying %s to example.com/c/%s", labels, change.gerrit_number
        )
    ...
    def ComposeCommitMsg(self):
        ...
        build_link = ""
        if self._buildbucket_id:
            build_link = "Uploaded by https://example.com/b/%s\n\n" % (
...
def GetCurrentLKGM(branch: str) -> openos_version.VersionInfo:
    ...
    return openos_version.VersionInfo(current_lkgm.strip())
...
def main(argv):
    ...
    if opts.force_overriding_user:
        ...
        user_email = "openos-commit-bot@example.com"
