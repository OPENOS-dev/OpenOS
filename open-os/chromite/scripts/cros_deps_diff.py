...
def UnversionedName(dep):
    """Returns the name of the package, omitting its version."""
...
def GetReverseDependencyClosure(full_name, deps_map, divergent_set):
    ...
    Walks the tree along all reverse dependency paths to find all the nodes of
    the divergent set that transitively depend on the input node.
    """
    s = set()
...
def GetVersionMap(input_deps):
    ...
    For every package, it maps the input data index to the full name
    (versioned) of the package in that input data. E.g.
...
def GetDivergentSet(version_map, count):
    ...
    dependency graphs, or are missing a version altogether.
    """
    divergent_set = set()
