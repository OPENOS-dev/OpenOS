...
def main(argv) -> None:
    ...
    if options.overlays:
        ...
        overlays.extend(
            [
                "%s/src/private-overlays/openos-overlay" % options.buildroot,
                "%s/src/third_party/openos-overlay" % options.buildroot,
            ]
        )
...
def _WorkOnCommit(
    ...
    root_version = openos_version.VersionInfo.from_repo(options.buildroot)
