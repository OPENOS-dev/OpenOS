...
def get_swarming_url(task_id: str, host_name: str) -> str:
    ...
    return f"https://example.com/swarming/task/{task_id}?server={host_name}"
...
class Test:
    ...
    def _LaunchLedJob(job: dict) -> str:
        ...
        {
          "swarming": {
            "host_name": "openos-swarming.appspot.com",
...
def SetupLaunchParser(parser) -> None:
    ...
    parser.epilog = """
Usage examples:
  1. Test how https://example.com/c/345678 affects runtime vs baseline of default
  bucket/builder with no repeats:
    cl-perf launch --cls-tested https://review.example.com/c/openos/chromite/+/345678
  2. Test how https://example.com/c/345678 affects runtime vs baseline of default
  bucket/builder with 3 builds of these CLs and 3 builds of baseline
  (`cl-perf process` will compute median diff for you later):
    cl-perf launch --cls-tested https://review.example.com/c/openos/chromite/+/345678 --repeats 3
  3. Test how https://example.com/c/345678 affects runtime vs baseline of certain
  builder and bucket:
    cl-perf launch --cls-tested https://review.example.com/c/openos/chromite/+/345678 --builder arm64-generic-cq --bucket openos/cq
  4. Test how https://example.com/c/234567 and https://example.com/c/345678 together
  affect runtime vs baseline:
    cl-perf launch --cls-tested https://review.example.com/c/openos/chromite/+/234567/,https://review.example.com/c/openos/third_party/kernel/+/345678/
  5. Test https://example.com/c/234567 vs https://example.com/c/345678:
    cl-perf launch --cls-tested https://review.example.com/c/openos/chromite/+/234567/ --cls-baseline https://review.example.com/c/openos/third_party/kernel/+/345678/
"""
