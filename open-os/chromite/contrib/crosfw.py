...
"""crosfw - OPENOS Firmware build/flash script.

Builds a firmware image for any board and writes it to the board. The image
can be pure upstream or include OPENOS components (-V). Some device
...
openos-u-boot ebuild, and runs cros_bundle_firmware to produce various
types of image, a little like the openos-bootimage ebuild.
...
--gbb-flags -force-dev-switch-on
--bmpblk ~/openos/src/third_party/u-boot/bmp.bin

For example: -a "--gbb-flags -force-dev-switch-on"

Note the standard bmpblk is at:
  ~/openos/src/third_party/openos-overlay/sys-boot/
      openos-bootimage/files/bmpblk.bin
"""
...
for b in [
    ...
    UBOARDS["openos_%s" % b] = "openos_coreboot"
...
def ParseCmdline(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Include OPENOS verified boot components",
    )
    parser.add_argument(
        "-z",
        "--size",
...
def SetupBuild(options):
    ...
    if options.verified:
        base_board = "openos_%s" % base_board
