...
"""Generate and update the firmware configs repo from a OPENOS board.

This is a developer-convenience tool to manage the directory for a board
in the openos/firmware-config repository using the data from an
existing OPENOS board.
...
from chromite.api.gen.openos import firmware_config_pb2
...
def build_config(
    ...
    chroot.run(
        [
            ...
            "--deep",
            "--newuse",
            "openos-base/openos-config",
        ]
    )


def get_configs_by_model(config: dict) -> dict:
    """Transform a openos-config JSON into per-model configs."""
    result = {}
    for model_config in config["openos"]["configs"]:
...
def get_bcs_uri(bcs_overlay: str, bcs_uri: Optional[str]) -> Optional[str]:
    ...
    gs_uri = (
        f"gs://openos-binaries/HOME/bcs-{bcs_name}/{bcs_overlay}/"
        f"openos-base/openos-firmware-{ebuild_name}/{file_name}"
    )
    return gs_uri
...
def process_model(
    ...
    # openos-binaries (BCS):
    # .../openos-firmware-brya/Anahera.14505.586.0.tbz2
