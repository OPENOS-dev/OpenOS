...
class PrepareTestCase(testing_utils.TestCase):
    ...
    def test_board_not_set_up(self) -> None:
        ...
        with self.assertRaises(FileNotFoundError):
            self.conductor.prepare(["openos-base/some-package"])
    ...
    def test_no_supported_packages(self) -> None:
        ...
        self.PatchObject(
            ...
            return_value=package_sleuth.SupportedUnsupportedPackages(
                supported=[], unsupported=["openos-base/unsupported-package"]
            ),
        )
    ...
    def test_success(self) -> None:
        ...
        _conductor.prepare(["openos-base/some-package"])
...
class GetSortedPackagesTestCase(testing_utils.TestCase):
    ...
    def test_circular_dependencies(self) -> None:
        ...
        pkg1 = self.new_package(
            package_name="pkg1",
            dependencies=[
                package.PackageDependency(name="openos-base/pkg2", types=[])
            ],
        )
        pkg2 = self.new_package(
            package_name="pkg2",
            dependencies=[
                package.PackageDependency(name="openos-base/pkg1", types=[])
            ],
        )
        ...
        pkg = self.new_package(
            dependencies=[
                package.PackageDependency(
                    name="openos-base/some-pkg", types=[]
                )
            ]
        )
