...
def _Update(arguments: UpdateArguments) -> UpdateResult:
    ...
    if not arguments.build_source:
        ...
        # openos-sdk builder finishes a successful build.
...
def _uprev_local_host_prebuilts_files(
    ...
    Args:
        binhost_gs_bucket: The bucket containing prebuilt files. This may either
            include or exclude the "gs://" prefix.
            Examples: "gs://openos-prebuilt", "openos-prebuilt".
...
def uprev_toolchain_virtuals(
    openos_overlay: Path = constants.SOURCE_ROOT
    ...
    virtuals_to_sync = [
        (
            openos_overlay / "dev-lang/rust",
            openos_overlay / "virtual/rust",
        )
    ]
...
def BuildPrebuilts(
    ...
    """Builds the binary packages that compose the OPENOS SDK.
...
def UploadPrebuiltPackages(
    ...
    cros_build_lib.run(
        [
            ...
            os.path.join(
                constants.SOURCE_ROOT,
                "src/third_party/openos-overlay/openos/binhost",
            ),
