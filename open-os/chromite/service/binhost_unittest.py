    ...
    def testAbsoluteUploadPath(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            binhost.UpdatePackageIndex(
                self.root, "gs://openos-prebuilt", "/target"
            )
...
CPV: package/prebuilt
    """
        ...
        self.assertEqual(actual.header["URI"], "gs://openos-prebuilt")
...
def test_regen_build_cache(tmp_path, run_mock, outside_sdk):
    ...
    run_mock.SetDefaultCmdResult(
        stdout="src/third_party/openos-overlay\n",
    )
    assert binhost.RegenBuildCache(chroot, constants.PUBLIC_OVERLAYS) == [
        str(
            constants.SOURCE_ROOT / "src" / "third_party" / "openos-overlay"
        ),
    ]
...
CPV: openos-base/openos-chrome-100.0.0-r1

CPV: openos-base/chrome-icu-100.0.0-r1
...
CPV: package/exclude-2
    ...
    def testCreateChromePackageIndex(self) -> None:
        ...
        self.assertEqual(
            [
                "openos-base/openos-chrome-100.0.0-r1.tbz2",
                "openos-base/chrome-icu-100.0.0-r1.tbz2",
            ],
            actual_packages,
        )
        self.assertIn(
        ...
        self.assertIn(
            "PATH: target/openos-base/openos-chrome-100.0.0-r1.tbz2",
            actual_packages_content,
        )
        self.assertIn(
            "CPV: openos-base/chrome-icu-100.0.0-r1", actual_packages_content
        )
        self.assertIn(
            "PATH: target/openos-base/chrome-icu-100.0.0-r1.tbz2",
            actual_packages_content,
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "gs://openos-dev-installer",
            f"{gs_urls_util.PUBLIC_BASE_HTTPS_URL}openos-dev-installer",
        ),
