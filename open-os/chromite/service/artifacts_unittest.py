...
class BundleOPENOSConfigTest(cros_test_lib.MockTempDirTestCase):
    """BundleOPENOSConfig tests."""
    ...
    def testBundleOPENOSConfig(self) -> None:
        """Verifies that the correct OPENOS config files are bundled."""
        # Create parent dir for OPENOS Config output.
        config_parent_dir = self.chroot.full_path("build")

        # Names of OPENOS Config files typically found in a build directory.
        config_files = (
            "config.json",
        ...
        test_config_payload = {
            "openos": {"configs": [{"identity": {"platform-name": "Samus"}}]}
        }
        with open(
        ...
        config_filenames = artifacts.BundleOPENOSConfig(
    ...
    def testNoOPENOSConfigFound(self) -> None:
        """Verifies None is returned when no OPENOS config file is found."""
        self.assertEqual(
            len(
                artifacts.BundleOPENOSConfig(
                    self.chroot, self.sysroot, self.archive_dir
                )
            ),
            0,
...
class BundleFpmcuUnittestsTest(cros_test_lib.MockTempDirTestCase):
    ...
    def testBundleFpmcuUnittests(self) -> None:
        ...
        unittest_files_root = chroot.full_path(
            "/build/%s/firmware/openos-fpmcu-unittests" % board
        )
...
class GeneratePayloadsTest(cros_test_lib.MockTempDirTestCase):
    ...
    def setUp(self) -> None:
        self.target_image = os.path.join(
            self.tempdir,
            "link/R37-5952.0.2014_06_12_2302-a1/openos_test_image.bin",
        )
    ...
    def testGenerateFullTestPayloads(self) -> None:
        ...
        cros_payload_path = os.path.join(
            self.tempdir,
            "openos_R37-5952.0.2014_06_12_2302-a1_link_full_dev.bin",
        )
    ...
    def testGenerateDeltaTestPayloads(self) -> None:
        ...
        cros_payload_path = os.path.join(
            self.tempdir,
            "openos_R37-5952.0.2014_06_12_2302-a1_R37-"
