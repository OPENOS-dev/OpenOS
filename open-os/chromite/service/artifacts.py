...
CPE_WARNINGS_FILE_TEMPLATE = "cpe-warnings-openos-%s.txt"
CPE_RESULT_FILE_TEMPLATE = "cpe-openos-%s.txt"
...
def BundleOPENOSConfig(
    chroot: "chroot_lib.Chroot",
    sysroot: "sysroot_lib.Sysroot",
    archive_dir: str,
) -> List[str]:
    """Outputs the OPENOS Config payload.
    ...
    config_paths = [
        ...
        for p in [
            chroot.full_path(
                sysroot.path, "usr/share/openos-config/yaml/config.yaml"
            ),
            chroot.full_path(
                sysroot.path,
                "build/share/openos-config-bsp-private/config_protos.zip",
            ),
        ]
        if os.path.exists(p)
    ]
