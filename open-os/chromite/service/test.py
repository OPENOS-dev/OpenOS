...
_PKG_ARTIFACTS_DIR = Path("var/lib/openos/package-artifacts")
...
def BundleHwqualTarball(
    ...
    with chroot.tempdir() as autotest_bundle_dir:
        ...
        output_tag = "openos-hwqual-%s-%s" % (board, version)
...
def _TestDeployChrome(sdk_cmd: _ChromeSDK, out_board_dir: str) -> None:
    """Test SDK deployment.
...
def _VMTestChrome(board: str, sdk_cmd: _ChromeSDK) -> None:
    """Run cros_run_test.
...
def _FindAutotestMetadataFiles(
    ...
    These files are installed during the openos-base/autotest ebuild.
    """
