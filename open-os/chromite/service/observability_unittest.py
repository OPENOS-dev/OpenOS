...
def test_parse_package_name__full_with_mmpe() -> None:
    ...
    lacros_pkg_info = package_info.parse(
        "openos-base/openos-lacros-104.0.5083.0-r1"
    )
    ...
    assert (
        lacros_identifier.package_name.atom == "openos-base/openos-lacros"
    )
    assert lacros_identifier.package_name.category == "openos-base"
    assert lacros_identifier.package_name.package_name == "openos-lacros"
...
def test_get_package_details_for_partition__rootfs(tmp_path) -> None:
    ...
    pkgs = {
        ...
        "openos-base": [
            "openos-chrome-104.0.5107.2_rc-r1",
...
def test_get_installed_package_data__bad_image_type(tmp_path, caplog) -> None:
    ...
    result = observability.get_installed_package_data(
        constants.IMAGE_TYPE_FACTORY, tmp_path / "openos_factory_image.bin"
    )
