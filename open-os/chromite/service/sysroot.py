...
from chromite.api.gen.openos import prebuilts_cloud_pb2
...
_CHROME_PACKAGES = ("openos-base/openos-chrome", "openos-base/chrome-icu")
...
BAZEL_BUILD_EVENT_JSON_FILE_PATH = "/tmp/openos_bazel_build_events.json"
...
class BuildPackagesRunConfig:
    ...
    def GetPackages(self) -> List[str]:
        ...
        if self.install_auto_test:
            packages.append("openos-base/autotest-all")
    ...
    with cpupower_helper.ModifyCpuGovernor(
        ...
        if not run_configs.bazel:
            ...
            TEMP_DUPLICATE_PACKAGES = {
                "openos-base/tast-cmd",
                "openos-base/tast-remote-tests",
                "openos-base/tast-remote-tests-cros",
                "openos-base/tast-remote-tests-crosint",
                "openos-base/tast-remote-tests-crosint_intel",
                "openos-base/tast-tests-remote-data",
...
def _GetBaseInstallPackages(
    ...
    # [ebuild r U] openos-base/tast-build-deps ... to /build/eve/ USE="..."
    # [binary U] openos-base/openos-chrome ... to /build/eve/ USE="..."
    ...
    each event looks like this:
    {
      "id": {
        "actionCompleted": {
          ...
          "label": "@@.../openos-base/chrome-icu:122.0.6226.0_rc-r1",
          "configuration": {
            "id": "..."
          }
        }
...
def _BazelBuild(
    ...
    if bazel_lite:
        ...
        # -test except openos-chrome and packages that depend on it.
    ...
    """
        ...
        exclusion_query = "union".join(
            [
                f"""
        filter(
            "//internal/packages/stage2/target/board/openos/{package}:",
...
in
    $targets except rdeps(
        ...
        # @portage//internal/(...)/openos-base/crosid:0.0.1-r209 (7729267)
