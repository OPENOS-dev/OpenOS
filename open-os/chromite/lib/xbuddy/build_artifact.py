...
FACTORY_FILE = "OPENOS-factory*.zip"
...
BASE_IMAGE_FILE = "openos_base_image.tar.xz"
TEST_IMAGE_FILE = "openos_test_image.tar.xz"
RECOVERY_IMAGE_FILE = "recovery_image.tar.xz"
SIGNED_RECOVERY_IMAGE_FILE = "openos_*recovery*[mM][pP]*.bin"
...
def _AddCrOSArtifact(tag, base, name, *fixed_args, **fixed_kwargs) -> None:
    """Add a data wrapper for OPENOS artifacts.

    Add a data wrapper that describes a OPENOS artifact's implementation to
    openos_artifact_map.
    """
    artifact = _CreateNewArtifact(tag, base, name, *fixed_args, **fixed_kwargs)
    openos_artifact_map.setdefault(tag, []).append(artifact)


_AddCrOSArtifact(
    artifact_info.FULL_PAYLOAD,
    MultiArtifact,
    r"openos_.*_full_dev.*bin(\.json)?\Z",
    is_regex_name=True,
    alt_name=[
        "openos_{build}_{board}_dev.bin",
        "openos_{build}_{board}_dev.bin.json",
    ],
)
_AddCrOSArtifact(
    artifact_info.DELTA_PAYLOAD,
    MultiArtifact,
    r"openos_.*_delta_dev.*bin(\.json)?\Z",
    is_regex_name=True,
)
_AddCrOSArtifact(
...
class OpenOSArtifactFactory(BaseArtifactFactory):
    """A factory class that generates OPENOS build artifacts from names."""

    def __init__(self, download_dir, artifacts, files, build) -> None:
        """Pass the OPENOS artifact map to the base class."""
        super().__init__(
            openos_artifact_map,
            download_dir,
            artifacts,
