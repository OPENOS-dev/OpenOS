...
def _ListOverlays(
    ...
    _AddRepo("openos", optional=True)
    path = os.path.join(
        buildroot, "src", "private-overlays", "openos-*-overlay"
    )
...
class EBuild:
    ...
    def GetAutotestSubdirsToRev(ebuild_path, srcdir) -> List[str]:
        ...
        # in the openos-overlay/eclass/autotest.eclass.
    ...
    def GetSourceInfo(self, srcroot, manifest, reject_self_repo=True):
        ...
        if self.category in ("openos-base", "brillo-base"):
            dir_ = ""
        else:
            dir_ = "third_party"
    ...
    def GetVersion(self, srcroot, manifest, default):
        ...
        the $FILESDIR (openos-version.sh).

        Raises:
            raise Error when openos-version.sh script fails to return the raw
            version number.
        """
        ...
        if not self.is_workon:
            raise EbuildFormatIncorrectError(
                self._ebuild_path_no_version,
                "Package has a openos-version.sh script but is not "
                "workon-able.",
            )

        srcdirs = self.GetSourceInfo(srcroot, manifest).srcdirs

        # The openos-version script will output a usable raw version number,
        ...
        if result.returncode or not output:
            raise Error(
                "Package %s has a openos-version.sh script but failed:\n"
                "return code = %s\nstdout = %s\nstderr = %s\n"
                % (
                    self.pkgname,
                    result.returncode,
    ...
    def RevWorkOnEBuild(
        ...
        def _CheckForChanges():
            # It is possible for openos-version.sh to have changed leading to
...
class PortageDB:
    ...
    def GetInstalledPackage(self, category, pv):
        ...
        Args:
            category: The category of the package. For example "openos-base".
...
def SplitEbuildPath(path):
    ...
    Examples:
        For /any/path/openos-base/power_manager/power_manager-9999.ebuild,
        returns ('openos-base', 'power_manager', 'power_manager-9999').
...
def GetRepositoryFromEbuildInfo(info):
    ...
    This command should return output that looks a lot like:
     ...
     CROS_WORKON_PROJECT=("openos/platform2")
    """
