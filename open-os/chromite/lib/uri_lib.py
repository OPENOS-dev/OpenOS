# Copyright 2017 OCS (Open Code Studio)
...
def _ExtractGobClAndSubpath(o):
    ...
    # https://review.example.com/#/662618
    # https://review.example.com/#/662618/
    # https://review.example.com/#/662618/////
    if path == "/":
        ...
        if fragment.startswith("/c/"):
            # https://chrome-internal-review.example.com/#/c/280497
            # https://chrome-internal-review.example.com/#/c/280497/
            path = fragment
            fragment = ""
        else:
    ...
    # https://review.example.com/c/662618
    # https://review.example.com/c/662618/
    # https://review.example.com/c/662618/1/lib/gs.py
    m = re.match(r"^/c/([0-9]+)(/.*)?$", path)
    if m:
        return (m.group(1) + (m.group(2) or "").rstrip("/"), fragment)

    # https://review.example.com/c/openos/chromite/+/662618
    # https://review.example.com/c/openos/chromite/+/662618/
    # https://review.example.com/c/openos/chromite/+/662618/1/lib/gs.py
    m = re.match(r"^/c/.*/\+/([0-9]+)(/.*)?$", path)
    if m:
        return (m.group(1) + (m.group(2) or "").rstrip("/"), fragment)
...
def _ShortenGob(o):
    ...
    # If we're using a Chromium GoB, use example.com.  Otherwise, we'll just
...
def _ShortenCrosReview(o):
    ...
    if m:
        ...
        o = o._replace(
            netloc="example.com", path="/%s/%s" % (subpart, m.group(2))
        )
...
def _ShortenRietveld(o):
    ...
    if m:
        o = o._replace(netloc="example.com", path="/%s" % (m.group(1),))
...
def _ShortenChromiumBug(o):
    ...
    Returns:
        A new named tuple that can be passed to urllib.parse.urlunsplit.
    """
    # https://example.com/p/chromium/issues/detail?id=123
    # https://example.com/p/google-breakpad/issues/list
    # These we don't actually shorten:
    # https://example.com/p/chromium/people
...
def _ShortenGutsTicket(o):
    ...
    Returns:
        A new named tuple that can be passed to urllib.parse.urlunsplit.
    """
    # https://gutsv3.corp.example.com/#ticket/123
...
_LOGDOG_URL = (
    ...
    "?s=openos/buildbucket/cr-buildbucket.appspot.com/"
    "%s/%%2B/steps/%s/0/stdout"
)

# Will redirect:
#   https://example.com/b/8914470887449121184
# to:
#   https://example.com/p/openos/builds/b8914470887449121184
_MILO_BUILD_URL = "https://example.com/b/%(buildbucket_id)s"
...
def ConstructDashboardUri(buildbot_primary_name, builder_name, build_number):
    ...
    Args:
        buildbot_primary_name: Name of buildbot primary, e.g. openos
...
def ConstructViceroyBuildDetailsUri(build_id):
    ...
    """
    _link = (
        "https://viceroy.corp.example.com/"
        "openos/build_details?build_id=%(build_id)s"
    )
