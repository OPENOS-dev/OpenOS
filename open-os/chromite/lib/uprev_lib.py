...
_CHROME_GIT_HOST = "git.example.com"
...
def get_version_from_refs(refs: List[GitRef]) -> Tuple[str, str]:
    ...
    def _version_file_version(commit: str) -> str:
        ...
        # We need other ways on OPENOS CQ to ensure a Chrome uprev won't be
...
class UprevChromeManager:
    ...
    def _mark_as_stable(
        ...
        Args:
            ...
            package_dir: Path to the openos-chrome package dir.
...
class UprevOverlayManager:
    ...
    def __init__(
        ...
        # branches.
        root_version = openos_version.VersionInfo.from_repo(self._source_root)
        no_self_repos_version = openos_version.VersionInfo("13099.0.0")
    ...
    def _populate_overlay_ebuilds(
        ...
        Args:
            ...
            force: Boolean indicating whether to consider deny-listed ebuilds.
        """
        # See example.com/c/1257944 for origins of this.
        root_version = openos_version.VersionInfo.from_repo(self._source_root)
        subdir_removal = openos_version.VersionInfo("10363.0.0")
...
def get_version_with_snapshot_from_manifest(rev: str) -> str:
    """Retrieves the OPENOS version string with snapshot ID from the manifest.

    Args:
        rev: The revision of the manifest to inspect.

    Returns:
        A string combining the OPENOS version and the snapshot identifier.
    """
    ...
    version_file = git.GetObjectAtRev(
        _CHROME_OVERLAY_PATH, "openos/config/openos_version.sh", overlay
    )
    version = openos_version.VersionInfo(
...
def validate_lkgm_builds_succeeded(
    ...
    def _get_build_url(build: build_pb2.Build) -> str:
        ...
        base_url = (
            f"https://example.com/p/{project}/builders/"
            f"{bucket}/{builder}"
        )
        if build.number:
    ...
    for board in boards:
        ...
        builder_id = builder_common_pb2.BuilderID(
            project="openos", bucket="postsubmit", builder=builder_name
        )
        ...
        buildset = (
            ...
            f"openos/manifest-internal/+/{rev}"
        )
