...
class PaygenPayload:
    ...
    TEST_IMAGE_NAME = "openos_test_image.bin"
    RECOVERY_IMAGE_NAME = "recovery_image.bin"
    BASE_IMAGE_NAME = "openos_base_image.bin"
    ...
    def _PreparePartitions(self, part_a: bool = True) -> str:
        ...
        if tgt_image_type == partition_lib.DLC_IMAGE:
            ...
            logging.info("Detected a OPENOS image.")
            if self.payload.minios:
...
def GenerateUpdatePayload(
    ...
    if path_util.DetermineCheckout().type != path_util.CheckoutType.REPO:
        raise Error("Need a openos checkout to generate payloads.")
