...
This includes definitions for various build flags:
    ...
    Example file paths:
        gs://openos-releases/blah-channel/board-name/1.2.3/payloads/LOCK_flag
"""
...
class Build(utils.RestrictedAttrDict):
    ...
    Fields:
        ...
        bucket: The bucket of the image. "openos-releases" as default.
...
class UnsignedImageArchive(utils.RestrictedAttrDict):
    """Define a unsigned OPENOS image archive.
...
class ChromeosReleases:
    """Name space class for static methods for URIs in openos-releases."""

    BUCKET = "openos-releases"

    TEST_BUCKET = "openos-releases-test"
    ...
    def BuildUri(build: Build) -> str:
        ...
        Returns:
            The url for the specified build artifacts. Should be of the form:
                gs://openos-releases/blah-channel/board-name/1.2.3
        """
        return "gs://%s/%s/%s/%s" % (
            build.bucket,
            build.channel,
    ...
    def BuildPayloadsUri(build: Build) -> str:
        ...
        Returns:
            ...
            gs://openos-releases/blah-channel/board-name/1.2.3/payloads
        """
    ...
    def BuildPayloadsSigningUri(build: Build) -> str:
        ...
        Returns:
            The url for the specified build's payloads. Should be of the form:
                gs://openos-releases/some-channel/board-name/1.2.3/
                    payloads/signing
        """
    ...
    def ImageName(
        ...
        Returns:
            ...
            openos_1.2.3_board-name_recovery_blah-channel_key.bin
        """

        template = (
            "openos_%(version)s_%(board)s_%(image_type)s"
    ...
    def UnsignedImageArchiveName(
        ...
        Returns:
            The name of the specified image archive. Should be of the form:
                OPENOS-type-R19-1.2.3-board-name.tar.xz
        """

        template = (
            "OPENOS-%(image_type)s-%(milestone)s-%(version)s-%(board)s.tar.xz"
        )
    ...
    def DLCImageUri(
        ...
        Returns:
            The url for the specified dlc's image. Should be of the form:
                gs://openos-releases/beta-channel/fizz/13505.33.0/
    ...
    def _ParseImageUriValues(
        ...
        exp = (
            ...
            r"(?P<version>.*)/openos_(?P<image_version>[^_]+)_"
    ...
    def ParseUnsignedImageUri(
        ...
        exp = (
            ...
            r"(?P<board>[^/]+)/(?P<version>[^/]+)/"
            r"OPENOS-(?P<image_type>%s)-(?P<milestone>R[0-9]+)-"
    ...
    def PayloadName(
        ...
        Returns:
            The name for the specified build's payloads. Should be of the form:
                ...
                openos_0.12.433.257-2913.377.0_x86-alex_stable-channel_
                    delta_mp-v3.bin-b334762d0f6b80f471069153bbe8b97a.signed

                openos_2913.377.0_x86-alex_stable-channel_full_mp-v3.
        ...
        if src_version:
            template = (
                "openos_%(src_version)s-%(version)s_%(board)s_%(channel)s_"
            ...
            template = (
                "openos_%(version)s_%(board)s_%(channel)s_"
    ...
    def MiniOSPayloadUri(
        ...
        Returns:
            ...
            gs://openos-releases/stable-channel/x86-alex/2913.377.0/payloads/
                minios/openos_0.12.433.257-2913.377.0_x86-alex_stable-channel_
                delta_mp-v3.bin-b334762d0f6b80f471069153bbe8b97a.signed

            gs://openos-releases/stable-channel/x86-alex/2913.377.0/payloads/
                minios/openos_2913.377.0_x86-alex_stable-channel_full_mp-v3.
                bin-610c97c30fae8561bde01a6116d65cb9.signed
        """
    ...
    def ParsePayloadUri(cls, payload_uri: str) -> Mapping[Any, object] | None:
        ...
        # Sample Delta URI:
        #   gs://openos-releases/stable-channel/x86-mario/4731.72.0/payloads/
        #   openos_4537.147.0-4731.72.0_x86-mario_stable-channel_delta_mp-v3
        #   .bin-3a90d8666d1d42b7a7367660b897e8c9.signed

        # Sample Full URI:
        # gs://openos-releases/stable-channel/x86-mario/4731.72.0/payloads/
        #   openos_4731.72.0_x86-mario_stable-channel_full_mp-v3.bin-
        #   969f24ba8cbf2096ebe3c57d5f0253b7.signed

        # Handle FULL payload URIs.
        full_exp = (
            r"^gs://(?P<bucket>.*)/(?P<channel>.*)/(?P<board>.*)/"
            r"(?P<version>.*)/payloads/openos_(?P<image_version>[^_]+)_"
        ...
        delta_exp = (
            r"^gs://(?P<bucket>.*)/(?P<channel>.*)/(?P<board>.*)/"
            r"(?P<version>.*)/payloads/openos_(?P<src_version>[^_]+)-"
...
class ChromeosImageArchive:
    """Namespace class for static methods for URIs in openos-image-archive."""

    BUCKET = "openos-image-archive"
    ...
    def BuildUri(
        ...
        Returns:
            The url for the specified build artifacts. Should be of the form:
                gs://openos-image-archive/board-release/R23-4.5.6
        """
