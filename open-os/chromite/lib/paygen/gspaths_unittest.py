...
class GsPathsChromeosReleasesTest(cros_test_lib.TestCase):
    ...
    # Standard OPENOS releases names.
    _CHROMEOS_RELEASES_BUCKET = "openos-releases"
    ...
    _IMAGE_NAME_TEMPLATE = (
        "openos_%(image_version)s_%(board)s_%(signed_image_type)s_"
        "%(image_channel)s_%(key)s.bin"
    )
    _UNSIGNED_IMAGE_ARCHIVE_NAME_TEMPLATE = (
        "OPENOS-%(unsigned_image_type)s-%(milestone)s-%(image_version)s-"
        "%(board)s.tar.xz"
    )
    _FULL_PAYLOAD_NAME_TEMPLATE = (
        "openos_%(image_version)s_%(board)s_%(image_channel)s_full_%(key)s"
        ".bin-%(random_str)s.signed"
    )
    _DELTA_PAYLOAD_NAME_TEMPLATE = (
        "openos_%(src_version)s-%(image_version)s_%(board)s_"
    ...
    _UNSIGNED_FULL_PAYLOAD_NAME_TEMPLATE = (
        "openos_%(image_version)s_%(board)s_%(image_channel)s_full_"
    ...
    def testParseImageUri(self) -> None:
        ...
        signer_output = (
            "gs://openos-releases/dev-channel/link/4537.7.0/"
            "openos_4537.7.1_link_recovery_nplusone-channel_"
    ...
    def testParseDLCImageUri(self) -> None:
        image_uri = (
            "gs://openos-releases/foo-channel/board-name/1.2.3/dlc/"
            "%s/%s/%s"
        ) % (
            self.dlc_id,
            self.dlc_package,
    ...
    def testDLCImageString(self) -> None:
        ...
        image_uri = (
            "gs://openos-releases/foo-channel/board-name/1.2.3/dlc/" "%s"
        ) % (dlc_image_str)
    ...
    def testBuildValuesFromUri(self) -> None:
        ...
        uri = (
            "gs://openos-releases/dev-channel/link/4537.7.0/"
            "openos_4537.7.1_link_recovery_nplusone-channel_mp-v4.bin"
        )
