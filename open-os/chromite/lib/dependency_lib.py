...
def get_source_path_mapping(
    ...
    Returns:
        Map from each package to the source path (relative to the repo checkout
            root, i.e: ~/openos/ in your cros_sdk) it depends on.
    ...
    if include_overlay:
        ...
        # openos-overlay specifies default settings for every target in
        # openos/config  and so can potentially affect every board.
        # TODO(b/236161656): Fix.
        # pylint: disable-next=consider-using-dict-items
        for package in results:
            # TODO(b/236161656): Fix.
