...
"""Utilities for reading and manipulating openos_version.sh script."""
...
class VersionInfo:
    """Class to encapsulate the OPENOS version info scheme.

    You can instantiate this class in three ways.
    1) using a version file, specifically openos_version.sh,
    ...
    def _LoadFrom(self, version_fh: Iterable[str]) -> None:
        ...
        for line in version_fh:
            ...
            match = self.FindValue("OPENOS_PATCH", line)
            if match:
                self.patch_number = match
                # For developer builds, append a date and time string.
                if os.environ.get("OPENOS_OFFICIAL") != "1":
    ...
    def UpdateVersionFile(self, message, dry_run, push_to=None) -> None:
        ...
        components = (
            ("OPENOS_BUILD", self.build_number),
            ("OPENOS_BRANCH", self.branch_build_number),
            ("OPENOS_PATCH", self.patch_number),
    ...
    def VersionComponents(self):
        ...
        # Released OPENOS (version number without snapshot suffix) is older than
        # snapshot OPENOS with the same version number (version number with a
        # snapshot suffix), because OPENOS release is done just after incrementing
