...
CPV: openos-base/shill-0.0.1-r1

CPV: openos-base/test-0.0.1-r1
DEBUG_SYMBOLS: yes
"""
...
"""
    ...
    def testFetchRealPackages(self) -> None:
        ...
        uri = "gs://openos-prebuilt/board/lumpy/paladin-R37-5905.0.0-rc2/packages"
...
class DebugSymbolsTest(cros_test_lib.TempDirTestCase):
    ...
    def testDebugSymbolsDetected(self) -> None:
        ...
        osutils.WriteFile(
            os.path.join(
                self.tempdir, "openos-base/shill-0.0.1-r1.debug.tbz2"
            ),
            "hello",
            makedirs=True,
        )
        ...
        self.assertEqual(
            index.packages[0]["CPV"], "openos-base/shill-0.0.1-r1"
        )
...
class UpdateAndSubmitKeyValueFileTest(cros_test_lib.MockTestCase):
    ...
    def testWithReport(self):
        ...
        report = {}
        git_file = "openos-overlay/openos/binhost/host/sdk_version.conf"
        abs_file = Path(git_file).absolute()
        data = {
    ...
    def testWithExistingReport(self):
        ...
        git_file = "openos/config/make.conf.amd64-host"
        abs_file = Path(git_file).absolute()
        data = {
