...
class ShortenUriTests(cros_test_lib.TestCase):
    ...
    def testGobUris(self) -> None:
        """Check GoB URIs using example.com"""
        HOST_MAPS = (
            ("chromium", "c"),
            ("chrome-internal", "i"),
            ("android", None),
        ...
        TESTS = (
            ...
            ("c/662618", "662618"),
            ("c/662618/", "662618"),
            ("c/openos/chromite/+/662618", "662618"),
            ("c/openos/chromite/+/662618/", "662618"),
            # Paths with subdirs.
            ("c/662618/1/lib/gs.py", "662618/1/lib/gs.py"),
            (
                "c/openos/chromite/+/662618/1/lib/gs.py",
                "662618/1/lib/gs.py",
            ),
        )
        ...
        for gob_name, crrev_part in HOST_MAPS:
            for input_part, exp_part in TESTS:
                input_uri = "https://%s-review.example.com/%s" % (
                    gob_name,
                    input_part,
                )
                if crrev_part is None:
                    exp_uri = "https://%s-review.example.com/%s" % (
                        gob_name,
                        exp_part,
                    )
                else:
                    exp_uri = "https://example.com/%s/%s" % (crrev_part, exp_part)
                self.assertEqual(exp_uri, uri_lib.ShortenUri(input_uri))

    def testCrosReview(self) -> None:
        """Check old review URIs"""
        TESTS = (
            ("https://example.com/c/123", "https://crosreview.com/123"),
            ("https://example.com/i/123", "https://crosreview.com/i/123"),
        )
    ...
    def testBuganizerUris(self) -> None:
        ...
        TESTS = (
            "b.corp.example.com/issue?id=123",
            "https://b.corp.example.com/issue?id=123",
            "http://b.corp.example.com/issue?id=123",
            "b.corp.example.com/issue?id=123",
            "b.corp.example.com/issues/123",
        )
        for test in TESTS:
    ...
    def testChromiumUris(self) -> None:
        ...
        TESTS = (
            (
                "https://crbug.com",
                "https://example.com/p/chromium/issues",
            ),
            (
                "https://crbug.com",
                "https://example.com/p/chromium/issues/list",
            ),
            (
                "https://crbug.com/123",
            ...
            (
                ...
                "https://example.com/p/google-breakpad/issues/"
                "detail?id=123",
            ),
        )
        for exp, uri in TESTS:
    ...
    def testGutsUris(self) -> None:
        """Check GUTS URIs"""
        TESTS = (
            ("http://t/123", "https://gutsv3.corp.example.com/#ticket/123"),
            (
                "http://t/#search/default/My%20Requests/0",
                "https://gutsv3.corp.example.com/#search/default/"
                "My%20Requests/0",
            ),
        )
        for exp, uri in TESTS:
...
class ConstructUrlTests(cros_test_lib.TestCase):
    ...
    def testConstructMiloBuildURL(self) -> None:
        ...
        expected = "https://example.com/b/bbid"
    ...
    def testConstructViceroyBuildDetailsUri(self) -> None:
        ...
        expected = (
            "https://viceroy.corp.example.com/openos/build_details?"
            "build_id=123"
        )
        self.assertEqual(actual, expected)
