...
_COPY_PATHS_COMMON = (
    ...
    # deleted in the openos-chrome ebuild. But we can not delete this line
...
def GetChromeRuntimeDeps(build_dir, build_target):
    ...
    # Runtime deps files are generated for test executables of OPENOS build
    # inside SDK env in testing/test.gni.
    #   https://example.com/chromium/src/testing/test.gni?rcl=9b95cd58&l=336
...
def GetChromeTestCopyPaths(build_dir, test_target):
    ...
    # build/openos/test_runner.py in chromium code.
    _IGNORELIST = [
        re.compile(r".*build/android.*"),
        re.compile(r".*build/openos.*"),
