...
BENCHMARK_AFDO_GS_URL = (
    "gs://openos-toolchain-artifacts/afdo/unvetted/benchmark"
)
CWP_AFDO_GS_URL = "gs://openos-prebuilt/afdo-job/cwp/chrome/"
KERNEL_PROFILE_URL = "gs://openos-prebuilt/afdo-job/cwp/kernel/{arch}"
KERNEL_PROFILE_VETTED_URL = (
    "gs://openos-prebuilt/afdo-job/vetted/kernel/{arch}"
)
RELEASE_PROFILE_VETTED_URL = "gs://openos-prebuilt/afdo-job/vetted/release"
...
MERGED_AFDO_NAME = "openos-chrome-{arch}-{name}"
...
AFDO_ALERT_RECIPIENTS = ["openos-toolchain-oncall1@example.com"]

# Full path to the openos-overlay directory.
...
class ChromeVersion:
    ...
    def parse(cls, s: str) -> "ChromeVersion":
        info = package_info.parse(f"openos-base/openos-chrome-{s}")
        return cls(info)
...
CHROME_BINARY_PATH = (
    "/var/cache/openos-chrome/chrome-src-internal/"
    "src/out_{board}/Release/chrome"
)

# cros-artifacts go here in the chroot.
_PACKAGE_ARTIFACTS_PATH = "var/lib/openos/package-artifacts"
...
def _ExtractChromeVersionFromDebugFileName(
    ...
    "130.0.6700.0_rc-r1"
    """
    r = re.compile(r"openos-chrome-[^-]+-([^-]+-r\d+)\..+")
...
def _ParseBenchmarkProfileName(profile_name):
    ...
    Examples:
        with input: profile_name='openos-chrome-amd64-77.0.3849.0_rc-r1.afdo'
...
def _ParseMergedProfileName(
    ...
    Examples:
        With input: profile_name='openos-chrome-amd64
...
def _WarnDetectiveAboutKernelProfileExpiration(
    ...
    alert_msg = (
        ...
        "https://buganizer.corp.example.com/issues?q=componentid:87200"
        "%20%22AutoFDO%20profile%20generation%20for%20kernel%22 or contact "
        "the cwp-team@example.com."
    )
    alerts.SendEmailLog(
        subject_msg,
...
class _CommonPrepareBundle:
    ...
    def _GetEbuildInfo(
        ...
        """Get the ebuild info for a category/package in openos-overlay.

        Args:
            package: package name (e.g. openos-chrome or openos-kernel-4_4)
            category: category (e.g. openos-base, or sys-kernel)
    ...
    def _GetArtifactVersionInGob(self, profile_arch: str) -> str:
        ...
        if version.endswith("_rc"):
            version = version[:-3]
        profile_path = (
            f"chromium/src/+/refs/tags/{version}/openos/profiles/"
    ...
    def _GetArtifactVersionInEbuild(self, package, variable):
        ...
        Args:
            package: name of the package (such as, 'openos-chrome')
    ...
    def _FindLatestAFDOArtifact(
        ...
        def _FilesOnBranch(
            ...
            Legacy PFQ results look like: latest-openos-chrome-amd64-79.afdo.
            The branch should appear in the name either as:
            - R78-12371.22-1566207135 for kernel/CWP profiles, OR
            - openos-chrome-amd64-78.0.3877.0 for benchmark profiles
            ...
            bench_afdo_pattern = re.compile(rf"openos-chrome-{arch}-{branch}")
    ...
    def _CreateAndUploadMergedAFDOProfile(
        ...
        try:
            benchmark_listing = self.gs_context.List(
                os.path.join(
                    benchmark_url,
                    f"openos-chrome-{self.arch}-*" + profile_suffix,
                ),
                details=True,
            )
...
class PrepareForBuildHandler(_CommonPrepareBundle):
    ...
    def _PrepareVerifiedKernelCwpAfdoFile(self):
        ...
        # For example: openos-kernel-4_4-4.4.214-r2087.ebuild.
        kernel_package_version = kernel_version.replace(".", "_")
        kernel_package_name = f"openos-kernel-{kernel_package_version}"
...
class BundleArtifactHandler(_CommonPrepareBundle):
    ...
    def _LocateChromeDebugInfo(afdo_tmp_path: Path) -> Path:
        ...
        """
        debug_glob = "openos-chrome*.debug"
    ...
    def _BundleVerifiedKernelCwpAfdoFile(self):
        ...
        profile_name = self._GetArtifactVersionInEbuild(
            f"openos-kernel-{kernel_version}", profile_var_name
        )
        if not profile_name:
            raise BundleArtifactsHandlerError(
                "Could not find AFDO_PROFILE_VERSION in "
                f"openos-kernel-{kernel_version}."
            )
        profile_name += KERNEL_AFDO_COMPRESSION_SUFFIX
        # The verified profile is in the sysroot with a name similar to:
        # /usr/lib/debug/boot/openos-kernel-4_4-R82-12874.0-1581935639.afdo.xz
        profile_path = self.chroot.full_path(
            ...
            "debug",
            "boot",
            f"openos-kernel-{kernel_version}-{profile_name}",
        )
    ...
    def _BundleClangCrashDiagnoses(self):
        ...
        See example.com/p/chromium/issues/detail?id=1056904 for context.
        """
...
class GetUpdatedFilesHandler:
    ...
    def _UpdateKernelMetadata(kernel_version: str, profile_version: str):
        ...
        kernel_name = f"openos-kernel-{kernel_version}"
        assert (
