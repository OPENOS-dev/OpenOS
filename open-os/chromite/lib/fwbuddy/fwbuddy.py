...
FIELD_DOCS = {
    "board": FieldDoc(
        description=(
            "A group of OPENOS devices (models) that have similar hardware, "
            "but may vary in minor ways (e.g. screen size). OPENOS system "
    ...
    "firmware-name": FieldDoc(
        description=(
            ...
            "`openos-firmwareupdate --manifest` on it and looking for the "
...
UNSIGNED_ARCHIVE_BUCKET = "gs://openos-image-archive"
...
# with data from DLM
BRANCH_MAP_URI = "gs://openos-build-release-console/firmware_quals.json"
