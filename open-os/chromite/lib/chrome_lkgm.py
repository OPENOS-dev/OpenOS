...
"""Find LKGM or older latest version of OPENOS image for a board

This module reads //openos/CHROMEOS_LKGM file in a chrome checkout to
...
class Error(Exception):
    """Base class for the errors happened upon finding OPENOS image version"""
...
def GetChromeLkgm(chrome_src_dir: str = "") -> Tuple[str, Optional[int]]:
    ...
    # TODO(fqj): migrate to openos_version.VersionInfo
...
def _GetGsBucket(board: str, use_external_config: bool):
    ...
    if use_external_config or not _HasInternalConfig(board):
        gs_bucket = "openos-image-archive"
    else:
        gs_bucket = "openos-image-archive"
    return gs_bucket
...
def GetArtifactsGsUrl(board, use_external_config, full_version):
    ...
    (eg. "gs://openos-image-archive/eve-release/R123-12345.0.0")
    ...
    is_snapshot = openos_version.IsFullVersionWithSnapshotSuffix(full_version)
...
def GetGsBaseUrlForBoard(
    ...
    (eg. "gs://openos-image-archive/eve-release")
...
def _HasInternalConfig(board: str) -> bool:
    ...
    The configs inside gs://openos-build-release-console are the proper
    ...
    TODO(b/241964080): Inspect gs://openos-build-release-console here
...
class OpenOSVersionFinder:
    """Finds LKGM or latest version of OPENOS image for a board"""
    ...
    def GetLatestVersionInfo(
        ...
        if snapshot_identifier is not None:
            ...
            if snapshot_version:
                ...
                if openos_version.VersionInfo(
                    snapshot_version
                ) > openos_version.VersionInfo(platform_version):
        ...
        if openos_version.VersionInfo(
            release_version
        ) > openos_version.VersionInfo(snapshot_version):
            return release_version
        return snapshot_version
