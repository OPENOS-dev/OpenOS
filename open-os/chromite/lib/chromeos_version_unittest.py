...
"""Test the openos_version module."""
...
class VersionInfoTest(cros_test_lib.MockTempDirTestCase):
    ...
    def WriteFakeVersionFile(
        ...
        info = openos_version.VersionInfo(version, chrome_branch)
    ...
    def testLoadFromFile(self) -> None:
        ...
        info = openos_version.VersionInfo(version_file=version_file)
    ...
    def testSuffixes(self) -> None:
        ...
        info = openos_version.VersionInfo(
        ...
        info = openos_version.VersionInfo(version_string=VERSION_WITH_BRANCH)
    ...
    def testLoadFromRepo(self) -> None:
        ...
        # Test for Dev/Local Builds.
        info = openos_version.VersionInfo.from_repo(self.tempdir)
        self.assertEqual(info.VersionString(), FAKE_VERSION_STRING)
        self.assertEqual(
            info.VersionStringWithDateTime(), FAKE_DEV_VERSION_STRING
        )
    ...
    def testLoadFromString(self) -> None:
        ...
        info = openos_version.VersionInfo(FAKE_VERSION_STRING, CHROME_BRANCH)
        self.assertEqual(info.VersionString(), FAKE_VERSION_STRING)
        self.assertEqual(info.VersionStringWithDateTime(), FAKE_VERSION_STRING)
    ...
    def CommonTestIncrementVersion(
        ...
        push_mock = self.PatchObject(
            openos_version.VersionInfo, "_PushGitChanges"
        )
        ...
        create_mock.assert_called_once_with(
            self.tempdir, openos_version._PUSH_BRANCH
        )
    ...
    def testIncrementVersionPatch(self) -> None:
        ...
        new_info = openos_version.VersionInfo(
    ...
    def testIncrementVersionChrome(self) -> None:
        ...
        new_info = openos_version.VersionInfo(version_file=version_file)
        self.assertEqual(new_info.VersionString(), "2.0.0")
        self.assertEqual(new_info.chrome_branch, "30")

    def testCompareEqual(self) -> None:
        """Verify comparisons of equal versions."""
        lhs = openos_version.VersionInfo(version_string="1.2.3")
        rhs = openos_version.VersionInfo(version_string="1.2.3")
        ...
        lhs = openos_version.VersionInfo(version_string="1.2.3-12345")
        rhs = openos_version.VersionInfo(version_string="1.2.3-12345")
        self.assertFalse(lhs < rhs)
        self.assertTrue(lhs <= rhs)
        self.assertTrue(lhs == rhs)
        self.assertFalse(lhs != rhs)
    ...
    def testCompareLess(self) -> None:
        ...
        lhs = openos_version.VersionInfo(version_string="1.0.3")
        rhs = openos_version.VersionInfo(version_string="1.2.3")
        self.assertTrue(lhs < rhs)
        self.assertTrue(lhs <= rhs)
        self.assertFalse(lhs == rhs)
        ...
        lhs = openos_version.VersionInfo(version_string="1.2.3-12344")
        rhs = openos_version.VersionInfo(version_string="1.2.3-12345")
        self.assertTrue(lhs < rhs)
        self.assertTrue(lhs <= rhs)
        self.assertFalse(lhs == rhs)
    ...
    def testCompareGreater(self) -> None:
        ...
        lhs = openos_version.VersionInfo(version_string="1.2.4")
        rhs = openos_version.VersionInfo(version_string="1.2.3")
        self.assertFalse(lhs < rhs)
        self.assertFalse(lhs <= rhs)
        self.assertFalse(lhs == rhs)
        ...
        lhs = openos_version.VersionInfo(version_string="1.2.3-12346")
        rhs = openos_version.VersionInfo(version_string="1.2.3-12345")
        self.assertFalse(lhs < rhs)
        self.assertFalse(lhs <= rhs)
        self.assertFalse(lhs == rhs)
...
class VersionCheckMethodsTest(cros_test_lib.TestCase):
    ...
    def testIsPlatformVersionn(self) -> None:
        ...
        self.assertTrue(openos_version.IsPlatformVersion(self.VERSION))
        self.assertTrue(
            openos_version.IsPlatformVersion(self.VERSION_WITH_SNAPSHOT)
        )
        self.assertFalse(openos_version.IsPlatformVersion(self.FULL_VERSION))
        self.assertFalse(
            openos_version.IsPlatformVersion(self.FULL_VERSION_WITH_SNAPSHOT)
        )
        self.assertFalse(
            openos_version.IsPlatformVersion(self.WRONG_FORMAT_VERSION1)
        )
        self.assertFalse(
            openos_version.IsPlatformVersion(self.WRONG_FORMAT_VERSION2)
        )
        self.assertFalse(
            openos_version.IsPlatformVersion(self.WRONG_FORMAT_VERSION3)
        )
        self.assertFalse(
            openos_version.IsPlatformVersion(self.WRONG_FORMAT_VERSION4)
        )

    def testIsFullVersion(self) -> None:
        """Tests IsFullVersion method."""
        self.assertFalse(openos_version.IsFullVersion(self.VERSION))
        self.assertFalse(
            openos_version.IsFullVersion(self.VERSION_WITH_SNAPSHOT)
        )
        self.assertTrue(openos_version.IsFullVersion(self.FULL_VERSION))
        self.assertTrue(
            openos_version.IsFullVersion(self.FULL_VERSION_WITH_SNAPSHOT)
        )
        self.assertFalse(
            openos_version.IsFullVersion(self.WRONG_FORMAT_VERSION1)
        )
        self.assertFalse(
            openos_version.IsFullVersion(self.WRONG_FORMAT_VERSION2)
        )
        self.assertFalse(
            openos_version.IsFullVersion(self.WRONG_FORMAT_VERSION3)
        )
        self.assertFalse(
            openos_version.IsFullVersion(self.WRONG_FORMAT_VERSION4)
        )

    def testIsFullVersionWithSnapshotSuffix(self) -> None:
        """Tests IsSnapshotFullVersion method."""
        self.assertFalse(
            openos_version.IsFullVersionWithSnapshotSuffix(self.VERSION)
        )
        self.assertFalse(
            openos_version.IsFullVersionWithSnapshotSuffix(
                self.VERSION_WITH_SNAPSHOT
            )
        )
        self.assertFalse(
            openos_version.IsFullVersionWithSnapshotSuffix(self.FULL_VERSION)
        )
        self.assertTrue(
            openos_version.IsFullVersionWithSnapshotSuffix(
