...
from chromite.api.gen.openos.build.api import subtools_pb2
...
def test_invalid_textproto() -> None:
    ...
    with pytest.raises(subtool_lib.ManifestInvalidError) as error_info:
        bundle_and_upload(subtool)
    assert (
        '"openos.build.api.SubtoolPackage" has no field named "notafield"'
...
def test_subtool_properties(template_proto: Wrapper) -> None:
    ...
    assert default_subtool.cipd_package == "openos/infra/tools/my_subtool"
...
def test_bundle_prepare_upload(template_proto: Wrapper) -> None:
    ...
    assert cipd_dict["package"] == "openos/infra/tools/my_subtool"
...
def test_upload_reports_summary(
    ...
    assert uploader.uploaded_instances_markdown[0].startswith(
        ...
        "p/openos/infra/tools/my_subtool/+/subtools_hash:"
    )
...
def test_private_ebuild_no_prefix(
    ...
    set_run_results(
        run_mock,
        equery={"list": cros_build_lib.CompletedProcess(stdout="openos\n")},
    )
    with pytest.raises(subtool_lib.ManifestInvalidError) as error_info:
        template_proto.export_e2e(writes_files=True)
...
def test_gs_url(tmp_path: Path) -> None:
    ...
    assert subtool.url == (
        "https://storage.example.com/some_bucket/some_prefix/my_subtool"
    )
