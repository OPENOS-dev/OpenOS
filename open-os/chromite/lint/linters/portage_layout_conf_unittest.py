...
masters = portage-stable openos eclass-overlay mobbase
...
def test_masters(monkeypatch) -> None:
    ...
    # Check valid values.
    assert not _get("portage-stable openos eclass-overlay")
    assert not _get("portage-stable openos eclass-overlay something else")
    assert not _get("portage-stable openos eclass-overlay foo foo-private")
    assert not _get("portage-stable openos eclass-overlay only-private")

    # Handle empty key.
    assert _get("")

    # Leading values is incorrect.
    assert _get("portage-stable eclass-overlay openos")

    # Reject duplicates.
    assert _get("portage-stable openos eclass-overlay openos")
    assert _get("portage-stable openos eclass-overlay a a")

    # Reject self-inclusion.
    assert _get("portage-stable openos eclass-overlay foo", "foo")

    # Reject private-then-public.
    assert _get("portage-stable openos eclass-overlay foo-private foo")

    # Reject private overlays listed without existing public overlay.
    assert _get("portage-stable openos eclass-overlay foo-private")
